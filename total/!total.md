...
| [[../gdn/paid-click-intent-receiver-access-result-20260506|paid_click_intent receiver 접근 검증 결과]] | Yellow Lane Preview 후속 실행 결과 | pass / HTTPS tunnel receiver verified | Google Ads landing-session 분모 분석과 GTM Production publish 승인안 작성 | 운영 publish 승인안 작성 후 close |
...
| Phase4 | [[#Phase4-Sprint6]] | 수집 개선 승인 패키지 | TJ+Codex | Preview/receiver 검증 통과 / 운영 publish 승인안 필요 | [[#Phase4-Sprint6\|이동]] |
...
- [ ] Google Ads landing-session 기준 click id 분모를 추가 분석한다. 무엇: 전체 주문 기준 0.8%와 별도로 Google Ads 랜딩 세션 기준 `gclid/gbraid/wbraid` 보존률을 계산한다. 왜: 전체 주문에는 다른 채널 주문이 섞여 있어 Google Ads 문제의 실제 크기를 판단하기 어렵다. 어떻게: GA4 BigQuery에서 Google Ads 유입 세션과 checkout/NPay intent/confirmed order 연결률을 본다. 산출물: 분모 분석 문서. 검증: read-only, platform send/write/publish 0건.
- [ ] GTM Production publish 승인안을 만든다. 무엇: `paid_click_intent v1`을 운영 GTM에 게시할지 판단할 문서다. 왜: Preview와 receiver는 통과했지만 운영 publish는 고객 사이트에 영향을 주는 단계다. 어떻게: publish 범위, rollback, TEST_/DEBUG_/PREVIEW_ 차단, no-send/no-write 유지, 24h/72h 모니터링 기준을 적는다. 산출물: 승인안. 검증: 승인 전 Production publish 0건.
