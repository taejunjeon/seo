...
|---|---|---|---|---|
| [[../gdn/google-click-id-preservation-plan-20260505|Google click id 보존률 개선 설계]] | 원인/전략. 전체 주문 기준 0.8%와 Google 후보 기준 50%를 분리해 해석 | active | Google Ads 랜딩 세션 기준 보존률을 추가 산출 | Production publish 후 v2 작성 |
| [[../gdn/paid-click-intent-gtm-preview-approval-20260506|paid_click_intent GTM Preview 승인안]] | Yellow Lane Preview only 승인안 | approved for Preview only / precheck passed | live v141 기준 fresh workspace Preview. 시작 직후 browser receiver 접근성 재확인 | Preview 결과 보고서 작성 후 close |
...
| 32 | [[#Phase4-Sprint6]] | Preview only 승인 / precheck 통과 | TJ+Codex | paid_click_intent GTM Preview 승인안을 만들고 실행 전 점검을 완료했다 | Google click id를 실제 전송 전에 Preview로만 보존 검증해야 한다 | 현재 GTM live latest는 `141 / pause_aw308433248_upde_20260505`로 read-only 확인했고, 로컬 7020 receiver CORS preflight도 `https://biocom.kr` 기준 통과했다. Preview는 v141 기준 fresh workspace에서만 진행한다 | [[#Phase4-Sprint6]] / [[../gdn/paid-click-intent-gtm-preview-approval-20260506]] | Preview 실행은 YES |
| 33 | [[#Phase2-Sprint4]] | 우리 기준 완료 | Codex | NPay rail source 공백을 분리했다 | publish 이전 NPay 주문을 unmatched나 Naver Ads 매출로 오해하면 안 된다 | [[../naver/npay-rail-source-gap-20260506]]에 2026년 4월 NPay 139건 중 publish 이전 126건 / 20,983,200원을 `source_unavailable_before_publish`로 기록하고, 표현은 `unknown_quarantine + payment_method=npay` 또는 `npay_payment_unattributed`로 고정했다 | [[#Phase2-Sprint4]] / [[../naver/npay-rail-source-gap-20260506]] | NO |
...
| 지금 막힌 것 | NPay source 접근, A급 GA4 중복 guard, 8건 payload 승인안, 운영 source confirmed purchase no-send dry-run, Google click id 보존률 진단, confirmed purchase no-send contract는 완료됐다. 남은 병목은 GTM/backend 운영 반영 전 preview 검증, 랜딩/체크아웃 시점 `gclid/gbraid/wbraid` 실제 보존률 개선, campaign mapping 미확정, Google Ads confirmed purchase 실제 실행 승인이다. |
...
- [ ] paid_click_intent GTM Preview를 실행한다. 무엇: 운영 publish 전에 `gclid/gbraid/wbraid`가 랜딩에서 storage와 `POST /api/attribution/paid-click-intent/no-send`까지 이어지는지 Preview로만 본다. 왜: 623건 중 5건만 Google click id가 남아 있어 Google Ads confirmed purchase의 1차 병목이다. 어떻게: [[../gdn/paid-click-intent-gtm-preview-approval-20260506]] 기준으로 현재 live `141 / pause_aw308433248_upde_20260505`에서 fresh workspace를 만들고 Preview만 실행한다. 시작 직후 browser network/console에서 local receiver 접근성을 다시 확인한다. 산출물: Preview 결과 캡처와 no-send receiver 응답. 검증: 실제 platform send/write/publish 0건, `TEST_` click id live 후보 차단.
