...
| [[../gdn/paid-click-intent-production-receiver-post-smoke-20260506|paid_click_intent production receiver POST smoke 결과]] | `att.ainativeos.net` receiver POST가 404임을 확인. receiver-enabled publish 전 backend route 배포 필요 |
| [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506|paid_click_intent production receiver 배포 승인안]] | backend 운영 deploy가 필요한 receiver route 배포 승인 문서 |
...
| [[../gdn/paid-click-intent-post-publish-monitoring-template-20260506|paid_click_intent post-publish 모니터링 템플릿]] | 운영 publish 후 24h/72h 확인 기준 | ready / pending receiver deploy and publish | publish 후 결과 문서에 사용 | publish 결과 문서 작성 후 close |
| [[../gdn/paid-click-intent-production-receiver-post-smoke-20260506|paid_click_intent production receiver POST smoke 결과]] | production receiver route 접근성 확인 | blocked / production route missing | receiver route 배포 여부 판단 | route 배포 후 재실행 |
| [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506|paid_click_intent production receiver 배포 승인안]] | backend 운영 deploy 승인 문서. no-write receiver는 live payload validation만 하며 DB/ledger write는 하지 않음 | pending TJ approval | 모드 A는 receiver deploy only, 모드 B는 TEST/negative smoke 통과 후 receiver-enabled GTM publish와 24h/72h 모니터링까지 연속 진행 | deploy/publish 결과 문서 작성 후 close |
...
|  36 | [[#Phase4-Sprint6]] | 차단 확인                  | Codex    | production receiver POST smoke를 실행했다                          | preflight 204만으로는 실제 receiver route 존재를 보장하지 않기 때문이다                                        | [[../gdn/paid-click-intent-production-receiver-post-smoke-20260506]] 기준 TEST_GCLID payload POST 결과 `404 Route not found`였다. Google Ads/GA4/Meta 전송은 없었다                                                                        | [[#Phase4-Sprint6]] / [[../gdn/paid-click-intent-production-receiver-post-smoke-20260506]]                                                    | NO              |
|  37 | [[#Phase4-Sprint6]] | 승인 대기                  | Codex+TJ | production receiver 배포 승인안을 만들었다                              | receiver-enabled GTM publish를 하려면 `att.ainativeos.net`에 no-send route가 먼저 있어야 한다            | [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506]]에 route 범위, no-write/no-send 조건, TEST POST 성공 기준, 금지선을 기록했다. 실제 backend 운영 deploy는 하지 않았다                                                             | [[#Phase4-Sprint6]] / [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506]]                                               | YES             |
|  38 | [[#Phase2-Sprint4]] | 우리 기준 완료               | Codex    | NPay rail source 공백을 분리했다                                     | publish 이전 NPay 주문을 unmatched나 Naver Ads 매출로 오해하면 안 된다                                      | [[../naver/npay-rail-source-gap-20260506]]에 2026년 4월 NPay 139건 중 publish 이전 126건 / 20,983,200원을 `source_unavailable_before_publish`로 기록하고, 표현은 `unknown_quarantine + payment_method=npay` 또는 `npay_payment_unattributed`로 고정했다 | [[#Phase2-Sprint4]] / [[../naver/npay-rail-source-gap-20260506]]                                                                              | NO              |
...
| 지금 막힌 것 | NPay source 접근, A급 GA4 중복 guard, 8건 payload 승인안, 운영 source confirmed purchase no-send dry-run, Google click id 보존률 진단, landing-session 분모 분석, confirmed purchase no-send contract, Preview/receiver 검증, 운영 publish 승인안 작성은 완료됐다. 추가로 production receiver POST smoke에서 `404 Route not found`가 확인됐다. 남은 병목은 TJ님의 receiver route 배포 승인/보류 판단, 이후 `paid_click_intent v1` 운영 GTM publish 승인/보류 판단, 운영 publish 후 실제 Attribution VM/order ledger fill-rate 모니터링, campaign mapping 미확정, Google Ads confirmed purchase 실제 실행 승인이다. |
...
- [x] production receiver TEST POST smoke를 실행했다. 검증: [[../gdn/paid-click-intent-production-receiver-post-smoke-20260506]], `att.ainativeos.net` POST route는 404. 플랫폼 전송 0건.
- [x] production receiver 배포 승인안을 만들었다. 검증: [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506]], route 배포 범위와 금지선 기록. 실제 backend 운영 deploy 0건.
- [x] TJ님 컨펌 대기열을 만들었다. 검증: [[total-confirmation-queue-20260505]].

#### 남은 것

- [ ] TJ님이 production receiver route 배포를 승인할지 보류할지 판단한다. 무엇: `att.ainativeos.net`에 `POST /api/attribution/paid-click-intent/no-send` route를 배포할지 결정한다. 왜: 현재 production POST smoke가 404라 receiver-enabled GTM publish가 실패한다. 어떻게: [[../gdn/paid-click-intent-production-receiver-deploy-approval-20260506]]의 승인 문구와 금지선을 확인한다. 검증: 승인 전 backend 운영 deploy 0건.
- [ ] TJ님이 `paid_click_intent v1` 운영 GTM publish를 승인할지 보류할지 판단한다. 무엇: 랜딩 시점 Google click id 저장 태그를 운영 사이트에 게시할지 결정한다. 왜: Preview/receiver/BigQuery 분모 분석은 통과했지만 운영 GTM publish는 고객 사이트 추적에 영향을 준다. 어떻게: receiver route 배포 후 receiver-enabled publish 또는 backend deploy 없이 storage-only publish 중 선택한다. 검증: 승인 전 Production publish 0건.
