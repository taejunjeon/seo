(function (w, d) {
  ...
  var VERSION = '2026-05-30-biocom-npay-bridge-gtm-v1-2-preview';
  var ENDPOINT = 'https://att.ainativeos.net/api/attribution/npay-intent';
  var DEBUG_KEY = '__seo_attribution_debug';
  var SOURCE = 'gtm_npay_bridge_v1_2';
  var LOG_PREFIX = '[biocom-npay-bridge-gtm-v1.2]';
  var MAX_TEXT_SCAN = 180000;
  var CHECKOUT_BRIDGE_SESSION_KEY = '__biocom_npay_checkout_bridge_v12';
  ...
  var lastCheckoutStage = '';
  var lastBridgeOpenedAt = '';
  var lastCheckoutOpenedAt = '';
  var lastLoginGateObservedAt = '';
  var lastOrderInitObservedAt = '';
  ...
  function readStorageText(storage, key) {
    try {
      if (!storage) return '';
      return trim(storage.getItem(key));
    } catch (error) {
      return '';
    }
  }
  ...
  function createLocalId(prefix) {
    try {
      if (w.crypto && typeof w.crypto.randomUUID === 'function') {
        return prefix + '_' + w.crypto.randomUUID();
      }
    } catch (error) {}
    return prefix + '_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 12);
  }

  function getOrCreateCheckoutBridgeId() {
    var existing = readStorageJson(w.sessionStorage, CHECKOUT_BRIDGE_SESSION_KEY);
    if (existing && trim(existing.id)) return trim(existing.id);

    var created = {
      id: createLocalId('npay_bridge'),
      created_at: nowIso(),
      version: VERSION
    };
    writeStorageJson(w.sessionStorage, CHECKOUT_BRIDGE_SESSION_KEY, created);
    return created.id;
  }
  ...
  function classifyCheckoutStage(url) {
    var value = trim(url).toLowerCase();
    if (!value) return '';
    if (value.indexOf('nid.naver.com') >= 0) return 'login_gate_possible';
    if (value.indexOf('/order/checkout/') >= 0) return 'checkout_opened_possible';
    if (value.indexOf('/order/bridge/') >= 0) return 'bridge_opened';
    if (value.indexOf('orders.pay.naver.com') >= 0 || value.indexOf('pay.naver.com') >= 0) return 'bridge_opened';
    return '';
  }

  function rememberCheckoutStage(stage) {
    stage = trim(stage);
    if (!stage) return;
    lastCheckoutStage = stage;
    var observedAt = nowIso();
    if (stage === 'bridge_opened' && !lastBridgeOpenedAt) lastBridgeOpenedAt = observedAt;
    if (stage === 'checkout_opened_possible' && !lastCheckoutOpenedAt) lastCheckoutOpenedAt = observedAt;
    if (stage === 'login_gate_possible' && !lastLoginGateObservedAt) lastLoginGateObservedAt = observedAt;
    if (pending) {
      pending.payload.checkout_stage = stage;
      pending.payload.bridge_opened_at = lastBridgeOpenedAt;
      pending.payload.checkout_opened_at = lastCheckoutOpenedAt;
      pending.payload.login_gate_observed_at = lastLoginGateObservedAt;
    }
  }

  function extractNpayCheckoutBridgeId(url) {
    var value = trim(url);
    if (!value) return '';
    try {
      var parsed = new URL(value, w.location.origin);
      var match = parsed.pathname.match(/\/order\/(?:bridge|checkout)\/mall\/([^/?#]+)/i);
  ...
  function rememberBridgeUrl(value, source) {
    ...
    rememberCheckoutStage(classifyCheckoutStage(url));
    ...
    if (pending && !pending.npayBridgeUrl) {
      ...
      pending.payload.npay_checkout_bridge_id = pending.payload.npay_checkout_bridge_id || extractNpayCheckoutBridgeId(url);
      if (pending.sent) {
    ...
    if (pending) {
      pending.payload.npay_checkout_bridge_id = pending.payload.npay_checkout_bridge_id || extractNpayCheckoutBridgeId(url);
      pending.payload.checkout_stage = lastCheckoutStage || pending.payload.checkout_stage;
      pending.payload.bridge_opened_at = lastBridgeOpenedAt || pending.payload.bridge_opened_at;
      pending.payload.checkout_opened_at = lastCheckoutOpenedAt || pending.payload.checkout_opened_at;
      pending.payload.login_gate_observed_at = lastLoginGateObservedAt || pending.payload.login_gate_observed_at;
    }
  ...
  function getCartSnapshot(productPrice) {
    var text = '';
    try { text = d.body && d.body.innerText || ''; } catch (error) { text = ''; }

    var largest = findLargestKrw(text);
    var delivery = 0;
    var discount = 0;
    var itemCount = 1;
    var quantityTotal = 1;
    ...
    try {
      var deliveryMatch = text.match(/(?:배송비|택배|배송)\s*[\n\s:+-]*([\d,]+)\s*원/i);
      if (deliveryMatch && deliveryMatch[1]) delivery = parseKrw(deliveryMatch[1]);
    } catch (error) {}

    try {
      var discountMatch = text.match(/(?:할인|쿠폰|적립금)\s*[\n\s:+-]*([\d,]+)\s*원/i);
      if (discountMatch && discountMatch[1]) discount = parseKrw(discountMatch[1]);
    } catch (error) {}

    try {
      var countMatch = text.match(/(?:상품금액\(총|총)\s*(\d+)\s*개/i);
      if (countMatch && countMatch[1]) itemCount = Math.max(1, Number(countMatch[1]) || 1);
    } catch (error) {}

    var subtotal = productPrice || 0;
    var expectedPayment = largest || productPrice || 0;
    if (delivery && productPrice && productPrice + delivery >= expectedPayment) {
      expectedPayment = productPrice + delivery - discount;
    }

    return {
      cart_item_count: itemCount,
      cart_quantity_total: quantityTotal,
      cart_subtotal_krw: subtotal,
      delivery_price_krw: delivery,
      discount_amount_krw: discount,
      expected_payment_amount_krw: expectedPayment,
      amount_source: largest ? 'visible_page_max_krw' : 'product_price'
    };
  }
  ...
  function buildPayload(event, bridgeUrl, triggerSource) {
    ...
    var cart = getCartSnapshot(productPrice);
    var bridgeId = getOrCreateCheckoutBridgeId();
    var stage = classifyCheckoutStage(bridgeUrl || lastBridgeUrl) || lastCheckoutStage || 'button_clicked';
    rememberCheckoutStage(stage);
    return {
      site: 'biocom',
      source: SOURCE,
      ...
      npay_checkout_bridge_id: bridgeId,
      local_session_id: readStorageText(w.sessionStorage, '__seo_funnel_session'),
      checkout_stage: stage,
      bridge_opened_at: lastBridgeOpenedAt,
      checkout_opened_at: lastCheckoutOpenedAt,
      login_gate_observed_at: lastLoginGateObservedAt,
      order_init_observed_at: lastOrderInitObservedAt,
      ...
      cart_quantity_total: cart.cart_quantity_total,
      cart_subtotal_krw: cart.cart_subtotal_krw,
      delivery_price_krw: cart.delivery_price_krw,
      discount_amount_krw: cart.discount_amount_krw,
      expected_payment_amount_krw: cart.expected_payment_amount_krw,
      amount_source: cart.amount_source,
      cart_fingerprint: [
        getQueryParam('idx'),
        getProductName(),
        productPrice,
        cart.cart_item_count,
        cart.cart_quantity_total,
        cart.cart_subtotal_krw,
        cart.delivery_price_krw,
        cart.discount_amount_krw,
        cart.expected_payment_amount_krw
      ].join('|'),
  ...
  function compactSendLog(payload) {
    return {
      ...
      hasCheckoutBridgeId: Boolean(payload.npay_checkout_bridge_id),
      checkoutStage: payload.checkout_stage,
      bridgeCaptureSources: payload.bridge_capture_sources,
      productIdx: payload.product_idx,
      productPrice: payload.product_price,
      expectedPaymentAmountKrw: payload.expected_payment_amount_krw
    };
  }
  ...
  function sendPayload(payload) {
    ...
    try {
      ...
      w.dataLayer.push({
        ...
        has_npay_checkout_bridge_id: Boolean(payload.npay_checkout_bridge_id),
  ...
  function sendBridgeUpdate(reason) {
    ...
    pending.payload.npay_checkout_bridge_id = pending.payload.npay_checkout_bridge_id || extractNpayCheckoutBridgeId(pending.payload.npayBridgeUrl);
    pending.payload.checkout_stage = lastCheckoutStage || pending.payload.checkout_stage;
    pending.payload.bridge_opened_at = lastBridgeOpenedAt || pending.payload.bridge_opened_at;
    pending.payload.checkout_opened_at = lastCheckoutOpenedAt || pending.payload.checkout_opened_at;
    pending.payload.login_gate_observed_at = lastLoginGateObservedAt || pending.payload.login_gate_observed_at;
    pending.payload.order_init_observed_at = lastOrderInitObservedAt || pending.payload.order_init_observed_at;
  ...
  function flush(reason) {
    ...
    pending.payload.npay_checkout_bridge_id = pending.payload.npay_checkout_bridge_id || extractNpayCheckoutBridgeId(pending.payload.npayBridgeUrl);
    pending.payload.checkout_stage = lastCheckoutStage || pending.payload.checkout_stage || 'button_clicked';
    pending.payload.bridge_opened_at = lastBridgeOpenedAt || pending.payload.bridge_opened_at;
    pending.payload.checkout_opened_at = lastCheckoutOpenedAt || pending.payload.checkout_opened_at;
    pending.payload.login_gate_observed_at = lastLoginGateObservedAt || pending.payload.login_gate_observed_at;
    pending.payload.order_init_observed_at = lastOrderInitObservedAt || pending.payload.order_init_observed_at;
  ...
  function onClick(event) {
    ...
    rememberCheckoutStage('button_clicked');
    ...
    try {
      ...
      var wrappedAjax = function () {
        ...
        if (isNpayOrderRequest) {
          lastOrderInitObservedAt = nowIso();
          rememberCheckoutStage('button_clicked');
          ensurePending('jquery_ajax_npay_add_order', null);
          if (pending) pending.payload.order_init_observed_at = lastOrderInitObservedAt;
            ...
            try {
              if (isNpayOrderRequest) {
                ...
                if (pending && result && typeof result === 'object') {
                  ...
                  pending.payload.imweb_order_code = trim(result.order_code);
                  pending.payload.channel_order_no = trim(result.channel_order_no || result.npay_order_no || result.order_no || '');
                  pending.payload.imweb_total_price = Number(result.total_price || 0) || 0;
                  if (pending.payload.imweb_total_price) pending.payload.expected_payment_amount_krw = pending.payload.imweb_total_price;
                  pending.payload.imweb_currency = trim(result.currency || '');
                  pending.payload.imweb_npay_result_observed = true;
                  pending.payload.order_init_observed_at = pending.payload.order_init_observed_at || nowIso();
                }
              }
            } catch (error) {}
        ...
        try {
          ...
          if (paymentType === 'npay' || paymentType.indexOf('npay') >= 0) {
            ...
            if (pending) {
              ...
              pending.payload.order_init_observed_at = lastOrderInitObservedAt;
