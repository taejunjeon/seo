...
| 순서 | 담당 | 할일 | 구체 내용 |
|---:|---|---|---|
| 1 | Codex | 첫 실험 랜딩을 자체 route로 만든다 | 무엇: `/shop_view?idx=25` 성격의 이벤트/체험권 랜딩을 Next.js route 1개로 만든다. 왜: 전체 홈페이지를 한 번에 옮기지 않고 고유입 랜딩 1개에서 리드 원장을 실운영 검증하기 위해서다. 어떻게: `frontend/src/app/aibio-native`의 폼/API 연결을 재사용하고, 기존 AIBIO 메뉴·CTA·UTM 수집을 붙인다. 산출물/검증: PC·모바일 Playwright screenshot과 `POST /api/aibio/native-leads` 저장 smoke 통과. |
| 2 | Codex | 아임웹 병행 대조 리포트를 운영자가 볼 수 있게 만든다 | 무엇: 자체 리드 수, 아임웹 fallback 수, 중복 후보, 누락 후보를 30일 window로 보여준다. 왜: 아임웹 입력폼을 끌지 말지 숫자로 판단해야 하기 때문이다. 어떻게: `GET /api/aibio/native-leads/fallback-comparison?rangeDays=30` 결과를 운영자 화면과 문서 표에 연결한다. 산출물/검증: 누락률·중복률·source·기준시각이 표시되고 빈 데이터일 때도 깨지지 않는다. |
| 3 | Codex | 운영자 리드 리스트에 담당자와 메모 저장 UI를 붙인다 | 무엇: 현재 상태 변경 중심 화면에 담당자, 메모, 예약일, 방문일 입력을 추가한다. 왜: 상담원이 실제로 쓰려면 “누가 처리 중인지”와 “다음 연락 내용”이 남아야 한다. 어떻게: 기존 `PATCH /api/aibio/native-leads/:leadId/status`에 준비된 assigned_to, note 계열 필드를 UI에서 전송한다. 산출물/검증: 저장 후 새로고침해도 담당자·메모가 유지된다. |
| 4 | Claude Code | 모바일 CTA와 폼 UX를 리뷰한다 | 무엇: 모바일 첫 화면 CTA, 전화번호 입력, 개인정보 동의, 성공/실패 문구를 점검한다. 왜: AIBIO 리드는 모바일 광고 유입 비중이 높고 폼 이탈이 매출에 바로 영향을 주기 때문이다. 어떻게: `/aibio-native`와 첫 실험 랜딩 screenshot을 기준으로 버튼 위치, 문구 길이, 오류 상태를 리뷰한다. 산출물/검증: 수정 제안 목록과 적용 우선순위가 남는다. |
| 5 | TJ | 리드 상태값과 운영 처리 규칙을 승인한다 | 무엇: 신규, 연락시도, 연락완료, 예약완료, 방문완료, 결제완료, 노쇼, 불량/중복의 이름과 의미를 확정한다. 왜: 상태 정의가 흔들리면 주간 퍼널과 상담원 처리율을 믿을 수 없다. 어떻게: 실제 상담원이 쓰는 표현과 맞지 않는 상태를 알려주고, 필요한 상태 추가/삭제를 승인한다. 산출물/검증: 운영 기준 상태값 v1 확정. 의존성: 부분병렬. |
| 6 | TJ | `AIBIO_NATIVE_ADMIN_TOKEN` 운영 secret을 확정한다 | 무엇: 원문 연락처 조회와 상태 변경에 쓸 관리자 token을 안전한 방식으로 정한다. 왜: 리드 목록에는 개인정보가 있어 공개 API처럼 열어둘 수 없기 때문이다. 어떻게: 운영 환경의 `.env` 또는 secret manager에만 저장하고 저장소에는 커밋하지 않는다. 산출물/검증: 운영 서버에서 관리자 API가 token 없이는 401, token 있으면 200을 반환한다. 의존성: 선행필수. |
| 7 | TJ + Codex | 30일 병행 운영 시작일과 Go/No-Go 기준을 확정한다 | 무엇: 자체 폼과 아임웹 폼을 같이 받을 시작일, 종료 후보일, 누락률 기준을 정한다. 왜: 감으로 아임웹을 종료하면 리드 누락 리스크가 생긴다. 어떻게: 시작일을 문서에 박고, 정상 저장률 99% 이상·누락률 3% 이하·상태 입력률 80% 이상 기준으로 매주 확인한다. 산출물/검증: `imweb/!imwebplan.md`와 운영 리포트에 기준일·window·source가 기록된다. |

## 10초 요약

AIBIO는 아임웹 관리자 전체를 복제할 필요가 없다. 2026-04-26 12:01 KST 기준 1순위는 홈페이지 전체 완성이 아니라 `자체 리드/예약 원장`이다. 공개 폼은 로컬 SQLite 원장 저장까지 연결했고, 운영자 화면은 실제 API 목록과 주간 퍼널을 읽는다. 운영 기준은 아직 0%다. 이유는 배포, 권한, 실제 상담원 사용, 아임웹 30일 병행 대조가 아직 남아 있기 때문이다.
...
| Phase3 | [[#Phase3-Sprint4]] | 폼/예약/리드 원장 | Codex | 65% / 0% | [[#Phase3-Sprint4\|이동]] |
...
| Phase6 | [[#Phase6-Sprint7]] | 운영 관리자/RBAC | Codex + Claude Code | 35% / 0% | [[#Phase6-Sprint7\|이동]] |
...
4. 결제는 예약금 또는 체험권 1개부터 토스페이먼츠 결제위젯/결제창으로 직접 붙일 수 있다.
5. 전체 쇼핑몰, 배송, 정기구독은 이번 범위에서 제외한다.
6. 초기 1~3개월은 아임웹 입력폼과 기존 운영 흐름을 fallback으로 유지한다.
...
1. [Codex] NestJS 적용 범위를 skeleton 문서로 만든다 — 무엇: `AuthModule`, `UsersModule`, `RbacModule`, `AibioModule`, `PaymentsModule`, `AttributionModule`의 책임을 정의한다. 왜: AIBIO가 전사 플랫폼 첫 모듈이 될 경우 인증·권한·결제 API가 흩어지면 나중에 다시 만들어야 하기 때문이다. 어떻게: 현재 Express 7020 route, AIBIO native lead API, Toss route를 모듈 단위로 분류한다. 산출물: NestJS module boundary 초안. 검증: 각 API가 Express 유지/신규 NestJS 후보 중 하나로만 분류된다.
2. [Codex] 기존 Express 7020 유지 범위를 확정한다 — 무엇: 이미 운영 검증 중인 `/api/aibio/native-leads` 계열은 단기 유지하고, 새 결제/RBAC는 NestJS 후보로 분리한다. 왜: 프레임워크 전환 때문에 Phase3 리드 원장 실운영이 늦어지면 안 되기 때문이다. 어떻게: endpoint별 사용 화면, 데이터 저장소, 테스트 여부를 표로 정리한다. 산출물: Express 유지표와 NestJS 이관 후보표. 검증: 리드 원장 운영화에 필요한 API가 중단되지 않는다.
...
2. [Codex] 첫 실험 랜딩 route를 만든다 — 무엇: `/shop_view?idx=25` 성격의 이벤트/체험권 랜딩을 자체 route 1개로 구현한다. 왜: 전체 홈페이지보다 작은 범위에서 광고 유입, CTA, 폼 저장을 검증하기 위해서다. 어떻게: 기존 `/aibio-native` 폼 컴포넌트와 attribution 수집 로직을 재사용하고 랜딩 copy/CTA만 분리한다. 산출물: Next.js route, CTA, native form 연결. 검증: 모바일/PC에서 폼 제출이 `/api/aibio/native-leads`에 저장된다.
3. [Codex] 기존 아임웹 URL redirect 초안을 작성한다 — 무엇: `/shop_view?idx=25`, `/59`, `/main`, `/bio_pulse_M`, `/56`을 자체 route 또는 아임웹 fallback으로 분류한다. 왜: 광고 소재와 검색 유입이 깨지면 리드가 끊기기 때문이다. 어떻게: 아임웹 통계의 인기 페이지와 로컬 문서의 폼 제출 URL을 대조한다. 산출물: redirect map v0.1. 검증: 각 URL에 source, 처리 방향, 운영 반영 전/후 상태가 적힌다.
4. [Claude Code] hero와 섹션 UX를 리뷰한다 — 무엇: 첫 화면 CTA, 섹션 순서, 폼 진입 버튼, 모바일 문구 길이를 점검한다. 왜: 랜딩의 목적은 설명이 아니라 상담 신청 전환이기 때문이다. 어떻게: Playwright screenshot 또는 로컬 브라우저 캡처를 기준으로 시각적 우선순위와 이탈 지점을 표시한다. 산출물: UX 수정 제안 목록. 검증: CTA가 첫 viewport에서 보이고 폼 진입까지 1번 클릭으로 가능하다. 의존성: 병렬가능.
5. [TJ] before/after 이미지와 후기 표현 사용 범위를 승인한다 — 무엇: 변화 후기, 전후 이미지, 감량 수치, 건강 표현의 사용 가능 범위를 결정한다. 왜: 의료·건강 표현은 광고 심사와 법적 리스크가 있기 때문이다. 어떻게: 실제 사용할 이미지·문구 후보를 보고 허용/수정/금지로 표시한다. 산출물: 표현 승인표. 검증: 승인 안 된 표현은 랜딩에 배포하지 않는다. 의존성: 부분병렬.
6. [Codex] Playwright PC/mobile screenshot으로 랜딩을 검증한다 — 무엇: desktop, mobile viewport에서 hero, CTA, 폼, 성공/실패 상태를 캡처한다. 왜: 디자인 모드에서 보이던 화면과 자체 구현 화면의 깨짐을 조기에 잡기 위해서다. 어떻게: frontend 7010에서 route를 열고 Playwright test를 실행한다. 산출물: screenshot과 테스트 로그. 검증: 텍스트 겹침, 폼 submit 실패, CTA 미노출이 없어야 한다. 의존성: 부분병렬.
...
| 운영자 리스트 연결 | `frontend/src/app/aibio-native/admin/AibioNativeAdmin.tsx` | mock 제거, API 연결 |
...
| 운영상태 | lead status, 담당자, 메모, 예약/방문/결제 필드 | 저장 필드 준비 |
...
4. [x] [Codex] phone hash 기반 아임웹 fallback 대조 API를 구현했다 — 무엇: 30일 window에서 native lead와 아임웹 fallback 후보를 비교할 API를 만들었다. 왜: 자체 폼 누락률과 중복률을 숫자로 봐야 아임웹 입력폼을 줄일 수 있기 때문이다. 어떻게: `GET /api/aibio/native-leads/fallback-comparison?rangeDays=30`을 추가했다. 산출물: 병행 대조 API. 검증: API가 empty state에서도 source/window를 반환한다.
5. [TJ] 실제 상담원이 쓰는 상태값을 최종 승인한다 — 무엇: 신규, 연락시도, 연락완료, 예약완료, 방문완료, 결제완료, 노쇼, 불량/중복의 이름과 의미를 확정한다. 왜: 상태가 현장 언어와 다르면 상담원이 입력하지 않아 퍼널 리포트가 무너진다. 어떻게: 상담원이 실제 쓰는 표현 기준으로 상태명을 유지/수정/삭제한다. 산출물: 리드 상태값 v1 승인. 검증: 승인된 상태만 admin UI에 노출한다. 의존성: 부분병렬.
6. [Claude Code] 모바일 고정 CTA와 폼 화면 구성을 리뷰한다 — 무엇: 모바일 하단 CTA, 전화번호 입력, 개인정보 동의, 에러/성공 문구를 점검한다. 왜: 광고 유입 사용자가 모바일에서 폼을 끝까지 제출해야 리드 원장이 쌓인다. 어떻게: `/aibio-native`와 첫 실험 랜딩 screenshot을 기준으로 UX 문제를 표시한다. 산출물: UX 수정 제안과 우선순위. 검증: CTA는 첫 화면 또는 고정 영역에서 보이고 입력 오류는 사용자가 이해할 수 있어야 한다. 의존성: 병렬가능.
7. [Codex] `/shop_view?idx=25` 성격의 고유입 랜딩 1개를 자체 폼으로 병행 운영한다 — 무엇: 첫 실험 랜딩의 CTA가 자체 native lead API로 저장되게 한다. 왜: 홈페이지 전체 완성 전에도 광고->랜딩->리드 원장 연결 효과를 검증하기 위해서다. 어떻게: 랜딩 route에 attribution capture와 native form submit을 붙이고 아임웹 입력폼은 fallback으로 유지한다. 산출물: 자체 랜딩 1개와 병행 리포트. 검증: 30일간 정상 저장률 99% 이상, 누락률 3% 이하를 본다. 의존성: 운영 배포 승인.
...
3. [Claude Code] 운영자 리스트/상세 화면을 설계한다 — 무엇: 리드 목록, 상세, 상태 변경, 담당자, 메모, 연락처 조회 버튼을 나눈다. 왜: 한 화면에 개인정보와 운영 메모가 섞이면 상담원이 실수하기 쉽다. 어떻게: `/aibio-native/admin` 기존 화면을 기준으로 리스트/상세 패널 구조를 제안한다. 산출물: admin UI 설계 또는 구현. 검증: 원문 연락처는 명시적 클릭과 권한 확인 후에만 보인다. 의존성: 병렬가능.
4. [TJ] 실제 운영진 계정과 역할을 확정한다 — 무엇: 운영자별 이메일, 역할, 개인정보 접근 가능 여부를 승인한다. 왜: 계정 부여와 개인정보 접근은 회사 운영·보안 판단이 필요한 TJ-only 작업이다. 어떻게: 상담원, 마케터, 관리자 명단을 role permission matrix에 맞춰 표시한다. 산출물: 운영 계정 승인표. 검증: 퇴사자/외부 계정이 포함되지 않는다. 의존성: 선행필수.
5. [Codex] audit log와 권한 회귀 테스트를 작성한다 — 무엇: 원문 연락처 조회, 상태 변경, 결제 조회에 actor, timestamp, action, lead id를 남긴다. 왜: 개인정보 접근과 상태 변경은 나중에 문제 추적이 가능해야 한다. 어떻게: API middleware 또는 service layer에서 audit event를 기록하고 unauthorized 케이스 테스트를 만든다. 산출물: audit log table/API test. 검증: 권한 없는 token은 401/403, 권한 있는 token은 audit row를 남긴다. 의존성: 부분병렬.
...
3. AIBIO 기능 인벤토리 보강
   ...
   - 네이버 예약/플레이스 사용 여부
   - 현재 삽입 코드 정본 재확인
4. NestJS skeleton 여부 결정
   - 리드 원장 실운영은 기존 Express 7020에서 먼저 검증
   - 전사 통합/RBAC/감사로그가 확정되면 NestJS `AibioModule`로 이관
