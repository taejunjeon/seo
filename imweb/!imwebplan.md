...
AIBIO는 아임웹 관리자 전체를 복제할 필요가 없다. 2026-04-26 11:00 KST 기준 1순위는 홈페이지 전체 완성이 아니라 `자체 리드/예약 원장`이다. 공개 폼은 로컬 SQLite 원장 저장까지 연결했고, 운영자 화면은 실제 API 목록과 주간 퍼널을 읽는다. 운영 기준은 아직 0%다. 이유는 배포, 권한, 실제 상담원 사용, 아임웹 30일 병행 대조가 아직 남아 있기 때문이다.
...
1. [Codex] lead schema 초안 확정 — 이름, 전화번호, 나이대, 상담 목적, 유입 경로, 동의, UTM. 완료.
2. [Codex] 자체 폼 DB 저장 API 구현. 완료.
3. [Codex] 운영자 리드 리스트 실제 API 연결. 완료.
4. [Codex] phone hash 기반 아임웹 fallback 대조 API 구현. 완료.
5. [TJ] 실제 상담원이 쓰는 상태값 최종 승인.
6. [Claude Code] 모바일 고정 CTA와 폼 화면 구성 리뷰. 의존성: 병렬가능.
7. [Codex] `/shop_view?idx=25` 성격의 고유입 랜딩 1개를 자체 폼으로 병행 운영. 의존성: 운영 배포 승인.
