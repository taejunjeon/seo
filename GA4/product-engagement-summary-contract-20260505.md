...
문서 성격: backend no-write contract. 실제 route 추가, deploy, DB write, GTM Preview는 하지 않는다.
...
먼저 내부 분석 장부에 `ProductEngagementSummary`라는 한 이벤트로 저장할 수 있게 contract만 고정한다.
...
- no-write dry-run contract만 확정한다.
- route 구현과 deploy는 별도 승인 전까지 하지 않는다.
- DB insert는 하지 않는다.
...
1. Codex: 이 contract를 기준으로 no-write route 구현안을 만든다. 왜: 실제 GTM Preview 전에 서버가 받을 값과 버릴 값을 자동 검증해야 한다. 어떻게: `dryRun=true` 응답만 하는 local route를 제안하고, PII reject unit test를 붙인다. 성공 기준: PII와 금액 필드가 들어오면 reject된다. 컨펌 필요: 구현은 NO, deploy는 YES.
...
3. Codex: 7~14일 내부 수집 후 주문 연결 분석 SQL을 준비한다. 왜: engagement가 실제 구매 확률과 관련 있는지 확인해야 Meta custom event 여부를 판단할 수 있다. 어떻게: `product_idx`, `ga_session_id`, `payment_success`를 조인하는 BigQuery/VM SQL 초안을 만든다. 성공 기준: 구매자/비구매자별 체류시간과 스크롤 분포가 나온다. 컨펌 필요: GA4 raw 권한은 YES.
