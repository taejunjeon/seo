...
### 2026-05-05 KST 운영 VM 접근 재확인

| 항목 | 결과 |
|---|---|
| 운영 backend `.env` 위치 | `/home/biocomkr_sns/seo/repo/backend/.env` |
| token 정책 | `NPAY_INTENT_ADMIN_TOKEN` 또는 `AIBIO_NATIVE_ADMIN_TOKEN`이 있으면 `GET /api/attribution/npay-intents` 조회 가능 |
| token 전달 방식 | `x-admin-token: <token>` 또는 `Authorization: Bearer <token>` |
| token 원문 처리 | 읽거나 출력하거나 문서화하지 않음 |
| VM 접근 계정 | `taejun` SSH + sudo read-only |
| 운영 SQLite 위치 | `/home/biocomkr_sns/seo/shared/backend-data/crm.sqlite3` |
| local snapshot | `backend/data/vm-npay-intent-20260505.sqlite3` |
| snapshot 검증 | SQLite integrity check PASS, `npay_intent_log` 823건 |
| dry-run 결과 | [[npay-roas-dry-run-vm-snapshot-20260505]] |

해석: 토큰은 운영 `.env`에 있고 API 조회에도 쓸 수 있지만, 토큰 원문을 로컬 문서나 대화에 노출할 필요는 없다. 이번에는 VM 계정 접근과 SQLite snapshot으로 같은 목적을 더 안전하게 달성했다.
...
- Source: VM SQLite `/home/biocomkr_sns/seo/shared/backend-data/crm.sqlite3`, table `npay_intent_log`
- Local snapshot: `backend/data/vm-npay-intent-20260505.sqlite3`
- Window: 2026-04-27 18:10 KST ~ 2026-05-06 00:00 KST dry-run
- Freshness: 최신 intent `2026-05-05 21:53 KST`
- Confidence: 92%
