...
- [x] [Codex] NPay 버튼 클릭 intent endpoint 연결 — 무엇: `POST /api/attribution/npay-intent`로 클릭 시점 정보를 저장한다. 왜: 결제 완료 후 biocom으로 자동 복귀하지 않아 client-side purchase가 빠질 수 있기 때문이다. 어떻게: GTM tag 118에서 `client_id`, `ga_session_id`, 상품 정보, 광고키를 beacon으로 보낸다. 산출물: `npay_intent_log`. 검증: 2026-04-30 기준 live intent 296건.
...
| 기준 | 상태 |
|---|---|
| 우리 기준 | 55%. read-only 매칭 로직, A/B 등급, ambiguous 원인 분해, BigQuery 조회 ID 목록까지 준비됐다. |
| 운영 기준 | 0%. DB `match_status` 업데이트, dispatcher, 광고 플랫폼 전송은 아직 열지 않았다. |
| 100%까지 남은 핵심 | 7일치 재실행, BigQuery `already_in_ga4` 확인, ambiguous 비율 축소 또는 전송 제외 확정, 운영 승인 패키지 작성. |

### 완료한 것

- [x] [Codex] confirmed NPay 주문 원장 read model 정의 — 무엇: `operational_postgres.public.tb_iamweb_users`에서 confirmed NPay 주문을 주문 단위로 읽는다. 왜: 구매 확정은 버튼 클릭이 아니라 주문 원장이 정본이기 때문이다. 어떻게: 결제수단 NPay 계열, 취소/환불/미입금 제외, `order_number` 기준으로 집계했다. 산출물: read-only order model. 검증: 2026-04-30 18:24 KST dry-run 기준 confirmed NPay 주문 11건.
...
- [ ] [Codex] 2026-05-04 18:10 KST 이후 7일치 dry-run 재실행 — 무엇: live publish 이후 정확히 7일 window로 같은 리포트를 다시 만든다. 왜: 현재 3일치 데이터만으로 dispatcher 후보 기준을 확정하면 표본이 작다. 어떻게: `NPAY_INTENT_DB_PATH=/home/biocomkr_sns/seo/shared/backend-data/crm.sqlite3 npm exec tsx scripts/npay-roas-dry-run.ts -- --start=2026-04-27T09:10:00.000Z --end=2026-05-04T09:10:00.000Z --format=markdown`로 실행한다. 산출물: 7일치 `npay-roas-dry-run` 리포트. 검증: confirmed NPay 주문 수, A급 strong 비율, ambiguous 비율, purchase_without_intent 비율이 표시된다. 의존성: 선행필수. 2026-05-04 18:10 KST 이후 가능.
- [ ] [Codex] BigQuery 권한 또는 결과가 들어오면 `already_in_ga4` guard 적용 — 무엇: A급 production 후보의 Imweb `order_number`와 NPay `channel_order_no`를 GA4 raw/purchase 전체에서 조회한 결과를 `--ga4-present`/`--ga4-absent`에 넣는다. 왜: 이미 GA4에 있는 주문을 Measurement Protocol로 다시 보내면 매출이 중복된다. 어떻게: BigQuery 결과를 쉼표 목록으로 CLI에 넣고 dispatcher dry-run 후보를 재계산한다. 산출물: `already_in_ga4=present/absent/unknown` 반영 리포트. 검증: BigQuery 미확인 주문은 전부 `unknown`으로 차단되고, 두 ID가 모두 absent인 주문만 후보가 된다. 의존성: 부분병렬. 권한 전까지는 목록 생성만 가능.
...
- [ ] [Codex] Phase2 승인 패키지 초안 작성 — 무엇: 7일치 결과와 BigQuery guard 결과를 바탕으로 Phase3 진입 여부를 요약한다. 왜: 실제 전송은 광고 플랫폼 신호를 바꾸므로 별도 승인이 필요하다. 어떻게: Go 기준 `A급 strong >= 50%`, `ambiguous <= 10%`, `purchase_without_intent <= 20%`, `already_in_ga4 확인률 100%`를 표로 판정한다. 산출물: Phase3 진입 승인안. 검증: TJ가 `YES/NO`로 답할 수 있는 추천안과 자신감 점수가 있다. 의존성: 7일치 리포트 후.
...
- [ ] [Codex] 7일치 기준 `clicked_no_purchase` 재집계 — 무엇: 현재 208건 분해를 7일 window로 다시 계산한다. 왜: 3일치 표본의 시간대/상품 편향을 그대로 운영 판단에 쓰면 위험하다. 어떻게: 2026-05-04 18:10 KST 이후 같은 CLI로 markdown 리포트를 재생성한다. 산출물: 7일 기준 상품/광고키/시간대 분해. 검증: `clicked_no_purchase / 전체 intent` 이탈률이 표시된다. 의존성: 선행필수.
- [ ] [Codex] 미결제 클릭의 운영 해석 표 작성 — 무엇: 상위 상품과 광고키별로 가능한 원인을 `가격/배송비`, `상품 상세`, `결제 UX`, `광고 유입 품질`로 나눈다. 왜: 숫자만 있으면 어떤 액션을 해야 할지 운영팀이 판단하기 어렵다. 어떻게: 상품별 가격, 배송비 포함 여부, 랜딩 URL, 광고키 조합을 함께 본다. 산출물: 원인 가설 표. 검증: 각 상위 항목에 `확정`, `유력 가설`, `추가 데이터 필요`가 붙는다. 의존성: 병렬가능.
...
| 기준 | 상태 |
|---|---|
| 우리 기준 | 20%. 어떤 조건에서 전환 복구를 열지 원칙과 금지 조건은 정리됐다. |
| 운영 기준 | 0%. GA4/Meta/TikTok/Google Ads purchase 전송은 아직 0건이다. |
| 100%까지 남은 핵심 | 7일 dry-run 통과, BigQuery 중복 guard 통과, GA4 MP 제한 테스트, Meta CAPI 제한 테스트, TikTok 식별값 보강, Google Ads 별도 승인. |
...
- [ ] [Codex] GA4 MP 제한 테스트 준비 — 무엇: A급 strong + `already_in_ga4=absent` + production_order만 GA4 MP 후보로 보는 테스트 설계를 만든다. 왜: 첫 전송은 GA4에서 중복/세션 귀속을 확인하기 위한 최소 범위여야 한다. 어떻게: 7일 dry-run 이후 absent 후보만 payload sample로 만든다. 산출물: GA4 MP 테스트 승인안. 검증: `already_in_ga4` 확인률 100%일 때만 승인 요청한다. 의존성: Phase2-Sprint3 완료 후.
...
| 기준 | 상태 |
|---|---|
| 우리 기준 | 30%. 매일 봐야 할 숫자, Go 기준, 다음 실행 순서는 문서화됐다. |
| 운영 기준 | 0%. 운영 대시보드/자동 알림/정기 리포트 배포는 아직 없다. |
| 100%까지 남은 핵심 | 7일치 리포트 고정, BigQuery guard 결과 반영, 운영자가 보는 요약 화면 또는 정기 markdown 템플릿 확정. |
...
- [ ] [Codex] 7일치 리포트 템플릿 확정 — 무엇: 2026-05-04 이후 매번 같은 항목이 나오는 markdown 템플릿을 고정한다. 왜: 사람이 매번 해석하면 느리고 기준이 흔들린다. 어떻게: 현재 `npay-roas-dry-run` 출력에 summary, BigQuery IDs, dispatcher preview, clicked_no_purchase breakdown을 유지한다. 산출물: 7일치 정식 리포트. 검증: Phase2 성공 기준 4개가 한 화면에서 보인다. 의존성: Phase2-Sprint3 7일 재실행.
