...
기준일: 2026-04-30
관련 문서: [[!npay|네이버페이 주문형 결제형 전환 검토]], [[npay-intent-quality-20260430|NPay Intent 수집 품질 점검]], [[npay-roas-dry-run-20260430]], [[npay-early-phase2-approval-20260430]], [[npay-ga4-mp-limited-test-approval]], [[GA4/gtm|biocom GTM 컨테이너 상태 정리]]
...
Freshness: dry-run report `2026-04-30 20:34 KST`, 분석 window end `2026-04-30 19:10 KST`, BigQuery robust query 결과 반영 `2026-04-30 20:36 KST`
Confidence: 90%
...
현재 버튼 클릭 intent 수집은 운영에서 작동한다. 2026-04-30 19:10 KST 기준 dry-run window 안의 live intent는 299건이고, 최근 점검 기준 `client_id`, `ga_session_id`, `product_idx` 채움률은 모두 운영 기준을 통과했다.
...
2026-04-30 20:34 KST에 현재까지 쌓인 데이터로 예비 dry-run을 다시 돌렸다. live intent 299건과 confirmed NPay 주문 11건을 read-only로 대조했고, strong match 8건을 A급 6건/B급 2건으로 나눴다. ambiguous는 3건, purchase_without_intent는 0건이다. A급 6건 중 1건은 TJ 수동 테스트 주문이라 전송 후보에서 제외한다.

TJ님이 A급 production 후보 5건의 Imweb `order_number`와 NPay `channel_order_no` 총 10개 ID를 BigQuery robust query로 확인했고, GA4 raw/purchase 전체에서 표시할 데이터 없음으로 나왔다. 그래서 이 5건은 `already_in_ga4=robust_absent`로 승격했다. dispatcher dry-run에서는 5건이 `send_candidate=Y` 후보로 보이지만, 실제 GA4/Meta/TikTok/Google Ads 전송은 여전히 0건이다.
...
| Phase2 | [[#Phase2-Sprint3]] | 실제 주문 매칭 | Codex | 78% / 0% | [[#Phase2-Sprint3]] |
| Phase2 | [[#Phase2-Sprint4]] | 미결제자 분리 | Codex | 55% / 0% | [[#Phase2-Sprint4]] |
| Phase3 | [[#Phase3-Sprint5]] | GA4/Meta/TikTok 전환 복구 | Codex + TJ | 40% / 0% | [[#Phase3-Sprint5]] |
| Phase3 | [[#Phase3-Sprint6]] | 운영 리포트와 승인 기준 | Codex + TJ | 70% / 0% | [[#Phase3-Sprint6]] |
...
| 바로 GA4/Meta/TikTok purchase를 보낼까 | NO | A급 후보는 나왔지만 실제 전송은 별도 승인 전까지 금지다 |
| 다음 1순위는 무엇인가 | GA4 MP 제한 테스트 승인 여부 결정 + 7일 후보정 | robust_absent 5건이 생겼으므로 1-2건 수동 제한 테스트 판단은 가능하지만, 자동 dispatcher는 아직 이르다 |
...
| A급 strong | 6건 |
| A급 production 후보 | 5건 |
| A급 production `robust_absent` | 5건 |
| B급 strong | 2건 |
| ambiguous | 3건 |
| 완전 미매칭 주문 | 0건 |
| dispatcher dry-run 후보 | 5건 |
| 실제 전송 | 0건 |
...
| GA4 MP 제한 테스트 승인안 | 진행 | 72시간 신선도 때문에 7일 대기만 하면 복구 가치가 낮아질 수 있다 | A급 + `already_in_ga4=robust_absent` + production_order만 후보로 승인안 작성 | TJ YES 전 실제 전송 금지 |
...
TJ님 BigQuery robust query 결과 A급 production 후보 5건은 `already_in_ga4=robust_absent`다. 그래서 dry-run 표에서는 `send_candidate=Y`로 보인다. 이 값은 "승인하면 보낼 수 있는 후보"라는 뜻이고, 실제 전송은 여전히 0건이다.
...
| 기준           | 상태                                                                                |
| ------------ | --------------------------------------------------------------------------------- |
| 우리 기준        | 78%. read-only 매칭 로직, A/B 등급, ambiguous 원인 분해, 수동 검토 큐, BigQuery robust guard 반영, dispatcher dry-run 후보 표시까지 완료했다. |
| 운영 기준        | 0%. DB `match_status` 업데이트, dispatcher, 광고 플랫폼 전송은 아직 열지 않았다.                     |
| 100%까지 남은 핵심 | GA4 MP 제한 테스트 승인 여부 결정, B급/ambiguous 제외 또는 규칙 보강 판단, 2026-05-04 18:10 KST 이후 7일치 후보정. |
...
- [ ] [TJ] GA4 MP 제한 테스트 승인 여부 결정 — 무엇: robust_absent A급 production 후보 5건 중 1-2건만 수동 전송 테스트를 할지 결정한다. 왜: 7일을 기다리면 GA4 MP 과거 이벤트/세션 귀속 품질이 떨어질 수 있지만, 전송은 되돌리기 어렵기 때문이다. 어떻게: [[npay-ga4-mp-limited-test-approval]]에서 `YES: 1건 제한 테스트` 또는 `NO: 7일 후보정까지 보류`로 결정한다. 산출물: 승인 또는 보류 결정. 검증: 승인 전까지 실제 GA4/Meta/TikTok/Google Ads 전송 0건.
...
| 기준 | 상태 |
|---|---|
| 우리 기준 | 40%. 전환 복구 원칙, payload preview, robust_absent guard, GA4 MP 제한 테스트 승인안까지 준비했다. |
| 운영 기준 | 0%. GA4/Meta/TikTok/Google Ads purchase 전송은 아직 0건이다. |
| 100%까지 남은 핵심 | TJ의 GA4 MP 제한 테스트 승인 여부 결정, 전송 후 검증 쿼리, 7일 후보정, Meta CAPI 제한 테스트, TikTok 식별값 보강, Google Ads 별도 승인. |
...
- [x] [Codex] GA4 payload preview 초안 작성 — 무엇: `NPayRecoveredPurchase_{order_number}` event_id와 client/session/value 후보를 리포트에 표시한다. 왜: 전송 전에 payload 품질을 눈으로 확인해야 한다. 어떻게: dispatcher dry-run log에 preview만 만들고 실제 endpoint 호출은 하지 않았다. 산출물: `Dispatcher Dry-run Log`. 검증: `send_candidate=Y` 후보는 5건이지만 실제 전송은 0건이다.
...
- [ ] [TJ] GA4 MP 제한 테스트 YES/NO 결정 — 무엇: 1건만 보낼지, 2건까지 보낼지, 아니면 7일 후보정까지 보류할지 결정한다. 왜: 실제 GA4 purchase 신호가 생기고 롤백이 제한되기 때문이다. 어떻게: [[npay-ga4-mp-limited-test-approval]]의 추천안 A/B 중 하나를 선택한다. 산출물: 명시 승인 또는 보류. 검증: 승인 전까지 실제 전송 0건.
- [ ] [Codex] 승인 시 GA4 MP 수동 제한 전송 스크립트 작성 전 검토 — 무엇: 승인된 주문 1-2건만 보낼 수 있는 one-off sender 설계를 만든다. 왜: 자동 dispatcher를 열지 않고 사람이 승인한 주문만 보내야 한다. 어떻게: dispatch key `npay_recovery_ga4_purchase:{site}:{order_number}`와 BigQuery guard를 전송 직전 다시 확인하도록 한다. 산출물: dry-run 가능한 sender 초안. 검증: 실제 send 모드는 TJ 승인 후 별도 실행.
...
| 기준 | 상태 |
|---|---|
| 우리 기준 | 70%. 매일 봐야 할 숫자, Go 기준, BigQuery robust guard 반영 리포트, GA4 MP 제한 테스트 승인안, 7일 후보정 리포트 템플릿을 분리했다. |
| 운영 기준 | 0%. 운영 대시보드/자동 알림/정기 리포트 배포는 아직 없다. |
| 100%까지 남은 핵심 | TJ의 GA4 MP 제한 테스트 승인 여부 결정, 7일 후보정 결과 반영, 운영자가 보는 요약 화면 또는 정기 markdown 배포 여부 확정. |
...
- [x] [Codex] BigQuery guard 결과 반영 리포트 작성 — 무엇: TJ robust query 결과를 `robust_absent` 상태로 반영했다. 왜: `unknown` 상태에서는 제한 테스트 후보가 0건이기 때문이다. 어떻게: 10개 ID를 `--ga4-robust-absent`로 넣고 dry-run을 재실행했다. 산출물: [[npay-roas-dry-run-20260430]] dispatcher dry-run log. 검증: `send_candidate=Y` 5건, 실제 전송 0건.
...
- [ ] [TJ] Phase3 제한 테스트 승인 여부 결정 — 무엇: 7일 리포트 전이라도 A급 robust_absent 후보 1-2건만 GA4 MP로 보낼지 결정한다. 왜: 실제 광고 플랫폼 신호를 바꾸는 결정이고 롤백이 제한되기 때문이다. 어떻게: [[npay-ga4-mp-limited-test-approval]]에 `YES` 또는 `NO: 보류`로 답한다. 산출물: 전송 승인 또는 보류 결정. 검증: 승인 전까지 운영 전송 0건. 의존성: 선행필수.
...
|---:|---|---|---|---|---|
| 1 | Codex | 현재 표본 승인 패키지를 만든다 | 이미 299 intent/11 주문이 있어 조기 판단이 가능하다 | A급 후보, robust_absent guard, ambiguous, clicked_no_purchase를 한 장으로 요약 | 현재 표본 승인안 |
...
| 4 | TJ | GA4 MP 제한 테스트 여부를 승인한다 | 7일 대기만 하면 과거 이벤트/세션 귀속 품질이 떨어질 수 있다 | `YES: 1건 제한 테스트` 또는 `NO: 7일 후보정까지 대기`로 결정 | 단계별 승인 |
...
자신감은 86%다. 버튼 클릭 intent 수집 품질은 이미 운영에서 통과했고 현재 299 intent/11 주문 표본으로 A급 후보와 robust_absent 5건까지 나왔기 때문에 제한 테스트 승인 판단은 가능하다. 다만 ambiguous 비율이 아직 높고 confirmed 주문이 11건뿐이므로, 자동 dispatcher는 바로 열지 않는 것이 맞다.
...
| 선택지 | 의미 | Codex 추천 |
|---|---|---|
| YES: GA4 MP 1건 제한 테스트 + 7일 후보정 | robust_absent A급 production 후보 중 1건만 수동 제한 전송하고, 7일치로 기준을 보정한다 | 추천 |
| NO: 7일 후보정까지 완전 대기 | 2026-05-04 이후에만 다음 판단을 한다 | 비추천 |

Codex 추천 답변은 아래다.

```text
YES: GA4 MP 1건 제한 테스트 + 7일 후보정
```
