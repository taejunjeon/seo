...
- 최종 결과보고는 짧은 완료 문장으로 끝내지 말고 최소한 `한 줄 결론`, `완료한 것`, `하지 않은 것`, `검증 결과`, `현재 영향/서버·커밋 상태`, `남은 리스크`, `확인하면 좋은 문서`, `다음 할일`을 포함하시오. 단순 작업이라도 운영 영향, 외부 전송 여부, 커밋/푸시 여부, 테스트 실행 여부는 생략하지 마시오.
- 결과보고에서 TJ님이 확인해야 할 문서가 있으면 `내가 왜 봐야 하는지`와 함께 최대 3개까지 우선순위로 링크하시오. 불가피하게 3개를 넘으면 최대 5개까지만 제시하고, 확인할 문서가 없으면 `TJ님이 별도로 확인할 문서 없음`이라고 명시하시오.
...
- 결과보고는 기술 작업명을 먼저 던지지 말고, 사람이 이해하는 행동과 결정으로 번역하시오. 예를 들어 `offline conversion import 기준 confirmed_purchase 전환`이라고만 쓰지 말고, `Google Ads에 실제 결제완료 주문만 구매로 알려주는 새 전환 통로`라고 먼저 설명한 뒤 괄호로 기술명을 붙이시오.
- 결과보고에서 영어/플랫폼 용어를 쓸 때는 처음 1회 반드시 쉬운 풀이를 붙이시오. 예: `Primary 전환=Google Ads가 입찰 학습에 쓰는 핵심 구매 신호`, `Secondary 전환=입찰에는 안 쓰고 관찰만 하는 보조 신호`, `Red Lane=실제 외부 계정/운영 설정을 바꾸므로 TJ님 승인 전 중지`.
- TJ님에게 승인이나 확인을 요청할 때는 `내가 실제로 누를 화면`, `바꾸는 설정 이름`, `바꾸면 생기는 효과`, `안 바꾸면 남는 문제`를 먼저 적고, API/DB/스크립트 이름은 뒤에 근거로 적으시오. 사용자가 “그래서 내가 뭘 하라는 건지” 모르는 표현은 실패한 보고로 보시오.
- 결과보고에는 같은 숫자를 최소 두 이름으로 분리해 쓰시오. 예: `Google Ads ROAS=광고 플랫폼이 주장하는 값`, `내부 confirmed ROAS=실제 결제완료 주문 원장 기준값`. 플랫폼 주장값과 내부 매출값을 한 문단에서 섞을 때는 “예산 판단에 쓸 값/참고만 볼 값”을 명시하시오.
- NPay/카드/가상계좌처럼 결제수단이 섞인 전환 설명에서는 `클릭`, `결제 시작`, `결제 완료`, `구매 확정/취소 반영`을 구분하시오. `NPay가 나쁘다`처럼 쓰지 말고, `NPay 클릭을 구매완료로 세는 것이 문제이고, 실제 결제완료 NPay 매출은 포함해야 한다`처럼 행동과 기준을 분리하시오.
...
- “다음 할일”이 여러 단계라면 각 단계마다 `의존성`도 반드시 적으시오. 앞 단계가 끝나야 가능한 일인지, 독립적으로 병렬 진행 가능한 일인지, 또는 특정 데이터/승인/모니터링 결과가 필요한 일인지 명시하시오. 의존성이 없고 Green Lane이면 다음 보고로 미루지 말고 바로 실행하시오.
...
- 이미 승인된 Yellow Lane은 다시 `다음 승인 요청`으로 돌리지 말고 승인 범위 안에서 실행·검증·cleanup·결과보고까지 진행하시오. 실행이 막히면 승인 부족이라고 쓰지 말고 `접근 권한`, `계정/2FA`, `브라우저/CORS`, `데이터 부족`, `기술 실패` 중 어디서 막혔는지 특정하고, TJ님이 해야 할 최소 액션과 Codex가 계속할 대체 작업을 함께 보고하시오.
- HOLD 결과가 나오면 바로 TJ님 승인 대기로 넘기지 마시오. 먼저 HOLD 원인을 분류하고, read-only 조사, dry-run, 문서 보강, 로컬 테스트, 검증 스크립트처럼 Green Lane으로 줄일 수 있는 follow-up을 자동 수행한 뒤 남은 Yellow/Red/권한/사업 판단만 TJ님에게 넘기시오.
