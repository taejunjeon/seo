...
단, 이 숫자는 최종 예산 판단값이 아니라 강한 경보값이다. 한국어 export의 중복 헤더를 구매값으로 추정했기 때문에 Ads Manager 화면에서 구매값 metric을 한 번 대조해야 한다.

## Source 분류와 pending audit

2026-04-18 13:33 KST에 `/api/ads/tiktok/roas-comparison`에 source 분류 사유와 pending 상위 20건 audit을 추가했다.

분류 사유:

- high: `ttclid_direct`, `ttclid_url`, `metadata_ttclid_url`
- medium: `utm_source_tiktok`, `utm_campaign_tiktok`, `utm_medium_tiktok`, `utm_content_tiktok`, `utm_term_tiktok`
- low: `landing_tiktok`, `referrer_tiktok`, `metadata_url_tiktok`

현재 API 기준으로 TikTok payment_success 대부분은 `ttclid_direct`와 TikTok UTM을 함께 가진다. 따라서 단순 URL 텍스트만으로 넓게 잡힌 케이스라기보다는, 적어도 표본상 TikTok 귀속 근거가 강한 편이다.
...
|---|---:|
| audit 행 | 20 |
| 최상위 pending 주문 | `202604052259913` |
| 최상위 pending 금액 | 260,100,000원 |
| 최상위 pending 분류 | high (`ttclid_direct`, `ttclid_url`, TikTok UTM) |

다음 단계는 같은 기간 내부 Attribution/Toss 원장에서 TikTok 귀속 주문을 뽑아 다음 값을 사람 검수와 함께 확인하는 것이다.

1. Toss `DONE` 확정매출
2. Toss `WAITING_FOR_DEPOSIT` pending 금액
3. TikTok 플랫폼 추정 구매값 910,630,953원과의 gap
4. pending 상위 20건의 실제 fate: 미입금 유지, 후속 입금, 취소, 오분류 여부
