 ...
 * Version: 2026-04-17.tiktok-purchase-guard-dry-run.v2
 *
 * Dry-run only:
...
(function (w) {
  ...
  var VERSION = "2026-04-17.tiktok-purchase-guard-dry-run.v2";
  ...
  function log(label, payload) {
    if (!CONFIG.debug || !w.console || typeof w.console.log !== "function") return;
  ...
  function wrapTrack(owner, ownerName, stateKey) {
    ...
    if (owner.track[WRAP_KEY]) {
      state.wrappers[stateKey] = true;
      return true;
    }

    var originalTrack = owner.track;
  ...
  function wrapTiktokPixelInit(pixel) {
    ...
    state.wrappers.tiktokPixelInit = true;
    log("wrapped_TIKTOK_PIXEL_init", { version: VERSION });
    return true;
  }
  ...
  function wrapTtq() {
    return wrapTrack(w.ttq, "ttq", "ttqTrack");
  }
