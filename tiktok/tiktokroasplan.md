...
작성 시각: 2026-04-26 09:27 KST
기준일: 2026-04-26
버전: v3.15-ga4-crosscheck (이전본: `tiktok/tiktokroasplan.md.bak_20260426_ga4_crosscheck`)

## 2026-04-25 최신 상태

### 2026-04-26 내부 confirmed 0원 재해석과 GA4 교차검증

질문은 “TikTok 내부 confirmed가 계속 0원인데, 실제 매출이 없다는 뜻인가?”였다. 결론은 아니다. 현재 0원은 **운영 Attribution 원장의 strict TikTok confirmed** 정의가 0원이라는 뜻이고, GA4 session-source 기준으로는 TikTok 계열 구매가 별도로 보인다.
...
| 운영 strict TikTok confirmed | 0건 / 0원 |
| 운영 ledger 전체 payment_success | 461행 |
| 운영 ledger confirmed payment_success | 399행 |
| 운영 ledger TikTok 직접근거 payment_success | 0행 |
...
| GA4 숫자형 transaction_id 중 운영 confirmed 매칭 | 41건 / 11,894,430원 |
| GA4 숫자형 transaction_id 중 운영 canceled 매칭 | 1건 / 39,000원 |
| GA4 숫자형 transaction_id 중 운영 ledger 미조인 | 5건 / 2,502,000원 |
| 운영 confirmed 매칭 41건 중 ledger TikTok 직접근거 | 0건 |
| 운영 confirmed 매칭 41건 중 다른 채널/무관 UTM 근거 | 41건 |

해석:

- `internal confirmed = 0원`은 “바이오컴 결제완료가 없다”가 아니다. 운영 VM Attribution 원장에는 같은 기간 결제완료가 충분히 들어오고 있다.
- 문제는 TikTok 광고 유입 신호가 `payment_success` receiver에 보존되지 않는 것이다. `ttclid`, TikTok UTM, TikTok landing/referrer가 운영 ledger에 남지 않으므로 strict TikTok confirmed가 0원으로 계산된다.
- GA4 기준으로는 TikTok session source 구매가 64회 존재한다. 이 중 41건은 운영 ledger의 confirmed 주문번호와 금액까지 맞는다.
- 그러나 그 41건의 운영 ledger UTM/landing은 Meta, Naver, CRM, coupon, direct성 경로 등으로 남아 있다. 따라서 GA4 수치 11,894,430원을 TikTok high-confidence confirmed로 바로 승격하면 다른 채널 매출을 빼앗을 위험이 있다.
...
| 지표 | 위치 | 신뢰 등급 | 사용 방식 |
|---|---|---|---|
| strict TikTok confirmed | 운영 VM Attribution ledger | high | 예산 증액/확정 ROAS의 1차 기준 |
| GA4 TikTok transaction match | GA4 Data API + 운영 VM ledger join | medium | “TikTok 가능성” 검산, 0원 오해 방지 |
...
| 내부 확정매출 기준 | **운영 VM Attribution 원장** `https://att.ainativeos.net/api/attribution/ledger` | 실제 TikTok 후보 결제완료 이벤트 기준 |
| 주문 상태 검산 | **운영 PostgreSQL** `public.tb_iamweb_users` | 가상계좌 만료, 취소, 결제완료 상태 정본 |
...
| 채널 | 기준 | 최근 확인값 | 해석 |
|---|---|---:|---|
| TikTok | 로컬 TikTok Ads API 캐시 + 운영 VM Attribution 원장 | 광고비 6,190,363원 / TikTok 주장 구매값 4,493,392원 / 내부 confirmed 0원 | 플랫폼 주장은 있으나 내부 결제완료 귀속 증거가 없다 |
| Meta | 운영 VM `/api/ads/roas`, `last_7d` | 광고비 29,167,748원 / Attribution 매출 42,221,199원 / ROAS 1.45 | 과대 가능성은 별도 검산 대상이지만 내부 주문 귀속이 존재한다 |
...
| 운영 VM Attribution 원장 confirmed | 0건 / 0원 |
...
| 항목 | 값 | 의미 |
|---|---:|---|
| 운영 VM Attribution 원장 전체 row | 1,474행 | 원장 자체는 비어 있지 않다 |
...
1. 운영 VM Attribution 원장 `https://att.ainativeos.net/api/attribution/ledger`에서 가져온다.
...
| 원인 후보 | 현재 판정 | 근거 |
|---|---|---|
| 운영 VM 원장이 죽어서 0원 | 가능성 낮음 | 최근 7일 원장 1,474행, payment_success 406행 확인 |
...
> TikTok Ads는 17건 / 4,493,392원의 구매를 주장한다. 그러나 운영 VM Attribution 원장 기준으로 TikTok 직접 근거가 붙은 payment_success confirmed는 0건 / 0원이다. v2 event log는 Purchase 허용/차단은 검증하지만 광고 귀속 증거는 아니므로, TikTok 주장 구매를 내부 확정매출로 인정하기에는 아직 부족하다.
...
- 당시 남은 핵심 리스크: 운영 배포 전이므로 `tiktok_pixel_events`는 아직 운영 VM에 없었다. v2 배포가 늦어질수록 post-guard 데이터도 계속 exact 연결 불가 상태로 쌓이는 구조였다.
...
1. **[운영 확인] 2026-04-24 가상계좌 테스트 주문의 24시간/48시간 상태를 확인한다.**
   ...
   - 어떻게 하는가: 테스트 주문번호 `202604247459692`와 `202604248221933`를 운영 주문 DB, 운영 VM Attribution 원장, v2 `tiktok_pixel_events`에서 각각 조회한다. 2026-04-25 23:33 KST 이후 24시간 체크, 2026-04-26 23:33 KST 이후 48시간 체크를 한다.
   - 원장/DB 위치: 주문 상태 정본은 **운영 PostgreSQL** `public.tb_iamweb_users`, 이벤트 흐름은 **운영 VM SQLite** `#tiktok_pixel_events`, 화면 캐시는 **내 맥북 로컬 SQLite** `#tiktok_ads_daily`다.
...
2. **[완료 후 관찰] `/ads/tiktok`에서 2026-04-18 ~ 2026-04-24 7일 구간을 본다.**
   ...
   - 어떻게 하는가: `http://localhost:7010/ads/tiktok`에서 기간을 `2026-04-18 ~ 2026-04-24`로 둔다. 플랫폼 주장값은 TikTok Ads API, 내부 확정매출은 운영 VM Attribution 원장 기준이다.
   - 원장/DB 위치: 광고 데이터는 **내 맥북 로컬 SQLite** `backend/data/crm.sqlite3#tiktok_ads_daily`다. 내부 확정매출 비교는 **운영 VM Attribution 원장**을 read-only로 조회한다.
...
4. **[예산 판단] 소재 교체/일시중단 판단은 내부 confirmed가 0원인 점을 중심에 둔다.**
   - 왜 하는가: 최근 7일 TikTok은 구매 17건을 주장하지만, 운영 VM Attribution 원장 기준 confirmed는 0건 / 0원이다. 이 상태에서 TikTok 주장 ROAS만 보고 증액하면 판단이 흔들린다.
   ...
   - 원장/DB 위치: 예산 판단의 내부 기준은 **운영 VM Attribution 원장**과 **운영 PostgreSQL 주문 DB**다. TikTok Ads API는 플랫폼 주장값으로만 쓴다.
