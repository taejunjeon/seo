...
최근 업데이트: 2026-04-28 KST (UX 2차 고도화 + GSC P0 작업 진수)  
...
상단에 본문 최상단 sticky `TopDecisionBox`가 항상 떠 있음 (B/C 결정 카드 + 답변 한 클릭 복사). 사이드바 + 본문 스크롤과는 독립.

| # | 섹션 ID | 메뉴명 | 무엇을 보여주는가 | 운영 영향 | 데이터 소스 |
|---|---|---|---|---|---|
| 0 | (sticky) | 오늘 TJ 결정 2개 | B/C 결정 박스 (운영 반영 아님 명시) + YES/NO 결과물 | 결정 게이트 | hardcoded |
| 1 | `overview` | 종합 점수 | 54/100, 6항목별 「이게 뭐/왜 이 점수/어떻게 올려요」, 5문제 「왜/영향/해결」 | 읽기 전용 | `seo_audit_summary.md`, `page_seo_audit.csv` |
| 2 | `live-gsc` | 지금 검색 성과 | GSC 7일 KPI 4종 + sparkline + 활용 안내 | 실시간 데이터 | 백엔드 `/api/gsc/kpi` |
| 3 | `baseline` | 성과 기준선 | 7/28/90일 클릭/노출/CTR/순위 + 검색어/페이지 TOP5 비교 | 실시간 데이터 | 백엔드 `/api/gsc/query` × 9 |
| 4 | `canonical-check` | 대표 URL 검증 | 시범 6개 묶음별 GSC 노출 분포로 「같은 상품이 여러 URL로 흩어졌는가」 자동 판정 | 실시간 데이터 | 백엔드 `/api/gsc/query` (page 차원 28일) |
| 5 | `url-policy` | URL 종류별 처리 기준표 | 유형별 정책 매트릭스, 중복 그룹, URL 인벤토리 (기본 「문제만」 모드) + 4종 쉬운말 Glossary | 읽기 전용 | `url_policy_matrix.csv`, `duplicate_url_groups.csv`, `url_inventory.csv` |
| 6 | `jsonld` | 검색엔진 설명서 코드 | 페이지 6개 권장 schema, 5개 스니펫 (각 「뭐/왜/어디」), 검증 도구 안내. 게시는 별도 승인 | 읽기 전용 (게시는 별도 승인) | `jsonld_validation_matrix.csv`, `jsonld_insertion_snippets.md` |
| 7 | `product-text` | 상품 텍스트 초안 | 상품 4개 H1/H2/H3/FAQ + 모바일 미리보기 + 적용 원칙 | 제안서 생성 | `product_text_block_matrix.csv`, `drafts.md` |
| 8 | `aeo-explainer` | AEO 확장 | AI 검색 최적화가 뭐/왜/파급력 + 질문형 패턴 6종 + 3단계 작업 흐름 (운영 영향 단계별 명시) | 읽기 전용 | hardcoded |
| 9 | `checklist` | 운영 체크리스트 | 운영 전 6단계 + 각 게이트 설명, 운영 반영 7단계 + 단계별 「뭘 바꾸는지」, 롤백 | 운영 반영 필요 | `operation_change_checklist.md`, `action_plan.csv` |
| 10 | `approvals` | 승인 현황 | A/B/C 카드 (이게 뭐/지금 상태/왜 결정 필요/✅ YES 결과물/⚠️ NO 영향/답변 복사) + 별도 승인 5개 | 결정 게이트 | hardcoded + `!seoplan.md` |
...
- canonical 검증 카드의 매처를 reports/seo/url_inventory.csv 자동 기반으로 확장 (현재는 시범 6개 하드코딩)
- 시범 페이지 4개의 GSC 시계열을 product-text 카드 옆에 부착 (텍스트 블록 추가 전후 비교)
- 질문형 query 자동 추출 → AEO 카드 안에 「칼럼별 FAQ 후보 표」 자동 채움 (TJ가 「AEO 진행해」 회신 시)
- 운영 반영 baseline/포스트 7/14/28일 비교를 운영 체크리스트에 자동 부착
- TopDecisionBox에 「조건 수정」 버튼 → 모달로 부분 NO 답변 빌드 (예: B는 YES + 리뷰/검색 noindex 보류)
