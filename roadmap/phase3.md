...
**화이트리스트 운영:**
- 현재 `01039348641`만 허용 (테스트용)
- ✅ consent 데이터 확보 완료 (83,017명) → contact policy 자동 판정으로 전환 가능
- 다음 단계: 화이트리스트 해제 → consent 기반 발송 대상 자동 선별

---

#### 트랙 C: 수신 동의(Consent) 데이터 소스 확보 — ✅ 완료 (0402~0403)

**목표**: 고객의 마케팅 수신 동의 상태를 확인할 수 있는 데이터 소스 확보

**결과**: 아임웹 API로 3사이트 83,017명 회원 consent 동기화 완료.

**구현 내역**:
- 아임웹 `GET /v2/member/members` API로 직접 조회 가능 확인 ✅
- 3사이트 회원 총 83,017명 sync: 바이오컴 69,681 + 더클린커피 13,236 + AIBIO 100
- consent 필드: `marketing_agree_sms`(47.5%), `marketing_agree_email`(44.8%), `third_party_agree`(73.8%)
- SQLite `imweb_members` 테이블에 적재, `callnum`/`marketing_agree_sms`/`site` 인덱스
- 상세 문서: `imweb/memberagree.md`

**구현된 API:**
- `POST /api/crm-local/imweb/sync-members` — 3사이트 전체 sync (3~10분)
- `GET /api/crm-local/imweb/consent-stats` — 집계 통계
- `GET /api/crm-local/imweb/consent-check` — 전화번호로 개별 조회
- `POST /api/contact-policy/evaluate` — **consent 자동 조회 연동 완료** (전화번호만 넘기면 imweb_members에서 자동 lookup)

**운영 방침 전환**:
- ~~화이트리스트 + admin override로 소규모 발송~~ → consent 기반 자동 판정으로 전환
- 정보성 알림톡: consent 불필요 (법적 허용)
- 홍보성 메시지: `marketing_agree_sms = Y` 필수 (39,440명 대상 가능)
...
| consent 데이터 소스 | ✅ **83,017명 sync 완료 (0403)** | ✅ 달성 |
...
| **C. Consent 소스** | 1개 확보 | ✅ **아임웹 API → 83,017명 sync + contact policy 자동 조회 (0403)** |
...
| **P3-S4** | 알리고 알림톡 프론트 발송 UI | **75%** | 세그먼트 선택 UI, 스케줄링, 결과 통계 (consent 자동 조회 ✅, 채널 토글 알림톡/SMS ✅ 완료 0404) |
| **P3-S5** | 더클린커피 재구매 관리 — 후보 추출 + 일괄 평가 + 프론트 UI | **80%** | ✅ 백엔드 API 2개 (Codex), 프론트 UI 실데이터 가동 (0404). 남은 것: 알림톡 탭 일괄 발송 연결 |
| **P3-S6** | SMS 문자 발송 기능 | **70%** | ✅ 백엔드 완료 (기존), contact policy sms 채널 ✅, 프론트 SMS 워크플로우 ✅ (0404). 남은 것: SMS quota 표시, 080 수신거부 자동 추가 |

---

## 더클린커피 아임웹 CRM 캠페인 현황 분석 (0404)

> 아임웹 관리자 "카카오 메시지 자동화" 화면에서 캡처. API로는 조회 불가(관리자 전용 데이터).

### 기존 캠페인 목록

| 캠페인 | 유형 | 상태 | 발송 대상 | 발송 성공 | 등록일 |
|--------|------|------|---------|---------|--------|
| 7월 더클린 프로모션 CRM | 브랜드 메시지 | 임시저장 | 0 | 0 | 25.07.16 |
| 장바구니+1시간(쿠폰x) 상품 구매 유도 | 브랜드 메시지 | **실행중** | 1,473 | 970 | 25.04.23 |
| 장바구니+1시간(+3천원 할인쿠폰) | 브랜드 메시지 | 일시정지 | 964 | 649 | 24.10.16 |

### 실행중 캠페인 상세: "장바구니+1시간(쿠폰x)"

**트리거**: 장바구니에 상품 추가 후 1시간 동안 구매하지 않은 전체 고객
**발송 시간**: 08:00~20:00 매일

**최근 1주 퍼널 (03.28~04.04):**

| 단계 | 수 | 전환율 | 판정 |
|------|-----|--------|------|
| 캠페인 발송 | 24명 | — | 일 평균 3~4명 |
| 발송 성공 | 12명 | 50% | **낮음** — 절반 실패 (카카오 채널 미구독 추정) |
| 유입 전환 | 9명 | 75% | **좋음** |
| 구매 전환 | 3명 | 25% | **업계 상위** (일반 5~15%) |
...
| 기간 내 총 매출 | ₩7,732,593 | |
| 연관 매출 | ₩139,370 | |
| 매출 기여도 | 1.8% | **낮음** — 대상 모수가 적어서 |
| 캠페인 비용 | ~₩12,416 (1,552건 × ~₩8) | |
| 추정 ROAS | **11.2x** | **매우 높음** |

### 핵심 발견

1. **구매 전환율 25%는 우수**하지만, **발송 성공률 50%가 병목**. 실패의 주 원인은 카카오 채널 미구독.
2. **쿠폰 유/무 A/B 비교가 안 됨**. 쿠폰 캠페인이 일시정지 상태라 효과 비교 불가.
3. **매출 기여도 1.8%는 시나리오 부족** 때문. 장바구니 이탈만으로는 전체 매출에 영향이 작음.

### 우리 솔루션의 개선 기회

| 개선 | 기대 효과 | 구현 상태 |
|------|---------|----------|
| **SMS fallback** — 카카오 발송 실패 시 SMS로 재발송 | 도달률 50% → 90%+, 같은 전환율이면 매출 ~2배 | ✅ SMS 발송 + contact policy 가동 중 |
| **재구매 시나리오 추가** — 30~90일 미구매 고객 알림 | 대상 모수 확대 (현재 100명 후보 확인) | ✅ 재구매 후보 API + 프론트 가동 |
| **정밀 매출 귀속** — attribution ledger 기반 | "연관 매출" 모호함 → 결제 원장 직결 | ✅ live 20건 가동 |
| **A/B 실험** — 쿠폰 유/무 증분 효과 측정 | 쿠폰 ROI 정량 판단 | Phase 7에서 구현 |

**즉시 실행 가능한 액션**: SMS fallback으로 발송 실패 50%를 복구하면, 월 ₩139,370 → ₩250,000+ 추정.
