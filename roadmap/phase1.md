...
- owner:
  ...
  - `P1-S2`: Claude Code
- next action: `리인벤팅 W7 GTM 협의(tag_id 44 null-safe patch 또는 payment page 제외/제거 판단) + GA4 (not set) BigQuery/hourly compare 진단 고정 + 장거리 settlement backfill completion 확보 + P3 첫 operational live + 운영 DB cutover`
- blocked by: `리인벤팅 GTM-W7VXS4D8 payment page 오류`, `GA4 (not set) historical row 원인 미분리`, `장거리 settlement backfill 완료 기록`, `운영 DB cutover`
...
**`P1-S1A 결제 블랙박스`는 0408 기준 `status-aware ledger`와 biocom fetchfix live 검증까지 반영되었다.** live/replay/smoke뿐 아니라 `pending/confirmed/canceled`, `confirmedRevenue`, caller 식별자 누락률을 함께 본다.
특히 **`WAITING_FOR_DEPOSIT` 같은 가상계좌 미입금 주문을 `pending`으로 남기고 `confirmedRevenue`에서는 제외하는 것**은 현재 최우선 운영 기준 중 하나다.
다음 행동은 신규 장부 추가보다 `biocom payment page GTM 오류 정리 + GA4 (not set) 진단 루프 고정 + 일일 정합성 체크 + 운영 cutover`다.
...
- `(not set)` 문제도 이제 "결제완료 footer가 식별자를 전혀 못 보낸다" 단계는 지났다. recent biocom/coffee payment_success와 AIBIO form_submit live row에서 `ga_session_id / client_id / user_pseudo_id` 유입이 시작됐으므로, 다음 진단은 `historical row + biocom payment page GTM 오류 + BigQuery raw export`를 분리해서 읽는 쪽으로 옮겨갔다.
...
- biocom payment success 페이지에서는 GTM custom script 오류(`gtm.js?id=GTM-W7VXS4D8 ... includes`)가 콘솔에서 관찰됐다. public W7 컨테이너 파싱 기준 culprit은 리인벤팅 `tag_id 44` `Custom HTML`의 `c.includes("RETOUS_")`로 좁혀졌다.
- 이 태그는 같은 컨테이너 안 `c_retous_crm_open` 이벤트 생산자라서, 바로 삭제보다 먼저 리인벤팅과 실제 사용 여부를 확인해야 한다. 계속 쓰면 null-safe patch가 우선이고, 안 쓰면 payment page 제외 또는 W7 제거와 협업 중단 검토가 합리적이다.

남은 것:
- coffee KPI 재산출과 대시보드 반영
- 운영 DB cutover
- `ledger / sync-status / toss-join / crm-phase1`를 같은 날짜 범위로 보는 일일 대사 루틴 고정
- 리인벤팅 `GTM-W7VXS4D8` `tag_id 44` 사용 여부 협의, null-safe patch 적용 또는 payment page 제외/제거 결정
...
live row + UTM 추적, Toss 키 반영, local backfill, SQLite ledger 승격, `payment_status`, status sync route까지 반영되었다. 남은 것은 coffee KPI 재산출, `GA4 (not set)` 진단 루틴 고정, 운영 대사 루틴 고정이다.
...
- coffee KPI 재산출과 운영 문서 반영은 아직 남아 있다.
...
1. **biocom payment page GTM 오류 정리** → `GTM-W7VXS4D8` payment page custom script에서 `includes` 오류 원인 추적.
2. **커피 LTR/재구매 지표 재산출** → `store=coffee` 기준 2025~2026 backfill 결과를 지표에 반영.
3. **GA4 `(not set)` 진단 루프 고정** → `BigQuery raw export + hourly compare + caller coverage + crm-phase1/ops`를 같은 날짜 범위로 읽는다.
4. **일일 정합성 체크 루틴 고정** → `ledger / sync-status / toss-join / crm-phase1`를 같은 범위로 매일 본다.
5. 운영 DB에 attribution ledger cutover.
...
1. **biocom payment page GTM 오류 정리** → `GTM-W7VXS4D8` payment page custom script에서 `includes` 오류 원인 추적
2. **GA4 `(not set)` 원인 좁히기** → `BigQuery raw export`, `hourly compare`, `caller coverage`, `crm-phase1/ops`를 같은 날짜 범위로 대조
3. **일일 정합성 체크 루틴 고정** → `GET /api/attribution/ledger`, `GET /api/attribution/caller-coverage` 또는 동등 진단, `POST /api/attribution/sync-status/toss?dryRun=true`, `GET /api/attribution/toss-join`, `GET /api/crm-phase1/ops`
4. **settlement coverage 마감** → 장거리 backfill 완료 응답과 최종 coverage 수치 확정
5. **P3 첫 operational live 실행** → 세그먼트 1개 고정 후 실제 발송/전환 추적 시작
...
- biocom payment page console check -> `GTM-W7VXS4D8` custom script에서 `includes` 오류 관찰, attribution 적재 성공과는 별개로 후속 수정 필요
