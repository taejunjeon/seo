...
기준일: 2026-03-27 (최종 업데이트: 2026-03-31 #14)
작성 근거:
...
| [P1-S1A](#p1-s1a) | PG attribution 원장 · success receiver · 토스 조인 진단 | Codex (백엔드/설계) | 95% / 20% |
| [P1-S2](#p1-s2) | 실험 목록/상세/KPI 화면 구현 | Claude Code (프론트/UXUI) | 100% / 70% |

> **Phase 1 남은 작업**: 실제 고객 사이트에 `checkout-context/payment-success` receiver 연결, 운영 DB cutover, 운영 재검증.
...
- **P1-S1A replay 배선 점검 완료**: `POST /api/attribution/replay/toss`로 read-only `tb_sales_toss` 최근 승인 `5건`을 replay row로 적재했고, 현재 원장 요약은 `live 0 / replay 5 / smoke 2`, replay 기준 토스 조인율은 `5/5`
- **남은 핵심 병목은 live 연결 하나**: 실제 고객 사이트에서 들어온 live row는 아직 `0건`이라 `(not set)` 원인 확정은 여전히 미완료
- 남은 것: 실제 고객 사이트 receiver 연결, 운영 DB/운영 배포 반영, lead ledger 실데이터 적재, 운영 재검증
...
| [P2-S3](#p2-s3) | `/callprice` 상담사 가치 분석 대시보드 프론트                                          | Claude Code (프론트/UXUI) | 90%  |
| [P2-S4](#p2-s4) | `/crm` CRM 관리 페이지 (후속 관리 대상 리스트)                                         | Claude Code (프론트/UXUI) | 90%  |
| [P2-S5](#p2-s5) | callprice 추가 UXUI (상품 믹스, 분포, 첫 구매일수)                                    | Claude Code (프론트/UXUI) | 0%   |

> **Claude Code 남은 작업**: P2-S3 `/callprice` 마지막 10% — 분석유형별 차트가 `/cohort`에는 있지만 `/callprice`에는 아직 없음. P2-S4 `/crm` 마지막 10% — 상담사/주문매칭 테이블 컬럼 한글화. P2-S5 상품 믹스/매출 분포 p50·p75·p90/첫 구매일수 시각화 (백엔드 API 확장 필요).
...
| Sprint          | 목표                                                 | 담당          | 완료  |
| --------------- | -------------------------------------------------- | ----------- | --- |
| [P3-S1](#p3-s1) | ChannelTalk SDK 래퍼 · identity/event 연동 규칙          | Codex (백엔드/설계) | 80% |
| [P3-S2](#p3-s2) | ChannelTalk 프론트 래퍼 (boot/setPage/track/updateUser) | Claude Code (프론트/UXUI) | 90% |
| [P3-S3](#p3-s3) | 알리고 알림톡 백엔드 전송 wrapper · 템플릿 관리                    | Codex (백엔드/설계) | 55% |
| [P3-S4](#p3-s4) | 알리고 알림톡 프론트 발송 UI (`/crm`에 통합)                     | Claude Code (프론트/UXUI) | 0%  |

> **Codex 이번 턴 반영**: Revenue 백엔드에 `GET /api/crm/channeltalk/sync-preview`, `POST /api/crm/channeltalk/sync-users`, `GET /api/crm/channeltalk/stale-users`, `POST /api/crm/channeltalk/campaign-preview` 추가. `crm_contact_policy_service.py`로 발송 hard gate 실행 코드 추가. SEO 백엔드에는 `GET /api/aligo/status|health|profiles|templates|quota`, `POST /api/aligo/test-send` 추가.
>
> **현재 운영 blocker**: ChannelTalk는 실제 고객 사이트 live 삽입이 남았고, Aligo는 새 env를 읽은 뒤에도 provider가 `인증되지 않는 서버 IP`로 차단했다. 즉 자격증명은 닫혔지만 서버 IP whitelist가 아직 안 닫혔다.
>
> **용어 정리**:
...
>
> **Claude Code 남은 작업**: P3-S2 마지막 10% — biocom.kr용 SDK 래퍼는 완성, 실제 고객 사이트 배포는 별도. P3-S4 알림톡 발송 UI 전체 — 대상자 선택→템플릿 선택→발송→결과 확인 흐름. Codex P3-S3 백엔드 계약 기준으로 진행 가능.
...
> **Claude Code 남은 작업**: P7-S2 실험 운영 대시보드 — 대조군/실험군 비교 차트, 실시간 상태 배지. 로컬 SQLite 실험 데이터 기반으로 바로 구현 가능. P7-S5 리커버리랩 캠페인 UI — 서울 고객 세그먼트 미리보기, 쿠폰 발급/사용 현황. Codex P7-S4 설계 후 진행.
...
| [P8-S2](#p8-s2) | Amplitude/Mixpanel 도입 필요 여부 판단 | Codex (백엔드/설계) | 0% |
...
- 알리고 알림톡 발송에 추가 필요한 키: `ALIGO_API_KEY`(알리고 인증키), `ALIGO_USER_ID`(알리고 사용자ID), `ALIGO_SENDER_PHONE`(발신자 번호)
- 위 3개가 모두 채워지면 **CRM 관리 화면에서 알림톡 발송 기능**을 바로 연결할 수 있음
...
현재 기준 다음으로 가장 파급력 큰 작업은 **실제 고객 사이트에 P1-S1A live row를 붙이는 것**이다.

이유:

1. `live / replay / smoke` 중 아직 `live`만 비어 있어 `(not set)` 원인을 확정할 수 없다
2. P1-S1A가 닫혀야 checkout abandon 실험도, 상담 후속 실험도 같은 전환 원장 위에서 읽을 수 있다
3. ChannelTalk/알리고 채널은 이제 backend 계약이 크게 올라왔지만, live receiver가 비어 있으면 causal test 해석이 흔들린다
4. 이 작업은 새 DB 스키마 승인 없이도 실제 사이트 코드 연결만 되면 바로 진도가 난다
...
남은 일:
- 실제 고객 사이트의 checkout 시작 시점과 payment success 시점에서 위 receiver를 호출해야 한다
- DB write 권한 또는 승인 후 `JSONL -> DB ledger`로 승격한다
- 현재는 replay를 통한 조인 plumbing 검증까지 닫혔고, 최종적으로는 live receiver 1건 적재와 DebugView 수동 검증이 남아 있다

실무 해석:
- `P1-S1A`는 receiver, JSONL ledger, 토스 조인, replay backfill, GA4 not-set 비교표, `/crm` 진단 화면까지 올라갔다
- 즉 `배선 점검용 replay`는 닫혔고, `원인 확정용 live`만 남았다
- 하지만 live row가 없으므로 `(not set)=PG 리다이렉트`를 아직 확정할 수는 없다
- 따라서 현재 표현은 `유력 가설`이 맞고, 실제 고객 사이트 연결과 브라우저 결제 플로우 검증이 마지막 남은 단계다
...
- 실제 확인:
  ...
  - provider 호출 결과는 `code=-99`, `인증되지 않는 서버 IP`로 실패
- 해석:
  - 키 누락 단계는 지났다
  - 지금 blocker는 자격증명이 아니라 **Aligo 서버 IP whitelist**다
- 아직 안 된 것:
  - 승인 템플릿 코드 기준 testMode 발송 1건 성공
...
- Amplitude/Mixpanel 도입 필요 여부 판단
...
### P8-S2
> Amplitude/Mixpanel 도입 필요 여부 판단 | Codex
...
- Amplitude/Mixpanel 도입 필요 조건 체크리스트 작성
...
| `P3-S3` | 진행 중, 운영 blocker 확인 | 알리고 wrapper, 템플릿 조회, quota, test send route 운영 연결 | Aligo 서버 IP whitelist, 승인 템플릿 코드 | `testMode=Y` 기준 발송 1건 성공 |
...
| `P8-S2` | 즉시 문서화 가능 | Amplitude/Mixpanel 도입 판단 체크리스트 작성 | 없음 | 도입/보류 결론을 1페이지로 설명 가능 |
...
6. 외부 자격증명 의존 작업은 병렬로 체크리스트만 준비한다. `P3-S3`, `P5-S1`, `P5-S3`, `P6-S1`은 키가 열리는 즉시 착수할 수 있게 endpoint, env, 검증 절차를 문서로 먼저 묶는다.
...
- 자격증명 필요:
  - 알리고: `ALIGO_API_KEY`, `ALIGO_USER_ID`, `ALIGO_SENDER_PHONE`
