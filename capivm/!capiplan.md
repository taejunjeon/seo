...
harness_preflight:
  ...
  source_window_freshness_confidence:
    source: "VM Cloud attribution ledger + Meta CAPI send log + funnel-health/roas-summary live contract + local/backend metaCapi guard test + Meta UI 캡처/사용자 관측 + biocom identity/email canary deploy reports + Google Ads final URL/landing route read-only audit + InitiateCheckout browser eventID/fast-fire audit + capivm/!capiplan.md + capivm/meta-utm-source-bucket-audit-20260519.md + project/!indicatoragent.md + total/!total-current.md"
    window: "2026-05-14 ~ 2026-05-22 KST incident/recovery window, 최근 7일 ROAS/퍼널/선행지표 문서 기준"
    freshness: "문서 정리 시각 2026-05-22 14:20 KST. 2026-05-22 업데이트는 confirmed Purchase 누락 큐 0건, 최근 1일 CAPI success 70/failure 0, 이벤트 매칭 품질 canary read-only 집계, Browser Pixel 중간 이벤트 복구 후 Server CAPI 중간 이벤트 운영 ON 보류 판단을 반영한다. live 숫자는 API 재조회 시 달라질 수 있다."
    confidence: 0.9
```
...
| KR2 | [[#Phase1-Sprint2]]                      | 결제완료인데 Meta CAPI 성공 기록이 없는 row를 매일 볼 수 있게 한다       |    98% | VM Cloud cron이 매일 10시 KST confirmed-but-no-CAPI를 감시하고, 30분 grace 이후 Critical만 Slack으로 알린다. 2026-05-22 수동 post-check에서 누락 큐 0건, 1일 CAPI success 70/failure 0으로 닫혔다 | 다음 자동 cron 결과가 같은 기준으로 2회 연속 PASS인지 확인하고, 새 누락 row가 나오면 current sync issue로 사유 분류한다                               | current window 누락 row가 매일 사유별로 닫히고, legacy backlog는 `do_not_backfill`로 보존됨                                   |
| KR3 | [[#Phase2-Sprint4]]                      | Browser Pixel 중간 퍼널 신호를 복구한다                       |    90% | AddToCart / InitiateCheckout / AddPaymentInfo는 Block 4 fallback으로 복구됐고, VM Cloud live도 `/shop_cart` 장바구니 페이지 진입 28건을 반환한다. InitiateCheckout fast path는 급한 수정이 아니라 다음 아임웹 변경 패키지의 선택 항목으로 보관한다 | `ViewContent`가 장바구니로 오해되지 않게 로컬 fallback guard를 VM Cloud 반영 후보로 두고, 화면 설명을 더 쉽게 맞춘다. 다음 아임웹 변경 때 `nativeObserveMs` 단축 여부를 같이 판단한다 | 유입 -> 장바구니 -> 결제 시작 -> 결제수단 선택 -> 실제 결제완료가 화면에서 0 없이 설명됨                                                     |
...
| KR7 | [[#Phase3-Sprint6]], [[#Phase4-Sprint7]] | 외부 전송 준비와 실제 전송 실행을 분리한다                           |    88% | biocom confirmed Purchase 한정 ph/external_id/email canary가 켜졌고, read-only 집계에서 최근 성공 Purchase 109건 success 109/failure 0/duplicate 0, ph/external_id 67.89%, em 8.26%로 관측됐다 | Meta UI 점수 6.3/10 이후 24시간 추가 관찰을 유지하되, 문제가 없으면 biocom 범위에서는 유지한다. 더클린커피/Browser Purchase/event_id hash는 아직 확장하지 않는다 | Red 승인 전 수동 backfill/대량 send 0, canary 유지/중단 기준이 숫자로 닫히고, 문제 없으면 biocom 범위에서 유지됨                         |
| KR8 | [[#Phase2-Sprint5]]                      | Purchase 외 중간 전환 신호를 Meta CAPI로 안전하게 확장한다          |    58% | no-send payload preview가 준비됐고 AddPaymentInfo source gap이 `VM Cloud 저장 경로 없음`으로 닫혔다. Browser Pixel 중간 이벤트가 상당 부분 복구됐으므로 Server CAPI 중간 이벤트 운영 ON은 당장 필요하지 않다. InitiateCheckout은 browser/server eventID 중복제거 설계가 선행 조건이다 | VM Cloud no-send intermediate-event endpoint를 먼저 만들고, Browser가 누락되거나 eventID bridge가 닫힌 event만 Test Events 1건 이하 smoke 승인안으로 넘긴다. 운영 ON은 event별 staged 승인으로만 진행한다                                  | 중간 이벤트가 Meta Events Manager에 server source로 보이고, Purchase/ROAS 오염 0, 중복 이벤트 0, CPA/Ads attributed ROAS 개선 여부를 비교할 수 있음 |
...
### 2026-05-22 수동 close run

2026-05-22 14:08 KST 수동 close run:
...
- severity: ok
- Slack 발송: skip. 정상 상태이며 notify mode가 alert_only이기 때문이다.
- post-check verdict: PASS
- 해석: 현재 window 기준으로 “결제완료인데 Meta CAPI 성공 기록이 없는 주문”은 없다. 다음부터는 신규 발생 row만 current sync issue로 본다.

근거 파일:

- `data/project/meta-capi-daily-missing-queue-monitor-manual-20260522-capi-close.json`
- `data/project/meta-capi-phase1-sprint1-postcheck-manual-20260522-capi-close.json`

## 2026-05-22 우선순위 업데이트 — InitiateCheckout fast path는 선택 항목으로 보관

### 결론

`InitiateCheckout` 서버 CAPI 운영 ON은 보류한다.
현재 `InitiateCheckout`은 Browser Pixel 이벤트로 이미 잡히고 있으며, TJ님이 확인한 eventID도 Block4 fallback이 만든 안전한 브라우저 eventID 형태다.

쉽게 말하면:

> 결제 시작 신호는 이미 Meta에 가고 있다. 지금 바꿀 것은 "몇 초 더 빨리 보이게 할지"이지, "안 가는 신호를 살릴지"가 아니다. 그래서 속도 개선만을 위해 아임웹 코드를 단독 교체하지 않는다.

### 왜 지금 단독 수정하지 않는가

Block4는 `/shop_payment/` 진입 직후 바로 쏘지 않고 잠깐 기다린다.

- 기존 Meta/FBE/native Pixel이 먼저 `InitiateCheckout`을 보냈는지 확인한다.
- 주문 금액과 통화(`value`, `currency`)가 DOM에서 늦게 보이면 짧게 재시도한다.
- 같은 세션에서 같은 이벤트가 중복 발화되지 않게 막는다.

따라서 Pixel Helper에서 몇 초 늦게 보이는 것은 장애라기보다 중복 방지 장치의 결과일 가능성이 높다.
정확한 기준은 Pixel Helper UI가 아니라 Chrome Network의 `facebook.com/tr?ev=InitiateCheckout` 요청 시각이다.

### 다음 아임웹 변경 패키지에 묶는 항목

다음에 아임웹 헤더/푸터를 변경할 일이 생기면 아래 항목을 선택적으로 같이 검토한다.

- `nativeObserveMs`: 1200ms -> 400~600ms 후보
- `valueRetryMs`: 현재 0/600/1400/2600/4000ms -> 더 짧은 재시도 후보
- 조건: native/FBE가 먼저 보낸 경우 fallback skip 유지
- 조건: `cd[value]`, `cd[currency]` 누락 방지
- 조건: 같은 세션 중복 0
- 조건: Purchase 발화 0
- 조건: Server CAPI `InitiateCheckout` 운영 ON은 계속 별도 승인

### 근거 문서

- [[capivm/initiate-checkout-browser-eventid-audit-20260522]]
- [[capivm/initiate-checkout-fast-fire-review-20260522]]

### Source / Window / Freshness / Confidence

- source: Biocom Imweb Footer Block4 v0.5 후보 코드, TJ님 실제 eventID 관측, 로컬 코드 감사
- window: 2026-05-22 테스트 세션 기준
- freshness: 2026-05-22 13:35 KST
- confidence: 0.91

## 2026-05-22 우선순위 업데이트 — Browser 중간 이벤트 복구 후 Server CAPI 판단

### 결론

Browser Pixel 중간 이벤트가 많이 복구됐다고 해서 Server CAPI 중간 이벤트가 영원히 필요 없어지는 것은 아니다.
다만 지금은 Purchase CAPI와 이벤트 매칭 품질 canary가 더 중요하므로, **InitiateCheckout Server CAPI 운영 ON은 보류**한다.

쉽게 말하면:

> Meta가 이미 브라우저에서 장바구니/결제시작/결제정보 같은 중간 행동을 받고 있다면, 같은 행동을 서버에서도 바로 보내면 중복 위험이 생긴다. 지금은 “브라우저가 놓치는 구간을 보완할 후보”로만 설계하고, 실제 전송은 eventID 중복제거가 닫힌 뒤에 한다.

### Server CAPI 중간 이벤트가 유용한 경우

- 브라우저 차단, 앱 내 브라우저, 네트워크 손실 때문에 Browser Pixel 이벤트가 빠질 때.
- 서버가 이미 안전하게 알고 있는 이벤트라서 후보 정의가 더 정확할 때.
- Meta Test Events에서 server source 수신을 분리해 볼 필요가 있을 때.
- 장기적으로 광고 학습 표본을 늘리고 싶을 때.

### 지금 바로 켜지 않는 이유

- `InitiateCheckout`은 이미 Browser Pixel에서 잡히고 있다.
- TJ님이 확인한 eventID는 Block4 fallback 계열로 보이며, Server CAPI가 같은 eventID를 쓰는 구조가 아직 아니다.
- 같은 이벤트를 Browser와 Server가 서로 다른 eventID로 보내면 Meta가 두 번 본 이벤트로 처리할 수 있다.
- 중간 이벤트는 Purchase보다 위험은 낮지만, 퍼널/최적화 해석을 흐릴 수 있으므로 no-send preview와 Test Events smoke를 먼저 거쳐야 한다.

### 현재 판단

- Purchase CAPI: 계속 운영 주 경로.
- Browser 중간 이벤트: 현재 운영 주 경로.
- Server 중간 이벤트: 보조 경로 후보. 운영 ON 보류.
- `VirtualAccountIssued`: 결제완료가 아니라 “가상계좌 주문 생성” 상태이므로 별도 이벤트로 즉시 발화하는 방향을 유지하되, Purchase 후보와는 분리한다.
- `InitiateCheckout`: 다음 아임웹 변경 패키지에 fast path를 선택 항목으로 보관한다. 속도만 줄이기 위한 단독 교체는 보류한다.
...
### 2026-05-22 canary close 집계

최근 운영 성공 Purchase 109건을 raw 값 없이 presence만 집계했다.

- window: 2026-05-20 14:10:36 ~ 2026-05-22 14:04:04 KST
- site: biocom
...
- duplicate order event key: 0
- `em` presence: 9건 / 8.26%
- `ph` presence: 74건 / 67.89%
- `external_id` presence: 74건 / 67.89%
- `client_ip_address` presence: 80건 / 73.39%
- `client_user_agent` presence: 80건 / 73.39%
- `fbc` presence: 47건 / 43.12%
- `fbp` presence: 77건 / 70.64%
- Meta UI 관측 점수: 배포 전 6.0/10, 이후 사용자 캡처 6.3/10

판단:

- 실패와 중복이 0이므로 canary를 즉시 끌 이유는 없다.
- `ph`와 `external_id`는 실제 payload에 충분히 붙고 있다.
- `em`은 후보 자체가 제한적이라 점수 개선을 크게 만들기 어렵다. 회원 이메일 수집률 또는 Imweb 회원 주문 연동 품질이 개선되어야 오른다.
- Meta UI 점수는 6.3/10까지 올랐지만, Meta UI 반영 지연과 표본 변화가 섞이므로 canary 단독 효과라고 단정하지 않는다.

근거 파일:

- `data/project/meta-capi-emq-canary-aggregate-20260522.json`
...
- 어떻게 하는가: VM Cloud 원장에는 먼저 `purchase_candidate=false`로 저장하고, Meta CAPI는 no-send payload preview와 eventID 중복제거 설계를 통과한 event만 staged ON 후보로 올린다. `InitiateCheckout`은 이미 Browser 이벤트가 잡히므로 운영 Server CAPI ON은 보류한다.
- 성공 기준: 각 event가 Purchase와 분리되고, Meta Events Manager에서 server source 수신을 검증할 수 있는 payload가 준비된다. Browser와 Server를 같이 쓰는 event는 같은 eventID를 쓸 수 있거나, Server 전송을 보류한다.
- 실패 시 다음 확인점: health/wellness 제한, custom event 이름, 중복 eventID, event match quality, Ads Manager 최적화 이벤트 오염 여부, Browser native/FBE 선발화 여부.
- 담당: Codex.
...
#### C3. Block4 InitiateCheckout fast path
- 무엇을 보류하는가: 아임웹 Footer Block4에서 `InitiateCheckout` 발화 대기 시간을 1.2초에서 0.4~0.6초 수준으로 줄이는 선택 변경.
- 왜 보류하는가: 현재 `InitiateCheckout`은 이미 Browser Pixel 이벤트로 잡히고 있다. 속도만 줄이려고 아임웹 코드를 단독 교체하면 운영 검증 비용과 중복 발화 리스크가 더 크다.
- 재개 조건: 다음 아임웹 변경 패키지에서 stale click-id, VirtualAccountIssued, AddPaymentInfo 같은 더 중요한 수정과 함께 한 번에 smoke할 때 선택 항목으로 검토한다.
- 실패 시 해석: Pixel Helper에 몇 초 늦게 보이는 것은 즉시 장애가 아니다. 다만 Network 기준으로 `ev=InitiateCheckout` 자체가 사라지거나 중복이 늘면 우선순위를 올린다.
- 담당: Codex 초안/fixture, TJ님 아임웹 적용 승인.
- 승인 필요 여부: 문서/초안은 NO, 아임웹 저장은 YES.
- 의존성: 다음 아임웹 변경 패키지.
- 추천 점수/자신감: 지금 단독 실행 35%, 다음 아임웹 패키지에 포함 88%.
...
- 2026-05-22 13:35 KST: `InitiateCheckout` Server CAPI 운영 ON은 보류로 확정하고, Block4 fast path는 다음 아임웹 변경 패키지의 선택 항목으로 보관했다. KR8은 56%로 갱신했다. 실제 Meta send/GTM publish/VM Cloud deploy/아임웹 저장은 0.
