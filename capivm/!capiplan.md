...
harness_preflight:
  ...
  source_window_freshness_confidence:
    source: "VM Cloud attribution ledger + Meta CAPI send log + funnel-health/roas-summary live contract + local backend metaCapi guard test + GTM export audit + Meta UI 캡처/사용자 관측 + Google Ads final URL/landing route read-only audit + capivm/!capiplan.md + capivm/meta-utm-source-bucket-audit-20260519.md + project/!indicatoragent.md + total/!total-current.md"
    window: "2026-05-14 ~ 2026-05-20 KST incident/recovery window, 최근 7일 ROAS/퍼널/선행지표 문서 기준"
    freshness: "문서 정리 시각 2026-05-20 17:37 KST. 2026-05-20 업데이트는 더클린커피 Google Ads 실제 유입 재확인 결과를 Watch/backlog로 내리고, Meta CAPI를 P0로 유지하는 결정을 반영한다. live 숫자는 API 재조회 시 달라질 수 있다."
    confidence: 0.88
```
...
| KR2 | [[#Phase1-Sprint2]]                      | 결제완료인데 Meta CAPI 성공 기록이 없는 row를 매일 볼 수 있게 한다       |    92% | 5/17 current monitor에서는 Critical 누락이 0으로 닫혔고, 5/14~5/15 수리 전 backlog는 ROAS 오염 방지를 위해 backfill하지 않기로 결정했다 | 5/14~5/15는 역사적 backlog로 보관하되 전송하지 않는다. 5/17 이후 current window를 daily KPI로 보고, 새 누락 row만 사유 분류한다                               | current window 누락 row가 매일 사유별로 닫히고, legacy backlog는 `do_not_backfill`로 보존됨                                   |
...
| KR7 | [[#Phase3-Sprint6]], [[#Phase4-Sprint7]] | 외부 전송 준비와 실제 전송 실행을 분리한다                           |    65% | no-send/dedup/value guard는 일부 준비됐고, refund/cancel guard와 Test Events smoke가 남았다                          | Test Events 1건 이하 smoke, refund/cancel source priority, GA4/Google/TikTok/Naver별 approval packet을 분리한다                        | Red 승인 전 운영 send 0, 승인 시 payload/rollback/postcheck가 플랫폼별로 준비됨                                               |
...
다음 행동은 이 문서의 KR 기준으로 `누락 큐`, `UTM/source 미매핑 bucket`, `중간 전환 CAPI 후보`, `환불/취소 guard`, `Events Manager UI 검증`, `Test Events smoke`를 순서대로 닫는 것이다.
