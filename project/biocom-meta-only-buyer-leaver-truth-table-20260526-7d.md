...
- VM Cloud에서 Meta 유입으로 분류된 바이오컴 safe session은 245건이고, 이 중 GA4와 붙은 것은 205건(83.67%)이다.
- Meta 구매자는 154건, 비결제자는 91건이다.
- GA4로 붙은 row 기준 구매자 p50 체류시간은 188.5s, 비결제자는 133.06s로 차이는 55.44s이다.
- 비결제자처럼 보였지만 GA4 purchase가 있는 충돌 row는 3건(4.17%)이다. 이 row는 순수 이탈자가 아니라 보류 bucket으로 떼어내야 한다.
- 현재 판정은 high다. 즉, Meta-only 행동 비교를 볼 만큼 GA4-VM 연결은 충분히 닫혔다. 다만 비결제자 중 GA4 purchase가 4.17% 있어 진짜 이탈자와 분류 충돌 row를 분리해야 한다.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 구매자 | 154 | 133 | 86.36% | ₩48,398,854 | 188.5s | 333.86s | 91.73% | 45.86% | 8.27% | 98.5% | 2.26% | 99.25% |
| 비결제자 | 91 | 72 | 79.12% | ₩0 | 133.06s | 296.57s | 90.28% | 38.89% | 8.33% | 87.5% | 0% | 4.17% |

## 차이값

- 체류시간: 구매자 - 비결제자 = 55.44s.
- scroll90: 구매자 - 비결제자 = 1.45%.
- view_item: 구매자 - 비결제자 = 6.97%.
- cart/view_cart: 구매자 - 비결제자 = -0.06%.
- begin_checkout: 구매자 - 비결제자 = 11%.
- add_payment_info: 구매자 - 비결제자 = 2.26%.
...
- 충돌 row: 3건
- 비결제자 GA4 join row 중 비중: 4.17%

### 원인 bucket

| 원인 후보 | row | 사람이 읽는 의미 |
|---|---:|---|
| vm_payment_success_not_confirmed | 2 | VM Cloud에도 payment_success row는 있지만 status가 confirmed가 아니다. 운영DB/Imweb/Toss 확인 전 pending/unknown으로 남은 케이스다. |
| checkout_started_only_ga4_purchase | 1 | VM Cloud는 결제 시작까지만 봤고 완료를 못 봤는데 GA4는 purchase를 봤다. 세션 전환 또는 VM completion capture 누락 후보. |

### VM Cloud 흔적

- payment_success row: 2
- confirmed payment_success row: 0
- pending/unknown payment_success row: 2
- 완료 URL 흔적: 2
...
- 그래도 비결제자 cohort 안에 GA4 purchase event가 4.17% 남아 있다. 이것은 진짜 이탈자가 아니라 세션 window 차이, 결제창 이동, source 분류 차이일 가능성이 있다.
