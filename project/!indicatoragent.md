...
harness_preflight:
  ...
  source_window_freshness_confidence:
    source: "local docs + VM Cloud funnel/capi architecture + GTM export docs + Meta Conversions API official docs"
    window: "설계 기준 2026-05-17 KST, 분석 대상 window는 1d/7d/14d/30d로 설계"
    freshness: "문서 작성 시각 2026-05-17 15:20 KST, Meta 공식 문서 web check 2026-05-17"
    confidence: 0.86
```
...
프론트엔드 구현은 Claude Code가 담당한다. Codex는 Claude Code가 바로 화면화할 수 있게 API/data contract, blocker table, dry-run 산출물을 제공한다.
...
| P1 | [[#Phase2-Sprint1]] | 체류시간/스크롤/회원가입 지표를 만든다 | 구매 전 몰입 행동을 숫자로 봐야 한다 | GA4/VM Cloud를 조합해 dwell/scroll/signup feature를 만든다 | 더클린커피 구매자/이탈자 dwell/scroll 비교 가능. GA4 begin_checkout/add_payment_info 보강 설계 완료 | 62 | 각 feature의 purchase lift와 모수가 계산됨 | Codex: coffee middle-event test-only packet + signup feature | NO, Green | `project/coffee-ga4-middle-event-enhancement-design-20260517.md` |
...
#### B1. GTM Preview 또는 VM Cloud no-send endpoint 배포
