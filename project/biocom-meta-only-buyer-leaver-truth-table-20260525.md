...
- VM Cloud에서 Meta 유입으로 분류된 바이오컴 safe session은 225건이고, 이 중 GA4와 붙은 것은 177건(78.67%)이다.
- Meta 구매자는 143건, 비결제자는 82건이다.
- GA4로 붙은 row 기준 구매자 p50 체류시간은 200.03s, 비결제자는 122.52s로 차이는 77.51s이다.
- 비결제자처럼 보였지만 GA4 purchase가 있는 충돌 row는 2건(3.08%)이다. 이 row는 순수 이탈자가 아니라 보류 bucket으로 떼어내야 한다.
- 현재 판정은 high다. 즉, Meta-only 행동 비교를 볼 만큼 GA4-VM 연결은 충분히 닫혔다. 다만 비결제자 중 GA4 purchase가 3.08% 있어 진짜 이탈자와 분류 충돌 row를 분리해야 한다.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 구매자 | 143 | 112 | 78.32% | ₩46,673,104 | 200.03s | 340.61s | 91.07% | 53.57% | 7.14% | 97.32% | 2.68% | 99.11% |
| 비결제자 | 82 | 65 | 79.27% | ₩0 | 122.52s | 277.95s | 92.31% | 41.54% | 7.69% | 84.62% | 1.54% | 3.08% |

## 차이값

- 체류시간: 구매자 - 비결제자 = 77.51s.
- scroll90: 구매자 - 비결제자 = -1.24%.
- view_item: 구매자 - 비결제자 = 12.03%.
- cart/view_cart: 구매자 - 비결제자 = -0.55%.
- begin_checkout: 구매자 - 비결제자 = 12.7%.
- add_payment_info: 구매자 - 비결제자 = 1.14%.
...
- 그래도 비결제자 cohort 안에 GA4 purchase event가 3.08% 남아 있다. 이것은 진짜 이탈자가 아니라 세션 window 차이, 결제창 이동, source 분류 차이일 가능성이 있다.
