...
- VM Cloud에서 Meta 유입으로 분류된 바이오컴 safe session은 1032건이고, 이 중 GA4와 붙은 것은 992건(96.12%)이다.
- Meta 구매자는 601건, 비결제자는 431건이다.
- GA4로 붙은 row 기준 구매자 p50 체류시간은 202.77s, 비결제자는 148.64s로 차이는 54.13s이다.
- 비결제자처럼 보였지만 GA4 purchase가 있는 충돌 row는 33건(8.01%)이다. 이 row는 순수 이탈자가 아니라 보류 bucket으로 떼어내야 한다.
- 현재 판정은 high다. 즉, Meta-only 행동 비교를 볼 만큼 GA4-VM 연결은 충분히 닫혔다. 다만 비결제자 중 GA4 purchase가 8.01% 있어 진짜 이탈자와 분류 충돌 row를 분리해야 한다.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 구매자 | 601 | 580 | 96.51% | ₩188,861,442 | 202.77s | 330.08s | 93.1% | 49.31% | 13.97% | 98.1% | 1.72% | 97.59% |
| 비결제자 | 431 | 412 | 95.59% | ₩0 | 148.64s | 277.95s | 84.47% | 54.37% | 13.11% | 91.26% | 2.91% | 8.01% |

## 차이값

- 체류시간: 구매자 - 비결제자 = 54.13s.
- scroll90: 구매자 - 비결제자 = 8.63%.
- view_item: 구매자 - 비결제자 = -5.06%.
- cart/view_cart: 구매자 - 비결제자 = 0.86%.
- begin_checkout: 구매자 - 비결제자 = 6.84%.
- add_payment_info: 구매자 - 비결제자 = -1.19%.
...
- 충돌 row: 33건
- 비결제자 GA4 join row 중 비중: 8.01%

### 원인 bucket

| 원인 후보 | row | 사람이 읽는 의미 |
|---|---:|---|
| checkout_started_only_ga4_purchase | 21 | VM Cloud는 결제 시작까지만 봤고 완료를 못 봤는데 GA4는 purchase를 봤다. 세션 전환 또는 VM completion capture 누락 후보. |
| vm_payment_success_not_confirmed | 11 | VM Cloud에도 payment_success row는 있지만 status가 confirmed가 아니다. 운영DB/Imweb/Toss 확인 전 pending/unknown으로 남은 케이스다. |
...
- payment_success row: 11
- confirmed payment_success row: 0
- pending/unknown payment_success row: 11
- 완료 URL 흔적: 4
- 결제키/거래키 presence: payment_key 0, transaction_id 4
- value presence: 9
...
- 그래도 비결제자 cohort 안에 GA4 purchase event가 8.01% 남아 있다. 이것은 진짜 이탈자가 아니라 세션 window 차이, 결제창 이동, source 분류 차이일 가능성이 있다.
