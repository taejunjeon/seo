# Imweb footer v4.4.4 candidate - InitiateCheckout value/currency 보강

작성 시각: 2026-05-21 21:07 KST
기준일: 2026-05-21
문서 성격: 설계 + 아임웹 푸터 전체 교체 후보 코드
...
harness_preflight:
  ...
  source_window_freshness_confidence:
    source: local repository v4.4.3 footer + TJ님 Meta diagnostic screenshot
...
Meta가 `InitiateCheckout`에 `value`가 없다고 진단한 문제는 실제 구매 전환 문제가 아니라, 결제 시작 이벤트 품질 문제다.
이 후보는 새 `InitiateCheckout` 이벤트를 중복으로 쏘지 않고, 기존 `fbq('track', 'InitiateCheckout')` 호출 직전에 `value`와 `currency=KRW`를 보강한다.
`Purchase`와 `VirtualAccountIssued` 경로는 건드리지 않는다.
...
1. Phase 9 wrapper 버전을 `2026-05-21-biocom-funnel-capi-v4-4-4`로 올린다.
2. `InitiateCheckout` 이벤트에만 금액 보강을 적용한다.
3. 값 탐색 순서는 기존 payload → dataLayer → `__seo_checkout_context` → 주문서 DOM의 금액 라벨 → 화면 내 가장 큰 원화 금액이다.
4. 식별자 오인 방지를 위해 1,000원 미만과 10,000,000원 초과 숫자는 금액 후보에서 제외한다.
5. `currency`가 없으면 `KRW`를 넣는다.
...
- Meta 진단의 `InitiateCheckout 값 필드 없음` 비율을 낮출 수 있다.
...
- Meta Pixel Helper에서 `InitiateCheckout`에 `value > 0`, `currency=KRW`가 보인다.
- `Purchase`는 가상계좌 미입금에서 발화하지 않는다.
- `VirtualAccountIssued`는 가상계좌 완료에서 1회만 발화한다.
- 콘솔에 `[funnel-capi-v4.4.4] enrich InitiateCheckout value`가 보이거나, value가 이미 있으면 `already_has_value_and_currency`로 스킵된다.
...
PASS_WITH_NOTES. 후보 설계는 중복 이벤트를 늘리지 않고 기존 브라우저 Pixel payload를 보강하는 방식이라 현재 문제에 가장 좁게 대응한다.
