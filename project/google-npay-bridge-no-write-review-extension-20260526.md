...
harness_preflight:
  ...
  source_window_freshness_confidence:
    ...
    freshness: 2026-05-26 09:02 KST API generated, 2026-05-26 10:46 KST read-only 재조회
    confidence: high for aggregate counts, medium-high for visible row-level bridge grade before permanent snapshot
```
...
B급은 `가짜 주문`이라는 뜻이 아니다. 실제 NPay 결제완료와 내부 클릭 신호가 이어질 가능성은 있지만, 자동으로 A급으로 올리기에는 시간 간격이나 click id 증거가 부족한 후보라는 뜻이다.

2026-05-26 10:46 KST read-only 재조회 기준 전체 B급은 4건이다.

- Google click id가 있는 B급: 1건
- 시간 간격 때문에 막힌 B급: 2건
- 금액 불일치로 막힌 B급: 0건
- Google click id가 없어서 Google Ads 전송 후보가 될 수 없는 B급: 2건
- 지금 바로 A급으로 올릴 수 있는 B급: 0건

현재 공개 API의 `rows`는 상위 12건만 내려줘서 B급 4건 전체가 화면에 항상 보이지 않는다. 그래서 백엔드에 `gradeBRows` 필드를 추가해, 배포 후에는 B급 후보만 따로 최대 20건까지 볼 수 있게 준비했다. 이 변경은 화면 표시용 read-only 확장이고, VM Cloud 원장 write나 Google Ads 전송은 하지 않는다.

현재 공개 API에서 보이는 Google click id 포함 B급 1건은 아래 이유로 A급/전송 후보가 아니다.

- 주문 금액과 상품은 맞는다.
- Google click id와 campaign id도 있다.
- 하지만 클릭 intent와 결제완료 사이가 약 12.5분이다.
- 이 시간 간격은 자동 A급 기준보다 길다.
- 따라서 내부 수동 검토 후보로만 남기고, Google Ads 전송 후보로 올리지 않는다.
