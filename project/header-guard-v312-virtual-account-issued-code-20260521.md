# Header Guard v3.1.2 VirtualAccountIssued Code Draft

작성 시각: 2026-05-21 19:26 KST
기준일: 2026-05-21
문서 성격: 아임웹 헤더 코드 교체 초안 / 운영 저장 전 검토본
...
- Meta FBE/아임웹 자산 연결
- Block 4 fallback
- TikTok Purchase Guard

## v3.1.1 대비 변경점

- 완료 URL에서 가상계좌/무통장 힌트를 감지하면 `VirtualAccountIssued`를 1회 즉시 보낸다.
- 이 즉시 발화는 `Purchase` 시도에 의존하지 않는다.
- 같은 safe key는 `sessionStorage`로 중복 발화를 막는다.
- 기존 payment-decision pending 분기도 같은 dedupe를 사용한다.
- `VirtualAccountIssued`는 Meta 매출 이벤트가 아니므로 `value=0`, `is_purchase=no`, `is_paid=no`로 보낸다.
- `/shop_payment/` 일반 결제 페이지에서는 이 스크립트 자체가 실행되지 않는다.
...
(function () {
  ...
  var CONFIG = {
    ...
    decisionCachePrefix: '__biocom_payment_decision_guard_v312__:',
    ...
    logPrefix: '[biocom-server-payment-decision-guard-v3.1.2]'
  };
  ...
  if (window.__BIOCOM_SERVER_PAYMENT_DECISION_GUARD_V312_INSTALLED__) return;
  window.__BIOCOM_SERVER_PAYMENT_DECISION_GUARD_V312_INSTALLED__ = true;
  ...
  function buildVirtualAccountIssuedPayload(context, hint) {
    ...
    return {
      ...
      safeRef: fallbackSafeRef(context),
      source: 'completion_page_virtual_account_hint',
      endpointStatus: 0,
      fromCache: false,
      virtualAccountIssueSource: 'completion_page_immediate_fire',
  ...
  window.setTimeout(maybeFireVirtualAccountIssuedOnCompletion, 80);
...
1. 가상계좌 미입금 완료 URL에서 Network `facebook.com/tr`에 `ev=VirtualAccountIssued`가 1건만 보이는지 확인한다.
2. 같은 화면을 새로고침해도 같은 safe key 기준 중복 발화가 없는지 확인한다.
3. 같은 케이스에서 `ev=Purchase`가 0건인지 확인한다.
4. 카드 결제완료 케이스에서 `VirtualAccountIssued`가 0건인지 확인한다.
5. `/shop_payment/` 일반 결제 입력 페이지에서 `VirtualAccountIssued`와 `Purchase`가 모두 0건인지 확인한다.

## 운영 반영 판단

이 코드는 가상계좌 주문생성이라는 중간 전환 신호를 Meta에 알려주는 목적이다. 실제 입금 전 매출로 보지 않으며, Purchase 복구나 ROAS 계산에는 직접 사용하지 않는다.
