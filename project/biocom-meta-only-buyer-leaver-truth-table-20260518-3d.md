...
- VM Cloud에서 Meta 유입으로 분류된 바이오컴 safe session은 126건이고, 이 중 GA4와 붙은 것은 106건(84.13%)이다.
- Meta 구매자는 80건, 비결제자는 46건이다.
- GA4로 붙은 row 기준 구매자 p50 체류시간은 198.61s, 비결제자는 165.25s로 차이는 33.36s이다.
- 비결제자처럼 보였지만 GA4 purchase가 있는 충돌 row는 2건(5.41%)이다. 이 row는 순수 이탈자가 아니라 보류 bucket으로 떼어내야 한다.
- 현재 판정은 high다. 즉, Meta-only 행동 비교를 볼 만큼 GA4-VM 연결은 충분히 닫혔다. 다만 비결제자 중 GA4 purchase가 5.41% 있어 진짜 이탈자와 분류 충돌 row를 분리해야 한다.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 구매자 | 80 | 69 | 86.25% | ₩29,605,850 | 198.61s | 354.93s | 98.55% | 44.93% | 18.84% | 100% | 1.45% | 100% |
| 비결제자 | 46 | 37 | 80.43% | ₩0 | 165.25s | 265.92s | 83.78% | 56.76% | 21.62% | 86.49% | 5.41% | 5.41% |

## 차이값

- 체류시간: 구매자 - 비결제자 = 33.36s.
- scroll90: 구매자 - 비결제자 = 14.77%.
- view_item: 구매자 - 비결제자 = -11.83%.
- cart/view_cart: 구매자 - 비결제자 = -2.78%.
- begin_checkout: 구매자 - 비결제자 = 13.51%.
- add_payment_info: 구매자 - 비결제자 = -3.96%.
...
| vm_payment_success_not_confirmed | 1 | VM Cloud에도 payment_success row는 있지만 status가 confirmed가 아니다. 운영DB/Imweb/Toss 확인 전 pending/unknown으로 남은 케이스다. |

### VM Cloud 흔적

- payment_success row: 1
- confirmed payment_success row: 0
- pending/unknown payment_success row: 1
- 완료 URL 흔적: 1
- 결제키/거래키 presence: payment_key 0, transaction_id 1
- value presence: 1
...
- 그래도 비결제자 cohort 안에 GA4 purchase event가 5.41% 남아 있다. 이것은 진짜 이탈자가 아니라 세션 window 차이, 결제창 이동, source 분류 차이일 가능성이 있다.
