...
그로스팀 파트장 정정 기준으로 `원본/사본`은 단순 중복이 아니라 소재 구분이다.

- `meta_biocom_iiari_260518 - 종대사`
  - 이전 표시: `meta_biocom_iiari_260518 - 사본`
  - 소재 의미: 종합대사기능분석 서비스(종대사)
  - 라이브 광고명: `meta_biocom_iiari_acid_260518`
  - bridge 근거: adset ID `120245956430970396`, 26 rows, 결제완료 11건, 결제완료 금액 ₩4,436,570
  - Ads Manager 화면값: 7일 클릭 기준 웹사이트 구매 14건, 결과값 ₩5,109,770, 결과 ROAS 4.67
- `meta_biocom_iiari_260518`
  - 소재 의미: 음식물과민증분석 서비스(음과검)
  - 라이브 광고명: `meta_biocom_iiari_Igg_260518`
  - bridge 근거: adset ID `120245700952890396`, 89 rows, 결제완료 32건, 결제완료 금액 ₩10,569,200
  - Ads Manager 화면값: 7일 클릭 기준 웹사이트 구매 45건, 결과값 ₩14,096,500, 결과 ROAS 3.97

운영 해석:

1. A급 숫자 ID가 있는 row는 광고세트 단위에서 종대사/음과검 소재별로 읽을 수 있다.
2. 광고세트명이 수정되면 보고서 이름도 더 직관적으로 맞출 수 있다.
3. D급 16건은 숫자 ID가 없으므로 이 회신만으로 특정 소재에 자동 배정하지 않는다.
4. 이아리 공구는 2026-05-26 종료 예정이다. 2차 활용 광고가 열리면 신규 광고부터 고유 alias와 숫자 ID URL Parameters를 확인한다.

## 아직 확정하지 않은 것

- 16건이 어느 광고소재 1개에서 발생했는지는 아직 확정하지 않았다.
- 현재 원장에는 16건을 특정 광고소재로 자동 배정할 숫자 ID가 없다.
- Meta의 미리보기/공유/검토 클릭 경로 때문인지, 특정 소재 클릭 경로 때문인지는 추가 재현이 필요하다.
