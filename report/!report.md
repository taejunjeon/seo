...
2026-05-28 쿠팡 판매 분석 확인: F&B팀이 준 쿠팡 판매 분석 엑셀은 2026-04-25 - 2026-05-01 합계가 2,453,600원 / 주문 60건 / 판매량 71개로, 기존 주간 보고서 기준 2,100,400원 / 56건과 다르다. 쿠팡 Open API read-only 재현값은 판매자배송 커피 1,132,700원 + 로켓그로스 907,000원 = 2,039,700원이고 60,700원 / 2건 gap이 남는다. 첨부 엑셀과 기존 기준값 차이 353,200원은 주문서 API의 팀키토 상품 버킷과 정확히 같아, 같은 판매 분석 화면이라도 상품 필터가 달랐을 가능성이 높다. 공식 Open API에서 판매 분석 화면 요약 파일을 직접 내려주는 endpoint는 찾지 못했으므로, 정확한 F&B 기준 자동화는 Hermes 브라우저 다운로드를 primary 후보로 둔다. 상세는 [[reportcoffee-coupang-seller-insights-api-readiness-20260528]]에 둔다.
