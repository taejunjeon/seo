...
- `allow_purchase` 결과만 2분 TTL로 저장한다.
- `block_purchase_virtual_account`와 명시적 `block_purchase`는 30초 이하로만 저장한다.
- 명시적 server `unknown`은 10초 이하로만 저장한다.
- `decision_fetch_failed`, endpoint error, parse failure는 캐시하지 않는다.
- 기존 `allow_purchase` 캐시가 있으면 failure/unknown이 덮어쓰지 못한다.
- 캐시 키는 주문/결제 식별자를 직접 쓰지 않고 canonical material을 hash해서 만든다.
- `orderCode/orderNo/paymentCode`와 `order_code/order_id/order_no/payment_code/payment_key`가 같은 결제건이면 같은 cache key를 쓰도록 tier를 정규화한다.
