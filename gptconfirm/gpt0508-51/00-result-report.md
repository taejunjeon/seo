...
- 500 `terminated` 오류 대응: VM Cloud 원격 read 재시도, 서버 파일 캐시, 브라우저 저장본 fallback 추가.
- 화면 UX 보강: 계산 중 progress bar 표시, 새 계산 중에도 기존 결과 유지, 같은 기간은 저장본 즉시 표시.
...
- API smoke: PASS.
- API cache hit smoke: PASS.
- API force refresh smoke: PASS.
- Desktop Playwright smoke: PASS.
- Frontend browser cache smoke: PASS.
- Frontend progress smoke: PASS.
...
TJ님 화면의 `감사 데이터를 불러오지 못했습니다. terminated`와 `500 Internal Server Error`는 데이터가 없어서가 아니라 VM Cloud 보조 원장 read 요청이 중간에 끊겼을 때 백엔드가 그대로 실패를 반환한 문제였다.

수정 후 동작은 아래와 같다.

- 첫 계산 성공 시 백엔드가 `backend/data/tiktok-off-impact-audit-cache.json`에 서버 저장본을 만든다. 이 파일은 `backend/data/` ignore 범위라 커밋하지 않는다.
- 같은 날짜 조합으로 다시 열면 서버 저장본을 즉시 반환한다.
- 브라우저도 같은 결과를 `localStorage`에 저장한다.
- 사용자가 `다시 계산`을 누르면 `refresh=1`로 새 계산을 하고, 계산 중에는 progress bar를 보여준다.
- 새 계산이 실패해도 브라우저 저장본이 있으면 화면은 저장본을 계속 보여주고 오류 원인만 경고로 표시한다.
