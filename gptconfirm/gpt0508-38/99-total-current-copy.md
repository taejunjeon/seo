...
| 항목 | 상태 |
|---|---|
| Track A. ConfirmedPurchasePrep | 99% (R2 wire fixture PASS + ledger_lookup readiness) |
| Track B. Google Ads campaign_id 조인/ROAS 분해 | 93% (operationalDbFreshness + npayActualCorrection 라이브) |
| Track C. BigQuery campaign funnel quality | 85% |
| Track D/KR6. Meta funnel CAPI Test Events readiness | 74% (env blocker 유지) |
| Track E. Harness/HOLD Reducer | 97% (4-signal canary decision tree + Data Source Decision Guide v1) |
| Track F. Frontend/Data Trust Dashboard | 79% (정적 카드 + dynamic NPay correction + freshness backend wire) |
| R2 wire 운영 배포 | DEPLOY PASS, write_flag=false 기본, 1h canary 진행 중 (KST 02:41 cutoff) |
| GTM Custom HTML | PARKED (R2 fail 시 fallback) |
| imweb footer 직접 수정 | PARKED_LAST_RESORT |
| 운영DB sync lag | 78분 (lagged) — 직전 9h 에서 회복 |
| NPay actual confirmed 30d | 209건 / ₩37,638,900, internal ROAS 0.27 → 1.91 보정 |
| Google Ads upload / send / conversion action 변경 | 0 (sprint invariant) |
| GTM Production publish | 0 |
| 운영DB write | 0 |
| raw email/phone/order/payment/member_code 저장/logging | 0 |
...
harness_preflight:
  ...
  lane: Yellow 승인된 VM dashboard local_first 제한 배포 결과 기록 + Green BigQuery/ConfirmedPurchasePrep dry-run 문서화
  allowed_actions:
    - 실제 개발 순서 기준 정본 문서 업데이트
    - Google Ads ROAS 정합성 다음 작업만 실행 보드에 추림
    - Google Ads API read-only dashboard 조회
    - 승인된 VM dashboard local_first 제한 배포와 read-only smoke 기록
    - BigQuery archive+daily export union dry-run
  ...
  source_window_freshness_confidence:
    ...
    window: "2026-04-10~2026-05-09 KST Google Ads dashboard last_30d, 2026-04-11~2026-05-10 KST ConfirmedPurchasePrep 운영DB dry-run, 2026-05-03~2026-05-09 / 2026-04-26~2026-05-09 / 2026-04-10~2026-05-09 KST BigQuery union windows"
    freshness: "Google Ads dashboard VM local_first 배포 후 2026-05-10 20:17~20:18 KST status/last_7d/last_30d smoke PASS. BigQuery archive+daily union 2026-05-10 20:13 KST PASS. ConfirmedPurchasePrep repeatable no-send input 2026-05-10 20:22 KST 반영. Google Ads upload/send/publish 없음"
    confidence: 0.96
```
...
2026-05-10 20:18 KST 승인된 VM dashboard `local_first` 제한 배포를 완료했다. `/api/google-ads/status`, `/api/google-ads/dashboard?date_preset=last_7d`, `/api/google-ads/dashboard?date_preset=last_30d`가 모두 200이다. `last_30d`는 502에서 200으로 회복됐고, 내부 원장 조회는 VM 프로세스 안의 SQLite를 우선 읽는다. Google Ads ROAS=광고 플랫폼이 주장하는 값은 `last_30d 9.58x`이고, 내부 confirmed ROAS=실제 결제완료 주문 원장 기준값은 `last_30d 0.27x`다. 이 둘은 예산 판단 화면에서 반드시 분리해야 한다. Google Ads upload/send, GTM publish, raw PII/order/payment logging 증가는 모두 0이다. [[../gdn/google-ads-dashboard-local-first-deploy-result-20260510]]

2026-05-10 20:13 KST BigQuery archive+daily union dry-run은 PASS다. 바이오컴 자사몰은 최근 Hurdlers 관리 BigQuery에서 우리 관리 프로젝트로 넘어왔기 때문에, 신규 daily export만 보면 2026-05-07~2026-05-09 3일치만 보여 7/14/30일 추세 비교가 불가능했다. 이번에는 archive backfill `<=20260506`과 신규 daily export `>=20260507`을 합쳐 `last_7d=7개 날짜`, `last_14d=14개 날짜`, `last_30d=30개 날짜`를 실제로 읽었다. 따라서 source coverage warning은 “daily only 기준”에서는 유지하지만, archive+daily union 기준으로는 trend 비교 입력을 만들 수 있다. [[../gdn/bigquery-archive-daily-union-dry-run-20260510]]

## 현재 작업 OKR / 액션플랜 / 완성도

목표는 Google/NPay ROAS 과대계상 문제를 내부 confirmed purchase 기준으로 바로잡고, 실제 구매만 Google Ads/ROAS 판단 후보가 되도록 만드는 것이다.

현재 전체 완성도는 약 **77%**다. Path B 주문완료 bridge와 NPay status guard는 많이 닫혔고, Google click id 보유 confirmed 주문 31건은 campaign_id까지 조인됐다. dashboard route 502는 VM `local_first` 제한 배포로 회복됐고, BigQuery archive+daily union으로 7/14/30일 source coverage도 PASS했다. 다만 전체 confirmed 2,152건 중 campaign_id coverage가 낮고, Google Ads upload 전 no-send 후보 검증과 Red 승인 패킷은 아직 남아 있다.

### Track A~F 현재 진척률

- Track A. ConfirmedPurchasePrep 통합 input: **90%**. 직전 89%에서 +1%.
- Track B. Google Ads campaign_id 조인/ROAS 분해: **73%**. 직전 68%에서 +5%.
- Track C. BigQuery campaign funnel quality: **76%**. 직전 66%에서 +10%.
- Track D/KR6. Meta funnel CAPI Test Events readiness: **70%**. 직전 70%에서 +0%.
- Track E. Harness/multi-agent/HOLD Reducer: **87%**. 직전 87%에서 +0%.
- Track F. Frontend/Data Trust Dashboard: **46%**. 직전 42%에서 +4%.
...
|---:|---|---|---|---|---|---:|---|---|---|---|
| P0 | [[#Phase4-Sprint6]] | Google 광고 클릭 ID를 주문 후보까지 보존하고, NPay/가상계좌 결제 상태를 정확히 가른다 | Google Ads에 실제 결제완료 주문만 알려주려면 클릭, 주문, 로그인 identity가 이어지고 미결제/클릭-only 주문은 막혀야 한다 | paid_click_intent 수집 -> Path B order bridge -> NPay confirmed status guard -> unpaid/test exclusion -> confirmed-only dry-run 순서로 닫는다 | 실제 Google 광고 클릭 no-send bridge PASS. VM Cloud biocom/thecleancoffee status sync 1회 완료. 운영DB PAYMENT_COMPLETE 기반 최근 30일 repeatable input은 2,152건(homepage 2,009/NPay 143), send_candidate 0, upload 후보 0 | 98% | confirmed 결제완료만 후보로 남고, 미입금 가상계좌·NPay click/add_payment_info·controlled evidence는 upload 후보 0건으로 막힌다 | Codex: campaign_id coverage와 Google Ads upload Red 승인안 준비. TJ: 추가 실제 결제/입금 테스트는 보류 추천 | NO, Green. upload는 Red HOLD | [[../gdn/path-b-real-paid-click-actual-order-preview-result-20260510]], [[../gdn/confirmed-purchase-builder-repeatable-runbook-20260510]], [[../gdn/confirmed-purchase-npay-status-guard-20260510]], [[../gdn/vm-cloud-imweb-sync-status-review-20260510]] |
...
#### A0. Google Ads dashboard route를 정기 비교 입력으로 고정한다
- 무엇을 하는가: VM Cloud dashboard `last_7d`/`last_30d` 200 상태를 기준으로 플랫폼 주장 ROAS와 내부 confirmed ROAS를 반복 비교한다.
- 왜 하는가: Google Ads ROAS=광고 플랫폼이 주장하는 값과 내부 confirmed ROAS=실제 결제완료 원장 기준값을 같은 window로 보려면 dashboard route가 안정적이어야 한다.
- 어떻게 하는가: `GOOGLE_ADS_DASHBOARD_LEDGER_MODE=local_first` 상태에서 `/api/google-ads/dashboard?date_preset=last_7d,last_30d`를 read-only로 호출하고 source/window/freshness/confidence를 남긴다.
- 성공 기준: last_7d/last_30d 모두 HTTP 200, platform send 0, raw logging delta 0, send_candidate=true 0.
- 실패 시 다음 확인점: PM2 env 반영 여부, VM SQLite path, attribution ledger local read error, proxy timeout.
- 승인 필요 여부: NO, read-only. 추가 VM deploy/restart는 별도 승인.
- 산출물: [[../gdn/google-ads-dashboard-local-first-deploy-result-20260510]].
- 진척률에 미치는 영향: Track B 73% 유지/확장, Track F 46% 유지/확장.
- 의존성: local_first 제한 배포 PASS 완료.
