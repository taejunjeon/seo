...
| 우선순위 | source |
|---|---|
| primary | 운영DB `tb_iamweb_users` WHERE `payment_method='NAVERPAY_ORDER' AND payment_status='PAYMENT_COMPLETE'` |
...
### 질문 2. “이 매출 중 NPay는?”
1. 운영DB `tb_iamweb_users` WHERE `payment_method='NAVERPAY_ORDER' AND payment_status='PAYMENT_COMPLETE'`
