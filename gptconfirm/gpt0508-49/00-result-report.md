...
harness_preflight:
  ...
  source_window_freshness_confidence:
    ...
    window: thecleancoffee NPay last 30 days, post-snapshot checked 2026-05-13 00:57 KST, latest read-only checked 2026-05-13 02:02 KST
    freshness: live coffee post-snapshot=309/14902800, latest read-only=311/14970600, status_blank=16/1012700, warnings include status_sync_stale_over_6h
    confidence: 0.91
```

## 한 줄 결론

- 결론: 더클린커피 NPay 매출을 `bridge_pending`에서 live actual included 후보로 올리는 VM backend 배포/restart까지 PASS했다. 롤백 조건은 발생하지 않았고, ROAS gap 재계산과 로컬 프론트 대시보드 고도화까지 Green 범위에서 진행했다.
...
live post-snapshot 기준 더클린커피 최근 30일 NPay actual confirmed는 `included_with_warning` 309건 / 14,902,800원이다. source는 `imweb_v2_vm_cloud_imweb_orders`다. 2026-05-13 02:02 KST latest read-only 기준은 311건 / 14,970,600원이고 status blank는 16건 / 1,012,700원이다. warnings에는 `ga4_guard_not_actual_source`, `status_blank_rows_included_with_warning`, `status_sync_stale_over_6h`가 들어 있다.
...
`status blank`는 VM Cloud SQLite `/home/biocomkr_sns/seo/repo/backend/data/crm.sqlite3`의 `imweb_orders.imweb_status`가 비어 있다는 뜻이다. 운영DB PostgreSQL `dashboard.public.tb_iamweb_users`의 결제 상태 blank가 아니다. 이번 확인에서 blank row 16건은 모두 `imweb_status_synced_at` marker가 없고 최신 status sync 이후에 order sync로 들어와, 원인은 `source_freshness_gap/status sync lag`다.
...
- 추천 방향에 대한 자신감: 92%
- Lane: Green
- 무엇을 하는가: `site=thecleancoffee` summary API와 VM Cloud SQLite `imweb_orders`를 read-only로 다시 확인하고 311건 / 14,970,600원 근처가 유지되는지 본다.
...
2. `/total` correction line contract
- Codex 추천: 바로 진행 추천
- 추천 이유: ROAS recompute는 완료됐고, 이제 coffee actual을 dashboard correction 합계에 섞지 않는 공통 contract가 필요하다.
- 추천 방향에 대한 자신감: 88%
- Lane: Green
- 무엇을 하는가: `project/sprint2.md` 기준으로 coffee actual line item을 `/total`과 dashboard contract에 붙인다.
- 성공 기준: 예산 판단값과 참고값이 분리되고, GA4 revenue를 actual source로 쓰지 않는다.
- 실패 시 해석/대응: campaign/site spend mapping 부족이면 `cross_site_reference_only`로 유지한다.
- 승인 필요: NO, read-only/design이다.
