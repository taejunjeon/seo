...
| latest read-only included_with_warning | 318건 / 15,503,000원 |
| latest read-only status blank | 26건 / 1,663,600원 |
...
2026-05-13 10:37 KST root-cause query에서 blank 26건은 모두 `imweb_status_synced_at` marker가 없었다. 또 VM Cloud SQLite `imweb_orders.synced_at`은 `2026-05-13 01:30:03`까지 진행됐지만 최신 status sync 시각은 `2026-05-12 04:11:07`에서 멈춰 있었다. 따라서 현재 원인은 결제 실패가 아니라 `source_freshness_gap/status sync lag`다.
...
| 옵션 | 포함 숫자 | 장점 | 리스크 | 추천 |
|---|---:|---|---|---|
| `included_with_warning` | latest 318건 / 15,503,000원 | actual 누락을 줄이고 blank/freshness를 화면에 같이 노출 | status blank가 나중에 cancel로 바뀔 수 있음 | 추천 86% |
