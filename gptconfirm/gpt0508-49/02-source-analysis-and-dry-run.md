...
더클린커피 actual source는 운영DB `tb_iamweb_users`가 아니라 Imweb v2 / VM Cloud `imweb_orders(site='thecleancoffee')`로 봐야 한다. GA4 BigQuery는 구매 매출 source가 아니라 이미 GA4에 들어간 주문인지 확인하는 guard로만 쓴다.
...
| window | 최근 30일 |
| source | VM Cloud SQLite `imweb_orders` |
| site | `thecleancoffee` |
...
| 옵션 | 포함 숫자 | 장점 | 리스크 | 추천 |
|---|---:|---|---|---|
| `included_with_warning` | 308건 / 14,835,000원 | actual 누락을 줄이고 blank/freshness를 화면에 같이 노출 | status blank가 나중에 cancel로 바뀔 수 있음 | 추천 86% |
...
- source: VM Cloud SQLite `imweb_orders`, Imweb v2 captured order header.
