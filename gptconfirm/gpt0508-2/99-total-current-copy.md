...
최종 업데이트: 2026-05-08 23:03 KST
기준일: 2026-05-08
상태: active canonical
Owner: total / attribution
Supersedes: [[!total_past|기존 Phase 순서 정본]]
Next document: [[../gdn/path-b-no-send-hmac-local-implementation-20260508]] Path B no-send HMAC 로컬 구현 / [[../gdn/path-b-preview-tag-draft-20260508]] Path B Preview tag 초안 / [[../gdn/canary-capture-health-24h-20260508]] 24h 수집 건강검진 결과
...
2026-05-08 23:03 KST에는 Path B no-send HMAC endpoint 로컬 초안과 fixture test를 추가했다. `POST /api/attribution/order-bridge/identity-hmac/no-send`는 로컬 코드에만 추가됐고 운영 배포는 하지 않았다. fixture smoke 4개가 PASS했으며, raw echo 0, raw logging 0, no platform send 0 assertion을 통과했다. Preview tag 초안도 문서화했지만 GTM Production publish나 Imweb 저장은 하지 않았다.
...
|   2 | [[#Phase4-Sprint6]] | path_b_preview_yellow_candidate | Codex + TJ | Path B email/phone hash-only Preview와 no-send HMAC smoke 승인 여부를 판단한다 | Path C member_code source가 없어 회원/비회원 주문을 잇는 다음 결정적 연결키가 필요하다                       | [[../gdn/path-b-email-phone-hash-bridge-approval-20260508]], [[../gdn/path-b-email-phone-preview-plan-20260508]], [[../gdn/guest-order-attribution-ledger-design-v2-20260508]] 기준으로 raw 저장 없이 hash-only smoke를 준비한다 | 문서 완료. 실행은 GTM Preview + backend no-send smoke 승인 필요 | [[#Phase4-Sprint6]] / [[../gdn/path-b-email-phone-hash-bridge-approval-20260508]]          | YES, Preview/smoke 실행 |
...
| P0 | path_b_local_endpoint_done | Phase4-Sprint6 | wrapper Preview + Path B fallback bridge | member_code source가 없어 회원/비회원 주문을 잇는 hash-only bridge가 필요하다 | 비로그인 safety PASS. 로그인 NPay 클릭 200/201/203과 source 재탐색 모두 usable source 없음. Path B no-send HMAC endpoint 로컬 초안, fixture PASS, Preview tag 초안 작성. 운영 deploy/Production publish HOLD | Codex | YES 완료, 실행은 별도 Yellow | [[../gdn/path-b-no-send-hmac-local-implementation-20260508]] |
...
현재 open approval: **wrapper Preview only 승인 완료**. backend deploy, Production publish, actual send 관련 open approval은 **없음/HOLD**.
...
| 2026-05-08 23:03 KST | Path B no-send HMAC endpoint 로컬 구현 + Preview tag 초안 | `POST /api/attribution/order-bridge/identity-hmac/no-send` 로컬 초안, email/phone normalize, HMAC, raw echo 0, raw logging 0, no platform send 0 fixture test 작성. `npm --prefix backend run typecheck` PASS, `node --import tsx --test tests/order-bridge-identity-hmac.test.ts` 4 PASS. 운영 deploy/GTM publish 없음. 결과: [[../gdn/path-b-no-send-hmac-local-implementation-20260508]], [[../gdn/path-b-preview-tag-draft-20260508]] |
