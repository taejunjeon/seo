...
|---:|---|---|---|---|---|---:|---|---|---|---|
| P0 | [[#Phase4-Sprint6]] | Google 광고 클릭 ID를 주문 후보까지 보존하고, NPay/가상계좌 결제 상태를 정확히 가른다 | Google Ads에 실제 결제완료 주문만 알려주려면 클릭, 주문, 로그인 identity가 이어지고 미결제/클릭-only 주문은 막혀야 한다 | paid_click_intent 수집 -> Path B order bridge -> NPay confirmed status guard -> unpaid/test exclusion -> confirmed-only dry-run 순서로 닫는다 | 실제 Google 광고 클릭 no-send bridge PASS. NPay `complete_time`/`imweb_status` blank 단독 차단 금지 guard fixture PASS | 94% | confirmed 결제완료만 후보로 남고, 미입금 가상계좌·NPay click/add_payment_info·controlled evidence는 upload 후보 0건으로 막힌다 | Codex: confirmed-only dry-run 재측정. TJ: 추가 실제 결제/입금 테스트는 별도 판단 | NO, Green. upload는 Red HOLD | [[../gdn/path-b-real-paid-click-actual-order-preview-result-20260510]], [[../gdn/confirmed-purchase-npay-status-guard-20260510]], [[../gdn/npay-channel-order-mapping-dry-run-20260510]] |
...
#### A1. NPay actual confirmed status guard를 confirmed purchase input에 연결한다
- 무엇을 하는가: NPay 실제 결제완료 주문은 운영DB/admin confirmed source 기준으로 포함하고, VM Cloud `complete_time`/`imweb_status` blank만으로 차단하지 않는다.
- 왜 하는가: NPay 결제완료 주문도 배송대기/STANDBY 단계에서는 `complete_time=0`일 수 있고, VM Cloud status sync가 안 돌면 `imweb_status`가 전체 blank일 수 있다.
- 어떻게 하는가: [[../gdn/confirmed-purchase-npay-status-guard-20260510]] 기준으로 `PAYMENT_COMPLETE` 또는 `admin_confirmed=true`를 primary로 둔다. `npay_click`, `npay_count`, `payment_start`, `add_payment_info`, controlled evidence, unpaid/pending은 excluded order로 분리한다.
- 성공 기준: ConfirmedPurchasePrep fixture/dry-run에서 NPay actual confirmed는 후보에 남고, click-only/미완료/test evidence는 upload 후보 0건이다.
- 실패 시 다음 확인점: 운영DB 결제 상태 필드명, 관리자 confirmed source, NPay channel order mapping freshness.
- 승인 필요 여부: NO, Green local code/doc.
- 산출물: [[../gdn/confirmed-purchase-npay-status-guard-20260510]], `data/confirmed-purchase-npay-status-guard-dry-run-20260510.json`.
...
**현재 진척률**: 전체 기준 94% / Google Ads 실제 전송 기준 0%.
...
- NPay status guard를 운영DB 기반 ConfirmedPurchasePrep dry-run input에 연결하고, homepage/NPay actual confirmed 후보를 재측정한다.
