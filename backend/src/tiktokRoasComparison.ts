...
const haystackForEntry = (entry: RemoteLedgerEntry) => {
...
const isTikTokPaymentSuccess = (entry: RemoteLedgerEntry) => (
  ...
  && (haystackForEntry(entry).includes("tiktok")
    || haystackForEntry(entry).includes("ttclid")
    || Boolean(readString(entry.ttclid)))
);
...
const buildOperationalSummary = (entries: RemoteLedgerEntry[]) => {
  ...
  for (const entry of entries.filter(isTikTokPaymentSuccess)) {
    ...
    aggregate.orderSet.add(readString(entry.orderId) || readString(entry.paymentKey) || readString(entry.loggedAt));
    if (samples.length < 10) {
      samples.push({
        loggedAt: readString(entry.loggedAt),
        orderId: readString(entry.orderId),
        paymentStatus: key,
        amount: readEntryAmount(entry),
        utmSource: readString(entry.utmSource),
        utmCampaign: readString(entry.utmCampaign),
        hasTtclid: Boolean(readString(entry.ttclid)),
      });
    }
  }
