...
// Updated HTML — google_tag_manager[CID].dataLayer.set() 방식
// 피드백 gptfeedback_gtm_0421_2.md 반영:
//   - 핵심 주입: dataLayer.push() → google_tag_manager['GTM-W2Z6PHN'].dataLayer.set()
//   - 디버그 push 는 선택적, [154] 가 읽는 핵심 경로 아님
//   - guard 전부 로그 강화 (Preview 에서 어느 guard 가 어떻게 동작했는지 관찰 가능)
const HTML = `<script>
(function(){
  ...
  try {
    ...
    // ========== Guard 3: window.__seo_allow_purchase 플래그 ==========
    // header_purchase_guard 가 payment-decision 을 받아 false 로 설정한 경우 차단
    var allowFlag = null;
    try {
    ...
    // 피드백 근거: Google 공식 Tag Sequencing 문서 — setup tag 에서
    // 같은 이벤트 내 primary tag 가 즉시 읽어야 하는 값은 dataLayer.set() 사용.
    // dataLayer.push() 는 다음 이벤트에서 인식될 수 있다.
    ...
    // ========== 디버그용 push (Preview Data Layer 탭 관찰용; event 키 없음) ==========
    // [154] 가 읽는 핵심 경로 아님. 순수 debug trace.
    try {
...
console.log('html length:', (tag.parameter?.find(p => p.key === 'html')?.value || '').length);

// replace html param
const htmlParam = tag.parameter.find(p => p.key === 'html');
if (!htmlParam) { console.error('no html param'); process.exit(1); }
htmlParam.value = HTML;

// update notes
tag.notes = (tag.notes || '') + '\n2026-04-21 00:45 KST: dataLayer.push → google_tag_manager[CID].dataLayer.set 변경 (gptfeedback_gtm_0421_2.md 수용). guard 로그 강화.';
...
console.log('html length:', (updated.parameter?.find(p => p.key === 'html')?.value || '').length);

// confirm html contains dataLayer.set, NOT gtag(
const finalHtml = updated.parameter.find(p => p.key === 'html').value;
console.log('\n=== content verification ===');
console.log('  contains google_tag_manager[CID].dataLayer.set:', /google_tag_manager\[CONTAINER_ID\][^]*dataLayer\.set/.test(finalHtml));
console.log('  forbidden gtag event purchase:', /gtag\(["']event["'],\s*["']purchase["']/.test(finalHtml));
