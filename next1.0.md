...
### Sprint 6: 고급 프론트엔드 ✅ (부분 완료)

> **목적**: UX 완성도 향상
> **결과**: 6.1 비교 차트 + 6.3 CSS 토큰 문서화 완료. 6.2 반응형/6.4 다크모드는 현재 불필요(SKIP).

| # | 작업 | 담당 | 규모 | 상태 |
|:-:|------|:----:|:----:|:----:|
| **6.1** | **이전 기간 비교 차트** | Claude Code | 중 | ✅ TrendChart를 `/api/trends` 전환 + 이전 기간 오버레이(점선) + 변동률 배지 + 비교 요약 패널. position 데이터도 지원 |
| **6.2** | **반응형 모바일 최적화** | — | 대 | ⏭ SKIP (현재 불필요) |
| **6.3** | **CSS 변수 디자인 토큰 문서화** | Claude Code | 소 | ✅ `frontend/docs/design-tokens.md` — 색상/스페이싱/반경/폰트/그림자/트랜지션/글래스모피즘/차트색상/AI소스색상 정리 |
| **6.4** | **다크 모드** (선택) | — | 대 | ⏭ SKIP (현재 불필요) |
