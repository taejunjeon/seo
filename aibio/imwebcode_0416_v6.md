...
```html
<!-- AIBIO form-submit attribution v6 -->
<script>
(function () {
  var CONFIG = {
    ...
    logPrefix: '[aibio-form-submit-v6]',
    firstTouchKey: '_p1s1a_first_touch',
    lastTouchKey: '_p1s1a_last_touch',
    sessionTouchKey: '_p1s1a_session_touch'
  };
  ...
  function rememberPendingSubmit(widgetId, triggerType) {
    ...
    debugLog('pending_submit', pendingSubmit);
  ...
  window.__AIBIO_FORM_SUBMIT_V6__ = {
    snippetVersion: CONFIG.snippetVersion,
    source: CONFIG.source
  };
})();
...
아래처럼 테스트 URL로 접속해서 폼을 1건 제출한다.

```text
https://aibio.ai/59?utm_source=meta&utm_medium=paid_social&utm_campaign=meta_recoverylab_test_0416&utm_content=test_form_0416&fbclid=test_fbclid_0416&__seo_attribution_debug=1
```

확인할 것:

- 브라우저 콘솔에 `[aibio-form-submit-v6] send_payload`가 보이는지
- `/acquisition-analysis`에서 AIBIO 테스트 제출이 운영 분석에서 제외되는지
- 원장 row metadata에 `snippetVersion=2026-04-16-aibio-form-submit-v6`, `fbc`, `fbp`, `first_touch`, `last_touch`가 들어오는지
