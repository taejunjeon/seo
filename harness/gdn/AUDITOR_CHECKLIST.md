...
| GTM workspace lifecycle | Default Workspace 사용, backup 없는 cleanup, fresh workspace 전 write flag ON, 승인 없는 submit/create_version/publish |
...
| HOLD reducer 누락 | status=HOLD인데 `auto_green_followups_done`이 없거나 Green follow-up을 TJ님 할 일로 넘김 |
| GTM workspace preflight 누락 | GTM Preview인데 workspace capacity preflight 또는 live version unchanged 확인이 없음 |
...
| HOLD Reducer | HOLD이면 `hold_reason_category`, `auto_green_followups_done`, `remaining_blocker` 필수 |
| GTM Workspace Lifecycle | GTM Preview 작업이면 fresh workspace, capacity preflight, live unchanged 필수 |
