...
| GTM Preview workspace | workspace 이름, tag/trigger 범위, capacity preflight, cleanup backup, production publish 금지 |
...
1. live latest 기준 fresh workspace 생성 방식.
2. workspace capacity preflight.
3. old Preview workspace cleanup 필요 여부와 JSON backup 경로.
4. Default Workspace 미사용 확인.
5. submit/create_version/publish 금지.
6. VM Cloud write flag ON 조건과 cleanup 조건.
7. cleanup 후 live version unchanged 확인.
8. Preview 성공은 Production publish 승인이 아니라는 문구.

fresh workspace 생성이 실패하면 reuse는 fallback으로만 검토한다. 이때도 live version unchanged와 tag/trigger diff를 먼저 확인한다.
