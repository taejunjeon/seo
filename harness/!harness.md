...
| 부족한 것 | 왜 문제인가 | v0 보강 |
|---|---|---|
| 표준 task spec 없음 | 같은 작업을 매번 장문 프롬프트로 재설명해야 함 | `harness/npay-recovery/README.md`와 planned `TASK.md` |
| context pack 자동 로딩 없음 | Codex/Claude가 최신 숫자와 금지선을 놓칠 수 있음 | `CONTEXT_PACK.md` 설계 |
| trace/eval log schema 없음 | 실행 간 비교, 실패 패턴 축적이 어렵다 | `EVAL_LOG_SCHEMA.md` 설계 |
| auditor agent 없음 | 전송 금지/DB write 금지를 사람이 매번 확인해야 함 | `AUDITOR_CHECKLIST.md` |
| lessons-to-rules pipeline 없음 | 배운 점이 문서 여기저기에 흩어진다 | `LESSONS_TO_RULES_SCHEMA.md` |
...
harness/
  ...
  npay-recovery/
    ...
    TASK.md                 # v0 planned
    CONTEXT_PACK.md         # v0 planned
    RULES.md                # v0 planned
    VERIFY.md               # v0 planned
    APPROVAL_GATES.md       # v0 planned
    LESSONS.md              # v0 planned
    EVAL_LOG_SCHEMA.md      # v0 planned
```
...
자세한 초안은 [[npay-recovery/AUDITOR_CHECKLIST]]에 작성했다.
...
자세한 schema 초안은 [[npay-recovery/LESSONS_TO_RULES_SCHEMA]]에 작성했다.
