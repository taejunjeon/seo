...
| coffee-lesson-012 | observation | backend ledger list endpoint (`GET /api/attribution/coffee-npay-intents`) 의 응답에서 `payment_button_type` 컬럼이 **NULL 로 나옴** — sprint 19.3 ledger id=3/6/10/11/12 모두 payment_button_type=None. enforce POST 응답의 `ledger_row_preview` 에는 `npay` 가 정상 들어가지만 list 응답 매핑이 누락. backend `coffeeNpayIntentLog.ts` 의 line 308 schema / 546 preview 변환 / 762-772 INSERT SQL 모두 정상 보임. 즉 list endpoint 의 row → JSON 변환 로직에서 누락 가능 (별도 함수). publish 전 fix 필수 — 운영 monitoring 의 `payment_button_type_null_in_confirm_to_pay` EG-4 가 항상 FAIL 로 보임 | 2026-05-02 sprint 19.3 ledger 직접 조회 + backend code grep | 별도 sprint 로 backend list endpoint 의 row → JSON 변환 함수 점검 + payment_button_type 매핑 추가. publish 전 게이트 | 0.9 | Codex |
