...
| `tiktok-lesson-009` | candidate_rule | shadow 후보는 source 조회 limit과 write limit을 분리한다 | rebuild sprint에서 `--limit 50`이 원천 pixel event 조회까지 50건으로 제한되어 canary 주문 `202605036519253`이 dry-run 후보에서 빠졌다. 승인 조건의 50건 제한은 write row 상한이지 source scan 상한이 아니다 | tiktok/tiktok_events_api_shadow_rebuild_result_20260504.md §Dry-Run / Apply 결과 | candidate scan은 충분히 넓게 하고, write는 max 50으로 제한한다. 필요하면 eligible row와 hard-fail 검산 order를 selected rows에 우선 포함한다 | 1 | 0.9 | Codex | tiktok_events_api |
| `tiktok-lesson-010` | candidate_rule | manual/synthetic test order는 technical dedup 가능해도 business Events API 후보가 아니다 | v2에서 유일한 eligible row `202605035698347 / 11,900원`은 실제 광고 주문이 아니라 `codex_gtm_card_20260503_001` 테스트 URL 카드 결제 주문이었다. v2.1에서 `technical_eligible_for_future_send=1`, `business_eligible_for_future_send=0`, `block_reason=manual_test_order`로 분리했다 | tiktok/tiktok_events_api_shadow_rebuild_v21_result_20260504.md §핵심 주문 검토 | `codex/test/smoke/vm_smoke/gtm_live` 계열 ttclid/campaign/url/metadata 또는 known manual test order는 production send 후보에서 제외한다. 후보표에는 technical eligible과 business eligible을 분리해 표시한다 | 1 | 0.96 | Codex | tiktok_events_api |
