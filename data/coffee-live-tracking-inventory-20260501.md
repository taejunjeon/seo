...
**정본 백업 위치 (2026-05-01 KST 기준)**: [[coffee/!imwebcoffee_code_latest_0501|imweb 헤더/푸터 코드 latest 0501]] (총 2,292행).

이 파일은 4개 섹션으로 분리:

| 섹션 | line 범위 | 포함된 layer |
|---|---|---|
| 헤더 코드 상단 | 10~946 | `2026-04-14-coffee-server-payment-decision-guard-v3` (Purchase Guard. decisionEndpoint = `https://att.ainativeos.net/api/attribution/payment-decision`. vbank/unknown/blocked custom event 분기) |
| [헤더코드] | 947~978 | (보조) |
| [바디코드] | 979~994 | (보조) |
| [푸터코드] | 995~2292 | `2026-04-14-coffee-checkout-started-v1`, `2026-04-14-coffee-payment-success-order-code-v1`, `2026-04-15-thecleancoffee-funnel-capi-v3` |

핵심 발견 (코드 직접 확인):

| 항목 | 정답 (정본 line 번호) |
|---|---|
| funnel-capi `MIRROR_EVENTS` 목록 | line 2125~2131. `ViewContent`, `AddToCart`, `InitiateCheckout`, `AddPaymentInfo` 4종. **`Purchase` 는 의도적 제외** — Purchase Guard v3 가 단독 관리 |
| funnel-capi `SESSION_KEY` | line 2102. `var SESSION_KEY = '__seo_funnel_session'`. 형식 = `Date.now().toString(36) + Math.random().toString(36).slice(2, 8)` (예 `momrm95adwj94z`) |
| funnel-capi eid 형식 | line 2147. `eventName + '.' + key + '.' + SESSION_ID`. `key` = `content_ids[0]` 또는 `v<value>` (AddPaymentInfo 처럼 content_ids 없을 때 fallback) |
| funnel-capi sent dedup 키 | line 2115. `sessionStorage.setItem('funnelCapi::sent::' + eventId, '1')` |
| funnel-capi window 노출 변수 | line 2042 `window.FUNNEL_CAPI_CONFIG = { pixelId, endpoint, enableServerCapi, testEventCode, debug }`. line 2080~2081 `window.__FUNNEL_CAPI_INSTALLED = SNIPPET_VERSION` (예 `2026-04-15-thecleancoffee-funnel-capi-v3`) |
| funnel-capi fbq wrap 안전 가드 | line 2212. `orig.__FUNNEL_CAPI_V3_WRAPPED__` 로 이중 wrap 방지 |
| server CAPI endpoint | line 2044. `https://att.ainativeos.net/api/meta/capi/track`. 현재 `enableServerCapi=false` 라 client-only mirror 단계 |
| Purchase Guard decision endpoint | line 12~16 의 CONFIG. `https://att.ainativeos.net/api/attribution/payment-decision`. vbank/unknown/blocked 분기 처리. NPay 결제 Purchase 는 이 layer 가 단독 처리 |

이전 가설 (`funnel-capi 가 GTM tag 또는 외부 호스팅 스크립트로 들어왔다`) 은 **반증됨** — funnel-capi v3 본체가 imweb 푸터 custom code 안에 직접 박혀 있다.
...
| **`__seo_funnel_session`** | sessionStorage 키 | **정체 확정**: funnel-capi v3 가 `getOrCreateSessionId()` 로 발급한 자체 sessionId. 정본 [[coffee/!imwebcoffee_code_latest_0501]] line 2102~2112. 형식 = `Date.now().toString(36) + Math.random().toString(36).slice(2, 8)` (예 `momrm95adwj94z`). **window 변수에는 노출 안 됨**. 우리 design 의 `session_uuid` 발급 helper 는 `sessionStorage.getItem('__seo_funnel_session')` 로 simplify 가능 |
...
| NPay PC click | **2026-05-01 21:57 KST 진단 G 결과 — hook 정상 도달**. preview snippet wrap (`SITE_SHOP_DETAIL.confirmOrderWithCartItems` + 글로벌 `window.confirmOrderWithCartItems` 둘 다 wrap, v0.4) 가 호출됨. `bufferDelta: 1`, `intentSeqDelta: 1`, `lastPayload.intent_phase: "confirm_to_pay"` + `is_simulation: false`. session_uuid (`momuyeuikmcmug`) = funnel-capi `__seo_funnel_session` 재사용 OK. metadata_source: `inline_script_regex` (fallback 정상). prod 추출: `s20190901240a23893fa08` / 21,900원 / 수량 1 / estimated_item_total 21,900원 |  |
| NPay PC click 직후 funnel-capi 추가 marker | **InitiateCheckout eid 1건 추가**: `InitiateCheckout.o20260501dc2f080566c61.f0eb2`. content_ids[0] 가 `.` 포함 단일 값 (orderCode-like prefix `o` + ID). 4 layer 분석의 "NPay 외부 redirect 라 추가 marker 0 예상" 가설 **반증**. NPay click 시 imweb 이 자체적으로 fbq InitiateCheckout 을 발화 |  |
| NPay 결제 완료 callback | 미관찰 (sandbox 결제 미진행) |  |

## 8. NPay click / Purchase 관련 이벤트 유무 (2026-05-01 KST 1차 진단 + 정본 코드 분석)

| 질문 | 현재 답 |
|---|---|
| funnel-capi MIRROR_EVENTS 에 `InitiateCheckout`, `AddPaymentInfo` 등록되어 있는가 | **YES, 정본 line 2125~2131 에서 확정** (`ViewContent`, `AddToCart`, `InitiateCheckout`, `AddPaymentInfo` 4종) |
| funnel-capi MIRROR_EVENTS 에 `Purchase` 등록되어 있는가 | **NO 의도적 제외** (line 2130 주석: `Purchase 는 제외 — Purchase Guard v3 가 단독 관리`) |
| Purchase 이벤트는 어디서 처리되는가 | server payment decision guard v3 (정본 헤더 상단 layer, decisionEndpoint = `https://att.ainativeos.net/api/attribution/payment-decision`). vbank/unknown/blocked 분기 처리 |
| NPay 결제 흐름이 funnel-capi 의 `InitiateCheckout` 또는 `AddPaymentInfo` track 호출을 trigger 하는가 | **추가 코드 분석 필요** — checkout-started v1 layer 와 imweb 자체 fbq 호출이 NPay click 시 어떤 이벤트를 발화하는지 정본 line 1185~1561 (checkout-started) 추적 필요. 진단 G 의 console marker 캡처와 병행 |
| NPay PC 클릭 시 funnel-capi console marker 가 추가로 출력되는가 | 미확인 — 진단 G 로 확정 |
| `confirmOrderWithCartItems('npay', url)` 의 url 에 우리 query param 을 추가했을 때 funnel-capi 가 dedupe 키로 사용하는가 | 미확인 (sandbox 결제 + Imweb raw_data 검증 필요) — 단 funnel-capi 는 `eventId = eventName + '.' + content_ids[0] + '.' + SESSION_ID` 로 자체 발급하므로 url query param 을 dedupe 에 직접 사용하진 않을 가능성. 우리 query param 은 별도 confirmed order 매핑 키로만 사용 가능 |
| `SITE_SHOP_DETAIL.confirmOrderWithCartItems` wrap 이 NPay click 시 호출되는가 | **YES 확정** (2026-05-01 21:57 KST 진단 G v0.4 snippet 결과). bufferDelta 1, intent_phase confirm_to_pay, is_simulation false. v0.4 의 글로벌 함수 동시 wrap 정책으로 hook 도달 검증. 이전 v0.2/v0.3 미도달 원인은 thin wrapper 만 잡혔거나 페이지 reload 후 snippet 미실행으로 추정 |
| funnel-capi 가 NPay click 시 InitiateCheckout eid 박는가 | **YES 확정** (위 진단 결과). eid `InitiateCheckout.o20260501dc2f080566c61.f0eb2` 1건 추가. content_ids[0] 가 `.` 포함 단일 값. NPay click 시점에 imweb 이 fbq InitiateCheckout 호출 → funnel-capi mirror |
...
### 진단 E — sessionId 와 funnel-capi sent 목록 추출 (1줄로 축소)

MIRROR_EVENTS 와 `__seo_funnel_session` 정체는 정본 [[coffee/!imwebcoffee_code_latest_0501]] 분석으로 이미 확정. 따라서 진단 E 는 **현 페이지의 sessionId 와 sent 이벤트 목록 한 번 읽어두기** 만 하면 충분.

```js
// E (축소) — 1줄로 sessionId + sent eid 목록 + 최근 console 마지막 funnel-capi 줄 컨텍스트
({
  sessionId: sessionStorage.getItem("__seo_funnel_session"),
  funnelCapiInstalled: window.__FUNNEL_CAPI_INSTALLED || null,
  funnelCapiConfig: window.FUNNEL_CAPI_CONFIG || null,
  sentEids: Object.keys(sessionStorage)
    .filter(k => k.startsWith("funnelCapi::sent::"))
    .map(k => k.replace("funnelCapi::sent::", "")),
})
```
...
## 10. 발견의 함의 (design 측면) — 2026-05-01 KST 1차 진단 + 정본 코드 분석 반영

1. **funnel-capi sessionId 는 `sessionStorage.getItem('__seo_funnel_session')` 로 직접 읽기 가능**. 정체 확정 (정본 line 2102~2112). 우리 `session_uuid` 발급 helper 는 이 키 우선 → 부재 시 fallback 으로 simplify.
2. **funnel-capi MIRROR_EVENTS 4종 확정** (정본 line 2125~2131): `ViewContent`, `AddToCart`, `InitiateCheckout`, `AddPaymentInfo`. **Purchase 는 의도적 제외** — Purchase Guard v3 가 단독 관리.
3. **funnel-capi eid 형식 확정** (정본 line 2147): `<EventName>.<contentKey>.<sessionId>`. `contentKey` = `content_ids[0]` 또는 `v<value>`.
4. **window 노출 변수 확정**: `window.__FUNNEL_CAPI_INSTALLED` (= SNIPPET_VERSION), `window.FUNNEL_CAPI_CONFIG`. 진단 A 후보에 빠져 있어서 false 나옴 → 다음 진단 A 에서 보강.
5. `SITE_SHOP_DETAIL.confirmOrderWithCartItems` 는 thin wrapper 이고 진짜 함수는 글로벌 `window.confirmOrderWithCartItems`. wrap 시 둘 다 wrap 해야 안전.
6. preview snippet 의 PC NPay click hook 미발견 — 진단 F (직접 호출 sanity test) + 진단 G (click 직후 console marker 캡처) 로 분리 확정.
...
8. server CAPI 가 disabled 라는 건 client→server dedupe 로직이 아직 운영 안 됨. funnel-capi 가 켜질 때 dedupe 키 = funnel-capi eid 그대로 사용. server endpoint = `https://att.ainativeos.net/api/meta/capi/track`.
9. **Purchase 는 별도 트랙**: server payment decision guard v3 가 NPay/PG 결제 후 attribution 결정 후 Purchase 발화. 즉 NPay 결제 완료 후 Purchase 는 funnel-capi mirror 가 아니라 Purchase Guard 가 attribution endpoint 호출 후 직접 발화.
10. **우리 NPay intent beacon 의 역할 재정의 필요**: funnel-capi 가 이미 InitiateCheckout / AddPaymentInfo 시점에 first-party eid 를 박는다면, 우리 intent beacon 의 핵심 가치는 (a) NPay click 시점의 의도 (BUY_BUTTON_HANDLER 호출 자체 추적), (b) NPay 결제 시 Imweb URL Query Param 에 `intent_uuid` 부착했을 때 confirmed order 와 매핑 가능 여부 — 두 가지로 좁혀짐. 둘 다 funnel-capi 가 안 채우는 영역. 즉 design v0.4 는 funnel-capi 와 **공존하되 NPay 분기에서만 보강** 하는 형태로 정리.
11. **4 layer NPay 결제 흐름 분석 결과** ([[coffee-imweb-tracking-flow-analysis-20260501]]): NPay 결제는 일반 PG 결제와 달리 **`checkout-started v1` 단계가 의도적으로 비어 있다** (외부 NPay 도메인 redirect 때문). 즉 NPay click 시점 ~ 결제 완료 페이지 복귀 사이의 attribution context 가 비어 있고, **이것이 정확히 우리 NPay intent beacon 이 보강해야 할 영역**. design v0.4 의 보강 위치가 정본 코드 근거로 정당화됨. fbq wrap 중첩 순서는 funnel-capi (가장 바깥) → Purchase Guard → 원본 fbq.
12. **2026-05-01 21:57 KST 진단 G v0.4 snippet 결과** ([[coffee-npay-intent-beacon-preview-snippet-v04-20260501]]): PC NPay click hook 정상 도달 + funnel-capi InitiateCheckout eid 박음. 4 layer 분석의 한 가설 ("NPay 외부 redirect 라 funnel-capi 추가 marker 0 예상") **반증**. 실제로는 **NPay click 시점에 imweb 이 자체 fbq InitiateCheckout 발화** → funnel-capi mirror. 즉 NPay 도 InitiateCheckout 단계는 살아 있고, 비어 있는 것은 backend `checkout-started v1` attribution layer (외부 도메인 redirect 라 imweb JS 실행 0) 만임. 우리 wrap 의 가치는 (i) NPay click 자체 deterministic key (`intent_uuid`) + (ii) 비어 있는 backend checkout-started 단계 보강 — 두 영역으로 좁혀짐.
