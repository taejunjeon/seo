...
최근 업데이트: 2026-05-13 00:24 KST (gpt0508-49 더클린커피 Imweb v2 actual source 로컬 패치 규칙 추가)
문서 성격: 데이터 위치 관리 기준판
...
| 실제 결제완료 primary candidate (thecleancoffee, gpt0508-49 로컬 패치) | VM Cloud `imweb_orders(site='thecleancoffee', pay_type='npay')` + `order_time >= now-30d` + `payment_amount > 0` + `imweb_status NOT IN ('CANCEL','RETURN','EXCHANGE')` | coffee NPay actual included 후보. status blank와 freshness warning을 반드시 같이 표시 |
...
- 새 기준은 `derived.npay_revenue_30d_actual_confirmed`다. biocom은 운영DB actual confirmed aggregate를 붙인다. thecleancoffee는 gpt0508-49 로컬 패치 기준으로 VM Cloud `imweb_orders` actual candidate를 `included_with_warning`으로 주입할 수 있다. 단, live VM backend 배포 전 live API는 gpt0508-47의 `bridge_pending` 상태다.
