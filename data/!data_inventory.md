...
최근 업데이트: 2026-05-12 21:24 KST (NPay summary actual source 규칙 추가)
문서 성격: 데이터 위치 관리 기준판
...
## A15. NPay summary source rule (gpt0508-46)

`/api/attribution/site-landing/summary`의 NPay 매출은 아래 순서로 해석한다.

| 역할 | source | 쓰는 방식 |
|---|---|---|
| 실제 결제완료 primary | 운영DB `public.tb_iamweb_users` + `payment_method='NAVERPAY_ORDER'` + `payment_status='PAYMENT_COMPLETE'` + 취소/반품 제외 + 금액 양수 | 예산/매출 판단에 쓰는 actual confirmed 기준 |
| bridge source | VM Cloud 또는 로컬DB `imweb_orders.order_code/order_no` | `site_landing_ledger`의 order_code와 운영DB 주문번호를 이어주는 연결 증거 |
...
| freshness source | 운영DB `order_date/payment_complete_time`, VM Cloud `imweb_orders.synced_at/imweb_status_synced_at` | 최신성 경고와 sync lag 원인 분해 |
...
- 새 기준은 `derived.npay_revenue_30d_actual_confirmed`다. biocom은 운영DB actual confirmed aggregate를 붙이고, thecleancoffee는 운영DB site 격리가 검증될 때까지 `bridge_pending`으로 둔다.
