<!-- thecleancoffee footer full pasteable version -->
<!-- Fixed attribution endpoint confirmed 2026-04-08: https://att.ainativeos.net -->
<!-- Current GTM container GTM-5M33GC4 exposes GA4 measurement id G-JLSBXX7300 -->
<script>
(function () {
  try {
    var url = new URL(window.location.href);
    var params = url.searchParams;
    var isExternalReferrer =
      document.referrer &&
    ...
    if (!sessionStorage.getItem('_p1s1a_session_touch')) {
      sessionStorage.setItem('_p1s1a_session_touch', JSON.stringify(touch));
    }

    if (!localStorage.getItem('_p1s1a_first_touch')) {
      localStorage.setItem('_p1s1a_first_touch', JSON.stringify(touch));
    }

    if (hasMarketingSignal || !localStorage.getItem('_p1s1a_last_touch')) {
      localStorage.setItem('_p1s1a_last_touch', JSON.stringify(touch));
    }
  } catch (error) {}
})();
</script>
...
(function () {
  ...
  Promise.all([
    ...
    var payload = {
      ...
      metadata: {
        snippetVersion: '2026-04-08-coffee-fetchfix-v2',
