# 더클린커피 데이터 정합성 프로젝트 검토_old

작성 상태: old / superseded
새 정본: [[!coffeedata]]
old 처리 시각: 2026-05-07 01:52 KST
...
| 15 | VM 배포 sprint | 완료 | Codex | VM production backend 배포 사전 체크리스트 작성 + dry-run PASS | A-3 재개 선결 조건 — VM 외부 endpoint 가 살아 있어야 dispatcher 자동 검증이 의미 있다 | env diff 0 / schema migration idempotent (VM 테이블 부재) / rsync 권한 우회 (`--rsync-path="sudo -u biocomkr_sns rsync"` 동작 확인) / 로컬+VM rsync 설치 완료 / 1차 dry-run 에서 전체 repo sync 의 deletion 6,055건 위험 발견 → backend/ subset sync 로 전환 → 2차 dry-run PASS (deletion 0 / 신규 12 / 수정 3 = 전송 390KB). §6 명령 -n 제거 시 즉시 배포 가능 | [[coffee-vm-backend-deploy-precheck-20260502]] | YES, §6 배포 명령 실행은 별도 승인 게이트 |
| 16 | VM 배포 sprint | 완료 | Codex | VM endpoint reachability 확인 | 배포 직후 외부 도메인으로 신규 endpoint 도달 여부 검증 | S1 `/health` 200, S2 `/api/coffee/intent/stats` 200 (schema_version=2, enforce/window 모두 false, total_rows=0), S3 OPTIONS 204 + Origin allow `https://thecleancoffee.com` | [[coffee-vm-backend-deploy-precheck-20260502#12-배포-실행-결과-2026-05-02-kst]] | NO, read-only |
| 17 | VM 배포 sprint | 완료 | Codex | VM 배포 후 external dry-run 확인 | 외부 origin 통한 payload validation 동작 확인 (ledger write 0) | S4' POST `mode=dry_run` 200 + `ok=true` + `validation.ok=true` + `ledger_row_preview` 채워짐. ledger total_rows=0 유지, dry_run_ok 카운터 1 증가, dry_run_failed_validation 1 (S4 1차 schema 미스 흔적) | [[coffee-vm-backend-deploy-precheck-20260502#12-배포-실행-결과-2026-05-02-kst]] | NO, dry-run only |
| 18 | VM 배포 sprint | 완료 (PASS) | Codex + TJ | A-3 v2 GTM Preview dispatcher 재개 — 9 성공 기준 모두 충족 | 항목 15~17 PASS 후 runbook Step S1~T4 재진입 | smoke window id=1 (max 5, KST 02:04 만료), GTM workspace 20 + tag 80 + trigger 79 등록, Codex enforce sim 1건 INSERT + dedup, TJ 가 chrome 에서 PC NPay 2회 click → site dispatcher 2 row INSERT (UUID a3890f4e+28d2140d, intent_phase=confirm_to_pay, prod_code=s20260430baf1869c41c35 19,900원, funnel_capi_session_id=mon50xudmqa3m8 동일). Network OPTIONS 204 + POST 200, CORS allow 정상, enforce_inserted=3, enforce_deduped=3 (sweep race), invalid_origin/rate_limited/preview_only_violation 모두 0. dispatcher 1초 sweep race 발견 (운영 무해, fetch 효율 50%). cleanup 완료: window close + .env 임시 키 제거 + pm2 restart 37 → enforce/token/window 모두 false, ledger total_rows=3 test row 보존 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#a-3-v2-pass-2026-05-02-kst]] | NO, A-3 v2 PASS 처리 |
| 19 | dispatcher v2.1 sprint | v2.1 design 완료 + sprint 19.2 partial smoke | Codex + TJ | dispatcher v2.1 design + 승인 문서 + 재검증 runbook + 모니터링 자동화 — A-4 publish 선결 조건 | TJ 결정 (2026-05-02): A-4 Production publish 전 dispatcher v2.1 개선이 선결. imweb_order_code 가 deterministic join 핵심 키이므로 site dispatcher row 의 null 은 publish 전에 보강해야 한다 | **19 design (2026-05-02)**: dispatcher v2.1 HTML + A-4 publish 결정 문서 (15 섹션) + A-3 runbook v2.1 재검증 섹션 + monitoring script (read-only) + monitoring template. **19.2 partial smoke (2026-05-02 KST, Yellow Lane)**: backend prep + GTM workspace 21 + Codex sim 2 INSERT + 1 dedup + cleanup 자율 완료. **chrome 미수행** — agent 자동화 불가 (lesson coffee-lesson-009). **다음 단계**: sprint 19.3 chrome 측 dispatcher v2.1 재검증 (TJ 진행 시점) → 항목 20 publish 결정 | [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] / [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#a-3-v21-partial-smoke-2026-05-02-12-08-kst]] / [[harness/coffee-data/AUTONOMY_POLICY]] | YES, sprint 19.3 chrome 재검증 진입 별도 승인 |
| 19.3 | dispatcher v2.1 chrome 재검증 sprint | **완료 (자동화 PASS)** | Codex (Yellow Lane 자율) | dispatcher v2.1 chrome 측 동작 자동 검증 (playwright + monkey-patch + mock funnel-capi key) | TJ chrome 손 부담 없이 dispatcher v2.1 의 O1/O2/O3 자동 검증 가능성 입증 | **2026-05-02 12:38 KST PASS**: playwright + chromium 1217 (이미 설치) + 격리 디렉토리 [[harness/coffee-data/preview-playwright/a3v21_smoke.mjs]] (260줄). Scenario A (mock 안 set) → block_reason 마커 ✅ / Scenario B (hex mock) → forward INSERT (id=11, imweb_order_code 채워짐) ✅ / Scenario C (payment_button_type 누락 push) → fallback 발동 INSERT (id=10, 12) ✅ / **enforce_deduped: 0** (v2 의 50% race → 0%) / 12 성공 기준 11 PASS + 1 부분 (#7 payment_button_type 은 backend list endpoint 매핑 bug). cleanup 완료: window close, .env wc 205→201, pm2 restart 41 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#a-3-v2-1-자동-pass-playwright-2026-05-02-12-38-kst]] | NO, Yellow Lane 자율 종결 |
| 19.8 | A-4 monitoring user_agent_class split 추가 sprint | **완료 (Yellow Lane PASS)** | Codex (Yellow Lane 자율) | A-4 monitoring 에 mobile/pc/unknown bucket 별 7 지표 추가 — TJ 결정: 모바일 검증은 publish blocker 아님, monitoring 으로 자연 추적 | publish confidence 95% 유지 + mobile/pc 차이 publish 후 자연 측정 | **2026-05-02 PASS**: backend `listCoffeeNpayIntents` SELECT 에 `user_agent_class` 컬럼 추가 (sprint 19.4 와 동일 누락 패턴), monitoring script 에 uaBucket() + uaMetrics() + UA split yaml section 추가 (mobile/pc/unknown × 7 지표), monitoring template 에 §4.5 user_agent_class split 섹션 (UA-1~UA-7 + 판정 규칙). VM 재배포: rsync 1 file + tsc/build PASS + pm2 restart 49→50 (pid 233507). 자동 측정 가능: UA-1/2/3/4 (ledger 기반). manual: UA-5 (pm2 logs), UA-6/7 (join-report 별도 sprint) | [[coffee-a4-monitoring-report-template-20260502#4-5-user_agent_class-split-mobile-pc-unknown-7-지표]] | NO, Yellow Lane 자율 종결 |
| 19.7 | backend production mode 가드 sprint | **완료 (publish 차단 해소, 자신감 95% 복귀)** | Codex (Yellow Lane 자율) | A-4 publish I-1 직전 발견된 backend 차단 (smoke_window hard cap 50/2h) 해소 — 운영 모드 가드 추가 | publish 진행 시 50건 후 reject. backend 운영 모드 부재. K-1 결정 따라 진행 | **2026-05-02 KST PASS**: backend `coffeeNpayIntentLog.ts` 에 `COFFEE_NPAY_INTENT_PRODUCTION_MODE` env flag + `PRODUCTION_MODE_DAILY_QUOTA=500` + `countTodayInsertsKst()` + `production_mode_quota_exceeded` reject counter + stats 응답에 `production_mode_active/daily_count/daily_quota` 노출. `runEnforceInsert` 가드 변경: env_flag 후 (production_mode OR smoke_window) OR 분기, production_mode + smoke_window 부재 시 daily quota 검사. 검증: VM 재배포 (rsync 1 file, pm2 restart 46→47) → production_mode 활성 + smoke_window 부재 상태에서 enforce POST → **status 201 INSERT 정상** (id=16, note "INSERT OK — production mode") + dedup 정상. cleanup 완료 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#sprint-19-7-backend-production-mode-가드-2026-05-02-kst]] / [[harness/coffee-data/LESSONS#coffee-lesson-014]] | NO, Yellow Lane 자율 종결 |
| 19.6 (H-2) | real funnel-capi timing 측정 sprint | **완료 (자신감 92%→95%)** | TJ + Codex | real funnel-capi v3 의 InitiateCheckout key 발급 timing 실측 — A-4 publish 자신감 결정 | sprint 19.4 + H-1 종결 후 자신감 92% 였음. real timing 미관측이 핵심 미지 → H-2 진행 | **2026-05-02 KST PASS**: TJ chrome 1회 NPay click + observer snippet (50ms interval sessionStorage snapshot). **결과: NPay click → funnelCapi key 박힘 = +873ms** (KST 15:19:46 시작 timeline +16414ms click → +17287ms key 박힘). order_code 형식 `o\d{8}[0-9a-f]+` (`o2026050224ea5c5faa462`) — snippet 정규식 매치 정상. dispatcher v2.1 의 3초 wait window 안에 안전 (873ms ≪ 3000ms), snippet retry tick 1500ms 에 capture ✅. **sprint 19.3 의 imweb_order_code=null 원인 재해석**: v2 의 1초 sweep race (1000ms 시점에 fetch — 1500ms tick 보다 빠름). v2.1 의 wait 가 정확히 해결 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#h-2-real-funnel-capi-timing-검증-2026-05-02-kst]] / [[harness/coffee-data/LESSONS#coffee-lesson-013]] | NO, 측정만 |
| 19.5 (H-1) | mobile playwright 검증 sprint | **완료 (자신감 88%→92%)** | Codex (Yellow Lane 자율) | dispatcher v2.1 의 mobile flow (iPhone 14 emulation) 추가 검증 — A-4 publish 자신감 상승 | sprint 19.4 PASS 후 자신감 88% 였음. mobile path 미검증 우려 → H-1 진행 | **2026-05-02 KST PASS**: harness/coffee-data/preview-playwright/a3v21_mobile_smoke.mjs (260줄) 신규. iPhone 14 device emulation + `._btn_mobile_npay` selector 발견 (mobile 전용 A tag, visible) + `#naverPayWrap` (mobile invisible, PC 전용) 식별. mobile click 시 buffer 에 entry 2건 (click_to_dialog backup + confirm_to_pay main). dispatcher v2.1 가 click_to_dialog backup 을 timeout block (운영 무해) + confirm_to_pay main 을 정상 forward (id=15, payment_button_type=npay, imweb_order_code 채움, capture_delay_ms=1500). enforce_deduped=0. cleanup 완료 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#h-1-mobile-playwright-검증-2026-05-02-kst]] | NO, Yellow Lane 자율 종결 |
| 19.4 | backend payment_button_type fix sprint | **완료 (Yellow Lane PASS)** | Codex (Yellow Lane 자율) | backend list endpoint 의 payment_button_type NULL 매핑 bug fix + VM 재배포 + Playwright 재검증 12/12 PASS | sprint 19.3 의 partial → sprint 19.4 PASS. publish gate EG-4 정상화 | **2026-05-02 KST PASS**: backend `coffeeNpayIntentLog.ts` 의 `listCoffeeNpayIntents` SELECT 구문 누락 컬럼 (`payment_button_type`, `imweb_order_code_capture_delay_ms`, `ga4_synthetic_transaction_id_capture_delay_ms`) 추가 fix → 데이터 손실 0 (INSERT 처음부터 정상) → tsc PASS → rsync deploy (created 1 + transferred 1) → VM npm ci/typecheck/build/pm2 restart 41→42 → 기존 row id=1/3/6/7/10/11/12 의 payment_button_type 모두 'npay' 노출 정상 → smoke window id=4 + Playwright 재실행 → id=13 (capture_delay_ms=1500 정확 기록) + id=14 (O3 fallback npay 보정) → cleanup 완료 (window close, .env 205→201, pm2 restart 43→44, enforce/token/window 모두 false) | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#a-3-v2-1-12-12-pass-sprint-19-4-backend-bug-fix-재검증-2026-05-02-kst]] | NO, Yellow Lane 자율 종결 |
| 20 | A-4 publish 결정 sprint | **완료 (2026-05-02 LIVE)** | TJ + Codex | A-4 GTM Production publish 완료 — dispatcher v2.1 + snippet installer v1 둘 다 LIVE | TJ 명시 승인 (I-1) → backend 영구 활성 → GTM publish (1차 dispatcher v2.1, 2차 snippet installer v1) → Playwright sanity PASS | **2026-05-02 KST PASS**: backend env 영구 추가 (ENFORCE_LIVE+PRODUCTION_MODE, pm2 restart 50→51) → GTM API 로 workspace 22 + dispatcher tag 85 + trigger 84 (pageview) 생성 → TJ manual publish (live version 18) → sanity 시 **snippet 미install 차단 발견 (N-2+ 결정)** → installer workspace 23 + tag 87 + trigger 86 (windowLoaded + 8s ready retry) 생성 → TJ manual publish (live version 19) → Playwright 자동 install 검증 PASS (dispatcher_installed/snippet_installed/snippet_api_present 모두 true, siteConfirmWrapped true) + Scenario C mock buffer push → ledger INSERT 정상 (id=18, payment_button_type=npay, imweb_order_code 채움). production_mode_quota_exceeded=0, invalid_origin=0, pii_rejected=0. 5일 default monitoring 시작 가능 | [[coffee-a4-publish-decision-and-dispatcher-v21-20260502#7-publish-범위]] / [[harness/coffee-data/LESSONS#coffee-lesson-015]] | NO, A-5 monitoring 자동 진입 |
| 21 | A-5 5일 default monitoring sprint | **진행 중 (2026-05-02 publish ~ 2026-05-07)** | Codex (cron 자동) + TJ (3일 게이트 평가) | A-4 publish 후 5일 default monitoring + 3일 조기 게이트 9 조건 평가 | publish_ts=2026-05-02 16:00 KST. publish 후 자연 traffic 의 capture rate / dedup ratio / payment_button_type null / origin reject / pii reject / 5xx 추적 | **VM cron 등록 완료 (2026-05-02 16:50 KST)**: `/home/biocomkr_sns/seo/coffee-monitoring/run.sh` (chmod 755, biocomkr_sns 소유) — 매일 KST 09:00 자동 실행, yaml 출력 `/home/biocomkr_sns/seo/coffee-monitoring/{YYYYMMDD}.yaml`, log `cron.log`. 첫 manual 실행 PASS (verdict `closure-ready (auto-evaluated)`, EG-3/4/5/6 모두 PASS, EG-1 `n/a` 는 ledger items 0 — admin token 없는 read-only). +72h (2026-05-05) 시점에 9 조건 모두 PASS 면 조기 closure 인정 → A-6 진입. 일부 FAIL 시 +120h (2026-05-07) default 마감 후 7일 fallback (필요 시) | [[coffee-a4-monitoring-report-template-20260502]] / [[coffee-a4-publish-decision-and-dispatcher-v21-20260502#9-3일-조기-평가-기준]] | NO 자동 / YES 3일 게이트 평가는 TJ |
| 22 | A-6 외부 플랫폼 보강 전송 sprint | **design + skeleton 완료 (dry-run only)** | Codex (Yellow Lane 자율, design only) | A-6 design 문서 + backend skeleton + ledger join dry-run script — 외부 send 0 | A-5 closure 전 사전 진행 (O-1) — design 영역 전체. enforce 활성은 A-5 PASS 후 별도 sprint (22.1+, Red Lane) | **2026-05-02 PASS**: ① 신규 design 문서 [[coffee-a6-external-platform-send-design-20260502]] (13 섹션) — scope/payload/dedup/가드/cron flow/체크리스트/stop/monitoring 정리. GA4 MP 우선, Meta CAPI 후속. ② backend skeleton `coffeeNpaySendLog.ts` (schema `coffee_npay_send_log` 정의, `runA6SendDryRun` 동작, `runA6SendEnforce` throw — sprint 22.1+ 의무). routes 에 `POST /api/attribution/coffee-a6-send?mode=dry_run` + `GET /api/coffee/a6-send/stats` 추가. ③ ledger join dry-run script `coffee-a6-ledger-join-dry-run.ts` 실행 — 현재 ledger 13 row, real 5, real_with_imweb_order_code 3 (a6_join_eligibility 60%, 단 운영 traffic 후 재측정). tsc + audit 모두 PASS. **VM 배포 0** (skeleton 만, schema ensure 호출 안 함) | [[coffee-a6-external-platform-send-design-20260502]] | NO, design only |
| 22.1 | A-6 backend 배포 sprint | 대기 (A-5 closure 후) | Codex + TJ | sprint 22 의 backend skeleton 을 VM 배포 + dry-run 50건 검증 | A-5 closure 후 운영 ledger 충분 + capture rate ≥95% PASS 후 진입 | rsync + npm ci + tsc + build + pm2 restart → schema ensure 자동 적용 → debug endpoint validation → dry-run 50건 ROAS 시뮬레이션 | [[coffee-a6-external-platform-send-design-20260502#11-backend-구현-범위-skeleton]] | YES, VM 배포 별도 |
| 22.2 | A-6 GA4 MP enforce 활성 sprint | 대기 (22.1 PASS 후) | TJ + Codex | `COFFEE_NPAY_INTENT_A6_SEND_LIVE=true` 영구 활성 + cron batch send 시작 | 22.1 PASS + TJ 명시 send 승인 (Red Lane) | env 추가 + cron 등록 + GA4 MP send 시작 + monitoring | [[coffee-a6-external-platform-send-design-20260502#7-send-흐름-cron-또는-실시간]] | YES, send live 명시 승인 (Red Lane) |
...
23. **2026-05-02 00:57 KST Step A-2a 실제 smoke 실행 7 step PASS — TJ 9 성공 기준 모두 충족** (TJ A-2a 승인 직후 Codex 직접 진행, TJ 손 안 댐):
    - 7 step 모두 PASS: token 생성 + .env 추가 / backend 재시작 / stats 확인 (enforce=true) / window open (max=5, 30분, id=2) / INSERT 2건 (id=4/5, remaining 4→3) / stats+join-report 확인 (status pending_order_sync 2건) / window close / close 후 enforce reject / .env 키 제거 / 재시작 / 최종 enforce=false token=false window=false
    - TJ 9 성공 기준 모두 충족: insert 2건 정상, imweb_order_code 100% (2/2), intent_uuid 중복 없음, pii_rejected=0, endpoint_5xx=0, invalid_payload=0, rejected_origin/rate_limited 0, join-report pending_order_sync 2건 정상 분류, 종료 후 env/window OFF
    - ledger 2건 보존 (intent_uuid prefix `smoke_a2a_uuid_` 로 식별 가능, smoke 검증 결과 baseline 으로). TJ 가 `DELETE FROM coffee_npay_intent_log WHERE intent_uuid LIKE 'smoke_a2a_%'` 1줄로 언제든 정리 가능
    - reject_counters.enforce_inserted=2, 다른 reject 카운터 모두 0 — 가드 모두 정상 작동
    - 다음 단계 후보: A-3 (GTM Preview workspace dispatcher v2 등록 + smoke window 재활성 + 실제 NPay click 1~2건 forward 검증) — TJ 별도 승인 시 진입
22. **2026-05-02 00:48 KST Step A-2a smoke window 통제 메커니즘 + 회귀 13/13 PASS** (TJ A-1.5 조건부 승인 후속, env flag 상시 활성 비승인):
    - 3중 가드 적용: env `COFFEE_NPAY_INTENT_ENFORCE_LIVE=true` (kill switch) + `COFFEE_NPAY_INTENT_SMOKE_ADMIN_TOKEN` (auth) + smoke window record (burst limit). 셋 모두 만족해야 INSERT.
    - 신규 schema: `coffee_npay_intent_smoke_windows` (id PK / site / started_by / started_at / expires_at / max_inserts / inserted_count / manually_closed_at / note). default duration 30분 (max 120), default max_inserts 5 (hard max 50).
    - admin endpoints 3종: `POST /api/attribution/coffee-npay-intent-smoke-window` (open) + `/close` + `GET .../coffee-npay-intent-smoke-windows` (list). 모두 `Authorization: Bearer <token>` 필요.
    - enforce 분기 보강: window 미활성 / max_inserts 도달 시 `reason: smoke_window_not_active` reject. INSERT 후 window inserted_count +1. 응답에 `smoke_window: {active, window_id, expires_at, inserted_count, max_inserts, remaining}` 포함.
    - 회귀 13/13 PASS (Codex curl): admin token 401 (2종) / window 없음 reject / window open 201 / enforce INSERT OK / dedup / max_inserts 도달 reject / join report status_counts 정확 / window close / close 후 reject / ledger 정리.
    - 검증 종료 후 `.env` 임시 키 2개 제거 + backend 재시작 → 최종 enforce=false / token=false / window_active=false / total_rows=0 복귀. 운영 트래픽 0건.
    - 모니터링 윈도우: 5일 default + 3일 조기 평가 게이트 (TJ 9 조건 모두 충족 시) + 7일 fallback (애매한 경우만).
    - 승인안 v1.6 작성. TJ 활성 절차 (`.env` 임시 추가 → admin window open → INSERT → window close → `.env` 정리) 7 step 명시.
    - 가드 유지: GTM Production publish BLOCKED, dispatcher live 배포 BLOCKED, GA4 MP / Meta CAPI / TikTok Events / Google Ads send BLOCKED, 자동 dispatcher 운영 전환 BLOCKED.
21. **2026-05-02 00:30 KST A-1.5 phase endpoint hardening + dispatcher v2 보강** (TJ 조건부 승인 후속):
    - backend `coffeeNpayIntentLog.ts` hardening: Origin/Referer allowlist (`thecleancoffee.com`, `www.thecleancoffee.com`, dev bypass env `COFFEE_NPAY_INTENT_DEV_BYPASS=true`), rate limit (1초 5회, 1분 30회, sliding 60s window, key=ip+session_uuid), reject counter 14종 (pii_rejected/wrong_site/missing_required/invalid_intent_phase/preview_only_violation/payment_button_type_violation/payload_too_large/schema_version_unsupported/invalid_origin/rate_limited/enforce_disabled/is_simulation_blocked/enforce_inserted/enforce_deduped/dry_run_ok/dry_run_failed_validation), `payload_schema_version` 검증 추가.
    - `routes/coffee.ts` 에 `applyCoffeeIntentGuard()` middleware 함수 추가. dry-run + enforce alias endpoint 양쪽에 origin + rate limit 가드 통과 후에만 validation 진입.
    - stats endpoint 에 `reject_counters` + `origin_allowlist` + `rate_limit_policy` + `payload_schema_version_supported` + `dev_bypass_flag_active` + `process_started_at_ms`/`process_uptime_ms` 노출.
    - dispatcher v2 보강 (승인안 안 초안): `__coffee_intent_pending` / `__coffee_intent_sent` 분리, fetch 2xx 성공 후에만 sent 기록, 4xx 영구 실패, 5xx/network 실패 시 다음 sweep 재시도, max retry 5회 + TTL 24h, `payload_schema_version: 1` 명시.
    - 모니터링 윈도우 7일 → **default 5일 + 3일 조기 평가 게이트** (joined ≥ 90% AND 24h grace 통과 row ≥ 20건). 표본 크기 vs 95% CI 분석으로 정당화.
    - 모니터링 지표 8종 추가 (rejected_origin / invalid_payload / pii_rejected strict / rate_limited / endpoint_4xx_5xx / dispatcher_fetch_failed / retry_success / duplicate_intent).
    - live publish 중단 조건 6종 → 11종으로 확대.
    - 승인안 v1.5: "No-deploy verified" → "**No external send / No DB write / No GTM publish verified**" 정정. 실제 운영 변경 항목 (backend 재시작 2회, schema migration v2, route 4종 활성, hardening 활성) 명시.
    - 회귀 검증 10종 PASS (Codex curl): 허용 origin → ok=true, 허용 referer → ok=true, 다른 origin → CORS+invalid_origin 두 layer 차단, 빈 origin → invalid_origin reject, wrong site / PII / enforce flag 미활성 / rate limit (5회 후 429) / stats reject_counters 14종 노출 + origin_allowlist + rate_limit_policy / ledger row count 0 유지.
    - tsc PASS + backend production 재시작 (PID 99378 → 2837 → 5596). enforce flag 미설정으로 운영 INSERT 0건.
    - TJ 승인 항목 6종 중 1/4/6 즉시 OK, 2/5 보강 완료, 3 (env flag 활성) 은 별도 phase A-2 대기.
20. **2026-05-02 00:11 KST A-1 phase enforce 준비 + 제한적 live capture 승인안 작성** ([[coffee-npay-intent-ledger-enforce-approval-20260501]]):
    - backend `coffeeNpayIntentLog.ts` schema v2 정의 (id PK AUTOINCREMENT + (site, intent_uuid) UNIQUE + (site, imweb_order_code, intent_uuid) UNIQUE + 5종 인덱스). schema_versions 메타 테이블 + migration abort 가드 (row count > 0 시 reject).
    - validation 강화: PII (phone/email/name/address/orderer_*/receiver_*/option_text/memo/delivery_memo) reject + `preview_only !== true` reject + `is_simulation: true` ledger 진입 금지.
    - dry-run / enforce 분기 함수 추가. enforce 는 env flag `COFFEE_NPAY_INTENT_ENFORCE_LIVE=true` 일 때만 실제 INSERT, default false → 항상 reject.
    - endpoint 4종 추가: `POST /api/attribution/coffee-npay-intent` (mode=dry_run\|enforce), `GET /api/attribution/coffee-npay-intents`, `GET /api/attribution/coffee-npay-intent-join-report`, 기존 `GET /api/coffee/intent/stats`. coffee.ts router 에 alias 등록.
    - join report 5종 status 분류 (joined_confirmed_order / pending_order_sync / no_order_after_24h / duplicated_intent / invalid_payload). 7일 window default + read-only.
    - 회귀 검증 7종 PASS (Codex curl): valid dry_run / preview_only=false reject / PII (delivery_memo) reject / enforce flag 미활성 reject / join report empty / list empty / row count + index 확인. total_rows 0 유지.
    - tsc PASS + backend production 재시작 (PID 99378 → 2837, 다운타임 ~3초). schema migration v2 적용 완료. enforce flag 미활성으로 운영 INSERT 0건.
    - 승인안에 GTM publish 전 체크리스트 7종 + endpoint 배포 전 체크리스트 7종 + rollback 5종 + 7일 모니터링 지표 8종 + live publish 중단 조건 6종 + dispatcher GTM Custom HTML tag 초안 (preview_only/is_simulation 가드 + 1초 throttle + sessionStorage dedup) 포함.
    - 다음 phase: TJ 6개 항목 (schema / validation / enforce 조건 / endpoint path / dispatcher 초안 / 모니터링 지표) 승인 → Step A-2 (env flag 활성) → A-3 (dispatcher GTM Preview) → A-4 (publish 소수 트래픽) → A-5 (7일 모니터링) → A-6 (GA4/Meta CAPI 보강 별도 승인 게이트).
