...
biocom GA4 raw events 적재는 정상이다. 어제 (2026-05-06) `events_20260506` table에 **70,294 rows** 적재됐고, 최근 6일 purchase 카운트는 49~88 사이 (transaction_id 중복 없음 = 1:1 매칭).

"허들러스 빅쿼리를 우리것으로 이전"의 실제 의미는 **권한 이전**이다. 데이터 자체는 여전히 `hurdlers-naver-pay.analytics_304759974` dataset에 적재되고 있고, 우리 SA `seo-656@seo-aeo-487113`에 dataset Read 권한 + job 실행은 우리 `project-dadba7dd-0229-4ff6-81c` 또는 `seo-aeo-487113` project에서 수행하는 패턴.

운영 backend dist는 **옛 버전 (jobProjectId 분리 미적용)** 이라 `sourceFreshness.ts` 의 biocom GA4 BigQuery freshness check가 `bigquery.jobs.create permission denied` 로 fail. 최신 sourceFreshness.ts deploy로 즉시 해결 가능.

AIBIO (`analytics_326993019`)는 우리 SA로 어떤 project에서도 dataset 미존재/권한 없음. AIBIO는 Supabase가 정본이고 BigQuery 연결은 별 트랙으로 검토.
...
|---:|---|---|---|---|---|---|---|---|---|
| 1 | [[#Phase1-Sprint1]] | auto_ready | Codex | 운영 backend `sourceFreshness.ts` 최신 dist deploy | biocom GA4 freshness check가 옛 dist 때문에 jobs.create 권한 부족으로 fail. 최신 코드는 jobProjectId 분리되어 있어 deploy만 하면 정상 | 백업 → scp dist/sourceFreshness.js → PM2 restart --update-env → freshness check 재실행 | 독립 (errorHandler hardening deploy와 동일 절차) | [[../backend/src/sourceFreshness.ts]] | NO (Yellow 이미 수용된 deploy 절차 안에서) |
...
| biocom GA4 dataset | `hurdlers-naver-pay.analytics_304759974` (asia-northeast3) |
| coffee GA4 dataset | `project-dadba7dd-0229-4ff6-81c.analytics_326949178` (asia-northeast3) |
| aibio GA4 dataset | (미연결, 권한 없음) |
| biocom 최신 events_ table | events_20260506 (last_mod 2026-05-07 00:39 UTC, 70,294 rows) |
