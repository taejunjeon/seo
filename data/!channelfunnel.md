...
Next document: [[../gdn/path-b-real-paid-click-actual-order-preview-result-20260510]] / [[../gdn/unpaid-vbank-controlled-evidence-exclusion-20260510]] / [[../gdn/path-b-p0-next-roadmap-20260510]] / [[../gdn/canary-capture-health-24h-20260508]]
...
harness_preflight:
  ...
  source_window_freshness_confidence:
    ...
    freshness: "events_20260506 last_mod 2026-05-07 00:39 UTC + 운영DB(PostgreSQL)/VM ledger read-only 2026-05-08 17:23 KST + Path B hash-only design 2026-05-08 20:38 KST + actual Google ad click no-send bridge PASS 2026-05-10 01:31 KST + unpaid vbank exclusion guard 2026-05-10 01:51 KST 반영"
    confidence: 0.93
```
...
| P0 | [[#Phase2-Sprint1]] | NPay/주문완료 흐름과 paid_click_intent 수집을 연결한다 | Google Ads 후보를 만들려면 주문완료와 클릭 ID가 같은 흐름에 남아야 한다 | paid_click_intent ledger, Path B order bridge, actual Google ad click no-send evidence를 분리해 본다 | actual ad click no-send bridge PASS. 미입금 가상계좌 guard 필요 | 85% | confirmed 결제완료만 후보로 남고 미입금 가상계좌는 block된다 | Codex: unpaid/confirmed guard 연결 | NO, Green | [[../gdn/path-b-real-paid-click-actual-order-preview-result-20260510]], [[../gdn/unpaid-vbank-controlled-evidence-exclusion-20260510]] |
...
#### A1. 미입금 가상계좌 주문을 channel/ROAS 후보에서 제외한다
- 무엇을 하는가: actual Google ad click no-send PASS 주문 중 가상계좌 미입금 row를 evidence로만 남기고 confirmed ROAS/upload 후보에서 제외한다.
- 왜 하는가: 주문완료 화면은 봤지만 입금이 없으므로 실제 매출이 아니다.
- 어떻게 하는가: `payment_status != confirmed` 또는 `payment_method=vbank AND paid_at missing`이면 `block_reason=unpaid_vbank_controlled_evidence`, `exclude_from_upload=true`, `exclude_from_budget_roas=true`, `send_candidate=false`로 처리한다.
- 성공 기준: dry-run에서 해당 주문은 후보 0건이고 block_reason만 남는다.
- 실패 시 다음 확인점: 운영DB 결제 상태 필드, VM Cloud order bridge freshness, 가상계좌 paid_at 반영 방식.
- 승인 필요 여부: NO, Green.
- 산출물: guard 설계/fixture/dry-run 결과.
- 진척률에 미치는 영향: [[#Phase2-Sprint1]] 85% -> 92%.
- 의존성: 독립 실행 가능.
