...
| 20 (이전) | A-4 publish 결정 sprint | **추천 가능 — TJ 승인만 남음** | TJ + Codex | A-4 GTM Production publish 결정 — sprint 19.4 PASS 후 진입 | sprint 19.3 + 19.4 통해 dispatcher v2.1 의 12/12 자동 검증 + backend list endpoint bug fix 모두 완료 | publish 결정 단일 문서 [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] §6 체크리스트 11개 중 **10/11 PASS** (C-1~C-10 모두 ✅) — **C-11 TJ 명시 publish 승인만 남음**. publish 시 GTM Coffee container `GTM-5M33GC4` default workspace 18 에 dispatcher v2.1 tag publish. publish 후 5일 default 모니터링 (조기 3일 게이트 가능) | [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] | YES, publish 명시 승인 (Red Lane) |
| 20 | A-4 publish 결정 sprint | 보류 | TJ + Codex | A-4 GTM Production publish 결정 — dispatcher v2.1 Preview 재검증 PASS 후 진입 | A-3 v2 evidence + dispatcher v2.1 design 완료. publish 의 11개 체크리스트 중 C-1~C-4 (재검증 PASS 의존) 미충족 — 본 시점 보류 | publish 결정 단일 문서 [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] §6 체크리스트 11개 PASS 후 진입. 추천 게이트: imweb_order_code capture rate ≥95% + enforce_deduped ratio ≤5% + payment_button_type null in confirm_to_pay = 0 + invalid_origin/rate_limited/preview_only_violation/is_simulation_blocked/pii_rejected = 모두 0 + pm2 restart 안정 + TJ 명시 승인 | [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] | YES, publish 명시 승인 별도 |
