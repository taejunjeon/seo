...
| 15 | VM 배포 sprint | 완료 | Codex | VM production backend 배포 사전 체크리스트 작성 | A-3 재개 선결 조건 — VM 외부 endpoint 가 살아 있어야 dispatcher 자동 검증이 의미 있다 | env diff 0 / schema migration idempotent (VM 테이블 부재) / rsync 권한 우회 패턴 (`--rsync-path="sudo -u biocomkr_sns rsync"`) / rollback (SQLite snapshot + .env exclude) / 7-step pre-flight + 4-step post-deploy smoke 정리. 미해결 1건: 로컬 rsync 미설치 → `brew install rsync` 필요 | [[coffee-vm-backend-deploy-precheck-20260502]] | YES, 배포 명령 실행은 별도 승인 게이트 |
