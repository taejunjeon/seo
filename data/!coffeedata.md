...
| 19.3 | dispatcher v2.1 chrome 재검증 sprint | 대기 | TJ + Codex | dispatcher v2.1 chrome 측 동작 (GTM Preview + sessionStorage buffer + funnel-capi v3 retry capture) 검증 | sprint 19.2 partial 의 chrome 부분 분리. agent 가 chrome 자동화 불가하므로 TJ 가 chrome 진행 시점에 진입 | A-3 runbook 의 v2.1 재검증 절차 (Step T1'~T5') 따라 GTM Preview → site 진입 → all-in-one snippet paste → PC NPay click 1-2회 → dispatcher state 캡처. 결과로 imweb_order_code capture rate / enforce_deduped ratio / payment_button_type null in confirm_to_pay 실측 | [[coffee-npay-intent-a3-gtm-preview-dispatcher-runbook-20260502#a-3-v2-1-재검증]] | YES, Yellow Lane 1회 승인 + chrome 진행 |
