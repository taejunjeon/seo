...
| 19.4 | backend payment_button_type fix sprint | 대기 | Codex + TJ | backend list endpoint 의 payment_button_type NULL 매핑 bug fix | sprint 19.3 발견. ledger 의 enforce POST 응답 ledger_row_preview 에는 npay 정상이지만 list endpoint (`GET /api/attribution/coffee-npay-intents`) 응답 row 의 payment_button_type=NULL. publish 전 fix 필수 (EG-4 정상화) | backend `coffeeNpayIntentLog.ts` 의 list endpoint row→JSON 변환 함수 (line 823 부근) 점검 + payment_button_type 매핑 추가 + tsc + VM 재배포 (sprint 16/17 패턴 재사용) + 재검증 | [[harness/coffee-data/LESSONS#coffee-lesson-012]] | YES, backend code 변경 + VM 배포 별도 승인 |
