...
| 22 | A-6 외부 플랫폼 보강 전송 sprint | **대기 (A-5 closure 후)** | TJ + Codex | GA4 MP / Meta CAPI 보강 전송 — A-5 5일 default monitoring PASS 후 별도 승인 게이트 (Red Lane) | ledger 가 deterministic mapping source-of-truth 로 인정됨 + capture rate ≥95% 운영 측정 PASS 후만 진행 | A-5 closure 후 ledger 의 imweb_order_code → imweb_orders.order_code → tb_iamweb_orders 의 phone/transaction_id 결정성 join → GA4 MP / Meta CAPI 의 NPay purchase 보강 전송. 단 GA4 synthetic transaction_id 한계 (NPAY - id - epoch_ms 패턴) 와 부합 검증 필요. design 단계 sprint 진입 시 신규 결정 문서 작성 | [[coffee-a4-publish-decision-and-dispatcher-v21-20260502]] / [[#GA4 NPay synthetic transaction_id 한계]] | YES, 외부 send 는 명시 승인 별도 (Red Lane) |
