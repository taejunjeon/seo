...
| 11 | [[#Phase3-Sprint6]] | 완료 | Codex | 더클린커피 NPay 버튼 DOM 조사 + intent beacon preview-only 설계안을 작성한다 | 과거분 자동 매칭이 약하므로 미래분에서 click → 결제 사이의 의도를 분리해 두어야 한다. 다만 live publish 전 design + preview 절차 확정이 필요하다 | `shop_view/?idx=1` 정적 HTML 분석으로 PC `#naverPayWrap`, mobile `._btn_mobile_npay`, 공통 진입점 `SITE_SHOP_DETAIL.confirmOrderWithCartItems('npay', url)` 확인. function wrap + sessionStorage buffer + console.log 만 쓰는 preview snippet 초안과 5개 시나리오 검증 절차, live publish 금지 가드를 작성했다 | [[#Phase3-Sprint6]] / [[coffee-npay-intent-beacon-preview-design-20260501]] | NO, 설계 + preview only |
...
| Phase4 | [[#Phase4-Sprint8]] | 공통 Growth Data Harness 편입 | Codex + Claude | 88% / 0% | [[#Phase4-Sprint8]] |
...
4. preview-only 설계안을 [[coffee-npay-intent-beacon-preview-design-20260501]] 에 작성했다. selector 옵션 4종 비교, function wrap 패턴(D 채택, B 보조), beacon payload(PII 제외 14필드), sessionStorage buffer + console.log, 시나리오 5개 검증 절차, live publish 금지 가드를 정리했다.
...
| auditor script v1 고도화 | 현재 helper는 changed file 중심이라 report 숫자 대조는 수동이다 | `scripts/coffee_harness_audit.py`가 `!coffeedata` 숫자와 최신 report 숫자를 자동 대조하게 확장한다 | 숫자 mismatch 자동 감지 |
