...
바이오컴 2026년 1분기 공동구매는 회사 공유본 기준 `2,253건 / 645,941,680원`, 로컬 산출치 기준 `1,668건 / 512,906,191원`이다. 로컬은 매출 기준 79.4%를 포착했고, 차이의 핵심은 1월 초 주문 헤더 누락과 아랑 4차 미분류다. 1월 초 주문 헤더 누락은 주문 부재가 아니라 아임웹 v2 `shop/orders` API 백필 범위 문제로 판단한다. 운영 DB `tb_iamweb_users`에는 1월 1~7일 주문 헤더가 남아 있다. Toss 정산 원장도 1월 초 공동구매성 누락 주문 170개 전부를 매칭하므로 결제 검증 보조 원천으로 쓸 수 있다.
...
- 2026년 1~3월 바이오컴 공동구매 내역은 회사 공유본 `data/공동구매_26년 1분기.xlsx`와 캠페인/회차 단위 대사를 완료했다. 회사 기준은 `2,253건 / 645,941,680원`, 로컬 기준은 `1,668건 / 512,906,191원`이다. 차이는 `585건 / 133,035,489원`이며, 핵심 원인은 1월 초 주문 헤더 누락과 아랑 4차 미분류다. 회사 공유본에는 주문번호가 없어 주문번호 1:1 대사는 아직 아니다.
- 1월 초 주문 헤더 누락 원인은 로컬 `imweb_orders`가 아임웹 v2 `shop/orders` API 백필 결과만 갖고 있기 때문이다. 2026-04-21 19:27 KST 현재 API는 `totalPage=165`까지만 반환하고, 마지막 페이지의 가장 오래된 주문은 `2026-01-18T11:21:57Z`다. `offset=166` 이후는 빈 응답이다. 반면 PlayAuto 상품 라인에는 2026-01-01 ~ 2026-01-07 주문번호가 있고, 그중 `site` 미확정 716개 주문번호 중 713개가 운영 DB `tb_iamweb_users`에 매칭된다. Toss 정산 원장도 716개 중 605개를 매칭하고, 공동구매성 누락 주문 170개는 170개 전부 매칭한다. 따라서 복구는 운영 DB 장기 보관 원천 기반 백필을 1순위로 하고, Toss는 카드/계좌/가상계좌 결제금액 교차검증 원천으로 써야 한다.
...
5. [Codex/TJ] `공동구매내역.md`의 2026년 1~3월 로컬 산출치(`1,668건 / 512,906,191원`)와 회사 공동구매 추출본(`2,253건 / 645,941,680원`)을 맞춘다. 상태: 캠페인/회차 단위 1차 완료. 다음 단계는 회사 추출본에 주문번호를 붙여 주문번호 기준으로 닫는 것이다.
