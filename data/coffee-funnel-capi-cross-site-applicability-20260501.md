...
**결론**: biocom 에서도 NPay click 시 backurl 의 query string 이 imweb 측 raw_data 에 자동 보존될 자리가 없다. **biocom 도 트랙 (B) ledger + 휴리스틱이 현실적**. (A-) URL 한정 매핑은 chrome devtools 정찰 (1-A/1-B 단계) 결과 redirect URL chain 에 query 가 살아 있어야 가능.
