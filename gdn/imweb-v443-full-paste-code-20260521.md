# Imweb v4.4.3 전체 붙여넣기 코드

작성 시각: 2026-05-21 19:05 KST
기준일: 2026-05-21
문서 성격: 아임웹 운영 반영용 전체 코드 출력 / 헤더 상단 + 푸터 전체
...
harness_preflight:
  ...
  project_harness_read:
    - gdn/imweb-v443-paste-patch-20260521.md
    - gdn/imweb-v443-click-id-fixture-smoke-result-20260521.md
  required_context_docs:
    - docurule.md
  lane: Green for full paste-code generation; Red for Imweb production custom-code edit
  allowed_actions:
    - live HTML read-only extract
    - paste-code document generation
    - local JavaScript syntax check
    - no-send/no-write fixture verification
  forbidden_actions:
  ...
  source_window_freshness_confidence:
    source: biocom.kr live HTML read-only + TJ님 provided code markers + local fixture 7/7 PASS
    window: 2026-05-21 current operating HTML snapshot
    freshness: 2026-05-21 19:05 KST
    confidence: 0.94
```

## 10초 요약

아래는 찾기/교체 없이 그대로 붙여넣는 전체 코드다. 아임웹의 `헤더 코드 상단`에는 첫 번째 코드블록 전체를 넣고, `푸터 코드`에는 두 번째 코드블록 전체를 넣는다.

운영 반영 전에는 기존 헤더 상단과 푸터 코드를 각각 백업한다. 이 문서는 코드 출력만 만든 것이며, 아직 운영에 반영하지 않았다.
...
(function () {
  ...
  var CONFIG = {
    snippetVersion: '2026-05-15-server-payment-decision-guard-v3-1-1',
    ...
    decisionCachePrefix: '__biocom_payment_decision_guard_v311__:',
    wrapPollMs: [0, 50, 100, 200, 500, 1000, 2000, 3500, 5000, 8000],
    customEventRetryMs: [0, 150, 400, 800, 1500, 2500],
    logPrefix: '[biocom-server-payment-decision-guard-v3.1.1]'
  };
  ...
  if (window.__BIOCOM_SERVER_PAYMENT_DECISION_GUARD_V311_INSTALLED__) return;
  window.__BIOCOM_SERVER_PAYMENT_DECISION_GUARD_V311_INSTALLED__ = true;
...
(function () {
  ...
  var CONFIG = {
    pixelId: '1283400029487161',
    snippetVersion: '2026-05-15-biocom-meta-funnel-fallback-block4-v0-4',
    ...
    /*
      ...
      v0.4 sends an image beacon fallback.
    */
    nativeObserveMs: 1200,
    postCheckMs: 900,

    dedupePrefix: '__biocom_meta_funnel_fallback_sent_v0_4__:',
    eventIdStorePrefix: '__biocom_meta_funnel_event_id_v0_4__:',
    logKey: '__biocom_meta_funnel_fallback_v04',
    ...
    /*
      Hard guard.
      v0.4 must never send these events.
    */
    forbiddenEvents: {
      PageView: true,
      ViewContent: true,
  ...
  if (window.__BIOCOM_META_FUNNEL_FALLBACK_V04_VERSION__ === CONFIG.snippetVersion) return;
  window.__BIOCOM_META_FUNNEL_FALLBACK_V04_VERSION__ = CONFIG.snippetVersion;
  ...
  function log() {
    if (!isDebugMode()) return;
    try {
      console.log.apply(console, ['[biocom-block4-v0.4]'].concat([].slice.call(arguments)));
    } catch (error) {}
  }
  ...
  function buildCustomData(eventName, baseData) {
    ...
    */
    out.fallback_source = 'biocom_block4_v0_4';
  ...
  function sendFallbackLater(eventName, dedupeKey, customData) {
    window.setTimeout(function () {
      sendImageBeacon(eventName, dedupeKey, customData);
  ...
  function boot() {
    ...
    */
    ...
    log('Block4 v0.4 image beacon fallback booted');
  }
...
- `node scripts/imweb-v443-click-id-fixture.mjs`: 7/7 PASS
- no-send/no-write/no-publish: 유지
