...
2026-05-12 Option C 추가: 운영DB `tb_iamweb_users PAYMENT_COMPLETE`는 **biocom NPay actual confirmed**에는 primary로 쓴다. 그러나 `thecleancoffee`는 `tb_iamweb_users` 안에서 site key가 증명되지 않고 VM Cloud `imweb_orders` coffee NPay 주문번호 337건과 운영DB 주문번호 매칭도 0건이므로, summary API에서 actual included로 올리지 않는다. coffee는 site 격리 완료 전까지 `bridge_pending`으로 표시하고, Imweb v2/VM Cloud `imweb_orders`, 운영DB `tb_playauto_orders(shop_name='아임웹-C')`, 운영DB `tb_sales_toss(store='coffee')`, 엑셀을 cross-check로 본다.
...
| 우선순위 | source | 필터 |
|---|---|---|
| primary | 운영DB `tb_iamweb_users` | `payment_status='PAYMENT_COMPLETE'` AND `cancellation_reason/return_reason` 빈값 AND `final_order_amount > 0` |
...
| pending primary (thecleancoffee) | Imweb v2/VM Cloud `imweb_orders(site='thecleancoffee', pay_type='npay')` + 주문 단위 bridge. 운영DB `tb_iamweb_users` actual included 금지 |
...
2. thecleancoffee: site 격리 완료 전에는 `bridge_pending`. Imweb v2/VM Cloud `imweb_orders`와 PlayAuto/Toss/Excel cross-check를 분리해서 보고, `tb_iamweb_users` actual included는 금지한다.
