...
| primary candidate (thecleancoffee) | VM Cloud/로컬DB `imweb_orders(site='thecleancoffee')` | Imweb v2 API `type=npay` / `pay_type='npay'` / `payment_amount > 0` / 취소·반품·교환 status 제외 |
...
| primary candidate (thecleancoffee) | Imweb v2/VM Cloud `imweb_orders(site='thecleancoffee', pay_type='npay')` + `payment_amount > 0` + 취소/반품/교환 status 제외. 운영DB `tb_iamweb_users` actual included 금지 |
...
2. thecleancoffee: `tb_iamweb_users`는 actual source로 금지. 대신 Imweb v2/VM Cloud `imweb_orders(site='thecleancoffee', pay_type='npay')`를 primary candidate로 쓰고, 취소/반품/교환 status 제외 + status freshness + blank status count를 붙인다. live summary included는 code patch/test/deploy 후에만 가능하다.
