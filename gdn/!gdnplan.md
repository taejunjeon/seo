...
최근 업데이트: 2026-04-25 23:50 KST
기준일: 2026-04-23
버전: v1.5-npay-return-docs
문서 성격: 로드맵
...
| Phase3 | [[#Phase3-Sprint5]] | 플랫폼 ROAS와 내부 ROAS 비교표 | Codex | 80% / 55% | [[#Phase3-Sprint5\|이동]] |
...
5. PMax/GDN 계열은 내부 confirmed 매출과 직접 매칭되지 않으므로 증액 금지다. 현금 소진이 부담이면 임시로 20~30% 감액을 검토한다.
...
2. [Codex] 임시 운영 룰을 문서화한다. 완료: 2026-04-25 23:34 KST 기준 `광고 전체 OFF 보류`, `증액 금지`, `Google ROAS 신뢰 중지`, `PMax/GDN 20~30% 감액 조건부 검토`로 정리했다.
...
1. [TJ] Google Ads 증액을 중지한다. Google Ads ROAS와 `Conv. value`는 임시로 예산 증액 근거에서 제외한다.
2. [TJ] 아임웹 관리자와 네이버페이 파트너센터에서 NPay 결제 완료 후 return URL 또는 완료 후 이동 설정이 있는지 확인한다.
3. [TJ] Google Ads UI에서 `구매완료` action `7130249515`의 `Primary` 여부와 `send_to=AW-304339096/r0vuCKvy-8caEJixj5EB`를 확인한다.
4. [TJ] 확인 후 `구매완료`를 purchase primary에서 내릴지 승인한다. 승인 전에는 Google Ads 설정을 바꾸지 않는다.
5. [Codex] NPay intent capture endpoint와 GTM [118] sendBeacon draft를 설계한다. Option A 성공 여부와 무관하게 증거 수집 기반으로 쓸 수 있다.
...
7. [Codex] 최신 GTM snapshot을 재생성하고 v137 이후 Google Ads/NPay/purchase 태그 상태를 확인한다.
