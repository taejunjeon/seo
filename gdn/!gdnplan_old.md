...
paid_click_intent GTM Preview 실행 결과는 [[paid-click-intent-gtm-preview-result-20260506]]에 기록했다.
paid_click_intent HTTPS receiver 접근 검증 결과는 [[paid-click-intent-receiver-access-result-20260506]]에 기록했다.
Google Ads landing-session click id 분모 분석은 [[google-ads-landing-clickid-analysis-20260506]]에 기록했다.
paid_click_intent 운영 GTM 게시 승인안은 [[paid-click-intent-gtm-production-publish-approval-20260506]]에 기록했다.
운영 게시 후 24h/72h 모니터링 템플릿은 [[paid-click-intent-post-publish-monitoring-template-20260506]]에 기록했다.
production receiver POST smoke 결과는 [[paid-click-intent-production-receiver-post-smoke-20260506]]에 기록했다.
production receiver route 배포 승인안은 [[paid-click-intent-production-receiver-deploy-approval-20260506]]에 기록했다.
no-write receiver는 주문 원장 fill-rate를 직접 개선하는 단계가 아니라 live payload validation 단계다. receiver count는 DB row가 아니라 마스킹된 access/observability 기준으로만 보고, 실제 paid_click_intent ledger write는 별도 승인 이후 판단한다.
...
2026-05-06 15:45 KST Preview only 실행 결과, `gclid/gbraid/wbraid` storage 보존과 payload 생성은 PASS, Node-side no-send receiver validation도 PASS였다.
2026-05-06 15:58 KST 임시 HTTPS tunnel 재검증 결과, 브라우저 receiver 직접 호출도 세 케이스 모두 `200 ok=true`로 통과했다.
2026-05-06 16:12 KST GA4 BigQuery read-only 분석 기준, 최근 7일 Google Ads 증거 세션 6,879개 중 URL 또는 `collected_traffic_source`에 Google click id가 남은 세션은 6,724개, 보존률은 97.75%다.
따라서 광고 URL/auto-tagging 자체가 핵심 병목은 아니다. 병목은 랜딩 이후 checkout/NPay intent/결제완료 주문 원장까지 click id가 이어지지 않는 것이다.
다음 실행은 Google Ads 전송이 아니다. production receiver POST smoke에서 `404 Route not found`가 확인됐으므로, 먼저 `att.ainativeos.net`에 `paid-click-intent/no-send` receiver route를 배포할지 판단해야 한다. 배포 승인안은 [[paid-click-intent-production-receiver-deploy-approval-20260506]]에 있다.
배포 승인은 no-write/no-platform-send receiver에 한정한다. TEST POST 200이 확인되면 receiver-enabled GTM publish와 24h/72h 모니터링까지 연속 진행할 수 있지만, 이 연속 진행은 TJ님이 명시 승인한 경우에만 해당한다.
...
2026-05-06 최신 no-send 재실행 기준 운영 결제완료 주문은 623건이고, Google click id가 남은 주문은 5건, 전체 보존률은 0.8%다. 반면 Google Ads 랜딩 세션 기준 click id 보존률은 97.75%다. `imweb_operational`은 fresh로 복구됐고 source lag는 6.5시간이다.
...
| Phase4 | [[#Phase4-Sprint6]] | 전환 오염 제거와 보정 | TJ + Codex + Claude Code | 45% / 0% | [[#Phase4-Sprint6\|이동]] |
...
5. [[paid-click-intent-gtm-preview-result-20260506]] 기준 GTM Preview only 실행을 완료했다. `gclid/gbraid/wbraid` storage 보존과 payload 생성은 통과했고, Node-side no-send receiver validation도 통과했다.
6. [[paid-click-intent-receiver-access-result-20260506]] 기준 임시 HTTPS tunnel 재검증도 통과했다. 세 click id 케이스 모두 브라우저에서 no-send receiver `200 ok=true`를 받았다.
7. [[google-ads-landing-clickid-analysis-20260506]] 기준 Google Ads landing-session click id 분모 분석을 완료했다. 최근 7일 Google Ads 증거 세션 6,879개 중 6,724개에 click id가 남아 보존률은 97.75%다.
8. [[paid-click-intent-gtm-production-publish-approval-20260506]]에 운영 GTM publish 승인안을 작성했다. 이 문서는 승인안이며 실제 publish는 하지 않았다.
9. [[paid-click-intent-post-publish-monitoring-template-20260506]]에 publish 후 24h/72h 모니터링 템플릿을 작성했다.
10. [[paid-click-intent-production-receiver-post-smoke-20260506]] 기준 production receiver POST smoke를 실행했다. 결과는 `404 Route not found`다.
11. [[paid-click-intent-production-receiver-deploy-approval-20260506]]에 receiver route 배포 승인안을 작성했다. 실제 backend 운영 deploy는 하지 않았다.
12. 다음은 Google Ads confirmed purchase Red Lane 실행이 아니라, TJ님이 receiver route 배포 후 receiver-enabled publish로 갈지, storage-only GTM publish로 갈지 판단하는 것이다.
