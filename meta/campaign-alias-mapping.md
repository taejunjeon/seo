...
- 따라서 `/iiary02` 랜딩 경로만 남은 D급 row는 campaign-level까지만 매칭하고, 특정 광고세트/광고로 자동 배정하지 않는다.
- 숫자 `meta_campaign_id/meta_adset_id/meta_ad_id`가 남은 A급 row만 광고세트 단위로 해석하고, adset ID 기준으로 종대사/음과검 서비스 라벨을 붙인다.
...
- 따라서 UTM 진단 화면에서는 광고세트와 현재 하위 광고를 Section C 미맵핑에서 Section B 보완 필요로 올린다.
- 단, URL에 숫자 `campaign_id/adset_id/ad_id`가 없으므로 Section A 확정이나 ATT ROAS 산식 반영은 하지 않는다.
