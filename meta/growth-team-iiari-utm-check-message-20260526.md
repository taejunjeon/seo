...
## 그로스팀 회신 반영 — 2026-05-26

그로스팀 파트장 정정으로 `원본/사본`의 소재 구분이 확인됐다. 단순 복제 실수가 아니라 같은 이아리 공구 안에서 회사 서비스 소재를 나누는 구조다.

- `meta_biocom_iiari_260518 - 종대사`
  - 이전 표시: `meta_biocom_iiari_260518 - 사본`
  - 소재 의미: 종합대사기능분석 서비스(종대사)
  - 라이브 광고명: `meta_biocom_iiari_acid_260518`
  - 내부 원장 A급 광고세트 근거: adset ID `120245956430970396`, 26 rows, 결제완료 11건, 결제완료 금액 4,436,570원
  - Ads Manager 화면값: 7일 클릭 기준 웹사이트 구매 14건, 결과값 5,109,770원, 결과 ROAS 4.67
- `meta_biocom_iiari_260518`
  - 소재 의미: 음식물과민증분석 서비스(음과검)
  - 라이브 광고명: `meta_biocom_iiari_Igg_260518`
  - 내부 원장 A급 광고세트 근거: adset ID `120245700952890396`, 89 rows, 결제완료 32건, 결제완료 금액 10,569,200원
  - Ads Manager 화면값: 7일 클릭 기준 웹사이트 구매 45건, 결과값 14,096,500원, 결과 ROAS 3.97

해석:

1. 이아리 공구 캠페인은 campaign ID `120245003319500396` 단위로는 확정해서 볼 수 있다.
2. 숫자 ID가 남은 A급 row는 광고세트 단위에서 `종대사 소재`와 `음과검 소재`로 나누어 읽을 수 있다.
3. `사본`이 붙었던 광고세트는 종대사로 이름이 수정되었으므로, 다음 fresh API/export부터 이름 기준도 더 직관적으로 맞을 가능성이 높다.
4. 다만 D급 16건은 여전히 숫자 campaign/adset/ad ID가 없으므로, 이번 회신만으로 `종대사` 또는 `음과검` 어느 쪽에 자동 배정하면 안 된다.
5. 이아리 공구가 2026-05-26 종료 예정이므로, 종료 이후 2차 활용 광고를 만들 때는 신규 광고부터 고유 `campaign_alias`와 숫자 ID URL Parameters가 확실히 남도록 세팅하는 것이 좋다.
